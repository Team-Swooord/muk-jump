...
| 도약 | 높은 먹발 | 일반 자동 점프 정점에서 힘 40%의 2단점프, 먹떼 공용 쿨다운 12초 |
...
- 도약: `준비시간 -1.5%×3(결실: 점프 아이템 종료 방어막) / 점프 힘 +1%×4(결실: 벽 매달림 1.2초) / 점프 높이 +1.5625%×3(결실: 40% 2단점프·공용 12초)`
...
뛸 수 있게 합니다. `높은 먹발 결실`은 일반 자동 점프 정점에서 40% 힘의 2단점프를
열고 먹떼 전체가 12초 쿨다운을 공유합니다. `숨 고르기 결실`은 본체를 체력 1로
