...
- 그린 발판은 3.4초 동안 선명하게 남고 이후 1.1초 동안 처음 그린 쪽부터 먹이
  마르듯 사라집니다. 처음 최대 먹 용량은 실전 발판 약 1.5~2획인 4.8m이며, 이를 넘겨도
...
- 본체는 기본 내구 1칸으로 시작하고, 영구 성장의 먹피 경로를 열면
  2→3→4칸까지 늘어납니다. 분신은 기본 2/2 체력이고 먹피 경로의 마지막
  `먹피 결실`을 선택 적용하면 3/3이 되며, 각 먹방울 머리 위에 자기 최대치만큼 분리된
  체력 칸으로 표시됩니다.
...
- 화면 좌우의 보이지 않는 경계에 닿으면 안쪽 중앙으로 큰 폭으로 튀어 나오며,
  ...
  않아 무한 벽타기가 되지 않습니다. `돋는 먹발 결실`을 선택하면 이 즉시 반동 대신
  좌우 벽에 붙은 뒤 일반 발판과 같은 자동점프로 안쪽을 향해 뜁니다. 경계 안쪽
  0.95m 먹선 금지 띠도 유지합니다.
...
- 고도에 따라 고요한 산길·바람 고개·먹비 골짜기·낙묵 협곡을 지나 먹빛 성문·월련 성해·천하수의 무한 구간으로 이어집니다.

## 아이템

| 아이템 | 효과 |
|---|---|
| 먹물방울 | 살아 있는 먹떼 전체가 50m 상승하고 상승 중 장애물 피해를 받지 않음. `고른 박자 결실` 선택 시 획득 개체는 상승 종료 뒤 방어막 1회 획득 |
| 황금 붓 | 8초 동안 새 획을 그려도 오래된 획이 밀려나지 않음. 게이지 채움만 금빛으로 표시하고 종료 뒤 총량을 한 번에 정산. `가는 붓끝 결실` 선택 시 획득 즉시 방어막 1회 획득 |
| 먹 방어막 | 장애물 피해 또는 화면 하단 추락을 1회 막음. 보유 중 재획득은 소비하지 않음 |
| 먹분신 | 기본 한 마리 추가. `먹떼 결실` 선택 시 획득 1회 최대 2마리. 각 분신은 기본 체력 2칸이며 `먹피 결실` 선택 시 3칸. 생성 순간 좌우·위로 서로 다르게 튀어 먹떼가 일렬로 굳지 않음. 전체 생존 상한 24마리 |
...
노드를 모두 환불하되 누적 거리와 먹빛 수령 기록은 보존합니다. 선택한 갈래는 선명한
가지와 열매로 유지되고, 같은 분류의 나머지 두 갈래는 옅어진 가지 위로 가시덩굴이
감겨 현재 적용되지 않는 줄기임을 보여 줍니다. 가시는 상태 표시일 뿐 노드 확인이나
무료 교체를 막지 않습니다.
...
flowchart TB
    ...
    M --> S["생존 · 13개"]
    S --> SA["먹피<br/>본체 체력 1→4<br/>결실: 분신 체력 2→3"]
    S --> SB["숨 고르기<br/>무적 +0.16초<br/>결실: 부활 + 50m 상승"]
    S --> SC["먹발 버팀<br/>수평 밀림 -18%<br/>결실: 분신 생성 1→2"]

    M --> J["도약 · 13개"]
    J --> JA["준비시간 -4.5%<br/>결실: 점프 아이템 종료 방어막"]
    J --> JB["점프 힘 +4%<br/>결실: 좌우 벽 자동점프"]
    J --> JC["점프 높이 +4.69%<br/>결실: 2단점프"]

    M --> I["먹 운용 · 13개"]
    I --> IA["최대 먹 4.8→10.2m<br/>결실: 20.4m로 ×2"]
    I --> IB["획당 먹 소모 -8%<br/>결실: 황금 붓 획득 방어막"]
    I --> IC["게이지 회복 +40%<br/>결실: 회복 +10% 포함"]
```
...
| 분류 | 결실 | 실제 적용 효과 |
|---|---|---|
| 먹 운용 | 넓은 벼루 | 현재까지 성장한 최대 먹 게이지 용량을 2배로 만듦(최대 20.4m) |
| 먹 운용 | 가는 붓끝 | 황금 붓 획득 즉시, 획득한 먹방울에게 1회용 방어막 제공 |
| 먹 운용 | 마르는 먹 | 먹 게이지 회복 속도 +10%(해당 갈래 완성 +40%) |
| 도약 | 고른 박자 | 먹물방울 점프 상승이 끝나면 획득한 먹방울에게 1회용 방어막 제공 |
| 도약 | 돋는 먹발 | 본체·분신이 좌우 벽에 상시 붙고 일반 발판처럼 화면 안쪽으로 자동점프 |
| 도약 | 높은 먹발 | 모든 일반 자동 점프의 정점 직전 힘 40%의 연속 2단점프 |
| 생존 | 숨 고르기 | 본체가 장애물·추락으로 쓰러질 때 판당 1회 체력 1로 부활해 먹물 연출과 함께 50m 상승 |
| 생존 | 먹피 | 모든 먹분신의 최대 체력을 2→3칸으로 증가 |
| 생존 | 먹떼 | 먹분신 아이템 1회 생성 수를 1→2마리로 증가 |

- 생존: `본체 최대 체력 +1×3(결실: 분신 체력 +1) / 피격 뒤 무적 +0.04초×4(결실: 본체 1회 부활·50m 상승) / 피격 수평 밀림 -6%×3(결실: 먹분신 생성 최대 +1)`
- 도약: `준비시간 -1.5%×3(결실: 점프 아이템 종료 방어막) / 점프 힘 +1%×4(결실: 좌우 벽 상시 자동점프) / 점프 높이 +1.5625%×3(결실: 매 자동점프 40% 2단점프)`
- 먹 운용: `최대 먹 용량 +37.5%×3(결실: 현재 용량 ×2) / 획당 먹 소모 -2%×4(결실: 황금 붓 획득 방어막) / 게이지 회복 +10%×4`

같은 분류의 세 완성값은 동시에 적용되지 않습니다. 생존은 최대 체력형·피격 방어형·
먹떼형, 도약은 빠른 박자형·벽 활용형·2단 도약형, 먹 운용은 최대 용량형·아이템
방어형·빠른 순환형 중 하나를 선택합니다. `넓은 벼루 결실`은 성장한 먹 용량을
2배로 만들고, `가는 붓끝 결실`은 황금 붓 획득 개체에게 방어막을 줍니다.
`마르는 먹 결실`은 게이지 회복 속도를 10% 더 높입니다. `고른 박자 결실`은 점프
아이템 상승 종료 뒤 방어막을 주며, `돋는 먹발 결실`은 본체와 모든 분신이 이동
방향·높이와 관계없이 좌우 벽을 일반 발판처럼 사용해 안쪽으로 다시 자동점프하게 합니다.
`높은 먹발 결실`은 본체와 모든 분신의 일반 자동 점프마다 상승
속도가 15% 남은 정점 직전에 40% 힘의 2단점프를 한 번씩 이어 줍니다. `숨 고르기 결실`은 본체를 체력 1로
판당 한 번 부활시키고 먹물 상승 연출과 함께 현재 위치 기준 50m를 솟게 합니다. 추락 부활은 먼저 화면 안쪽
안전선으로 복귀한 뒤 상승합니다. 황금 붓은 용량 제한만 잠시 풀고 자연 소멸은 계속 진행합니다.
...
- 24마리 먹떼의 최고 생존자 카메라, 하위 중앙 진행 기준, 본체 1→4칸·분신 2→3칸 체력, 팝콘형 분신 생성 탄성, VFX 예산 관리
...
- [다른 개발자·AI 인수인계용 마스터 게임 바이블](docs/MUKJUMP_MASTER_GAME_BIBLE.md)
