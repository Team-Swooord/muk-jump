...
- `먹분신`은 기본 한 마리를 만들며, 영구 성장 `먹떼 I~IV`를 열 때마다 한 번의
  획득으로 생기는 수가 한 마리씩 늘어 최대 다섯 마리를 함께 만듭니다.
...
- 첫 플레이 프레임에는 해태의 측면 경고와 벽 하강을 한 번 보여 주고, 첫 30m 동안
  발판 그리기와 먹분신을 함께 익히게 합니다.
- 화면 좌우의 보이지 않는 경계는 상승 중 부딪히면 위쪽 힘을 끊고 안쪽으로
  돌려보냅니다. 경계 안쪽 0.95m에는 발판이 확정되지 않아 가장자리 먹선과 벽 사이를
  반복해서 타는 길은 만들 수 없습니다. 하강 중 접촉도 별도 매달림 없이 화면
  안쪽으로 반사됩니다.
...
| 먹분신 | 기본 한 마리 추가. 먹떼 성장 단계마다 획득 1회 생성 수 +1, 최대 5마리. 전체 생존 상한 24마리 |
...
전체 39개 노드의 비용은 모두 먹빛 1개이고, 한 갈래는 처음부터 끝까지 하나의
능력치만 단계적으로 올립니다. 마지막 열매도 별도 비기나 장착물이 아니라 같은
능력치의 4단계이며, 해금한 모든 수치는 항상 함께 적용됩니다.

- 생존: `최대 체력 +1×4 / 피격 뒤 무적 +0.04초×5 / 먹분신 아이템 생성 수 +1×4`
- 도약: `준비시간 -1.5%×4 / 점프 힘 +1%×5 / 점프 높이 +1.5625%×4`
- 먹 운용: `총 먹자리 +37.5%×4 / 획 소모 -2%×5 / 게이지 회복 +10%×4`

완성하면 체력은 1→5칸, 먹분신 아이템은 1회 1→5마리, 총 먹자리는
3.2→8.0m, 획 소모는 -10%, 회복 속도는 +40%가 됩니다. 먹 회복 성장은 선명
...
- 24마리 먹떼의 최고 생존자 카메라, 하위 중앙 진행 기준, 개체별 체력, VFX 예산 관리
