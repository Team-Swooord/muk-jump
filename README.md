...
| 먹분신 | 기본 한 마리 추가. `먹떼 결실` 해금 시 획득 1회 최대 2마리, `먹피 결실` 해금 시 각 분신 체력 2칸. 전체 생존 상한 24마리 |
...
`결실` 열매가 나무 꼭대기에 맺힙니다. 결실은 마지막 수치 성장과 함께 생존·도약·
먹 운용의 전용 패시브를 영구 적용하며, 해금한 모든 수치와 패시브는 항상 함께
적용됩니다.

```mermaid
flowchart TB
    M["먹빛 1개 = 열매 1개<br/>총 39개"]

    M --> S["생존 · 13개"]
    S --> SA["먹피<br/>본체 체력 1→5<br/>결실: 분신 체력 1→2"]
    S --> SB["숨 고르기<br/>무적 +0.16초<br/>결실: 방어막 1회"]
    S --> SC["먹발 버팀<br/>수평 밀림 -18%<br/>결실: 분신 생성 1→2"]

    M --> J["도약 · 13개"]
    J --> JA["준비시간 -6%<br/>고른 박자 결실"]
    J --> JB["점프 힘 +5%<br/>결실: 그린 먹선 +6%"]
    J --> JC["점프 높이 +6.25%<br/>결실: 하단 낙하 완화"]

    M --> I["먹 운용 · 13개"]
    I --> IA["최대 먹 3.2→8.0m<br/>결실: 선명 유지 +0.6초"]
    I --> IB["획당 먹 소모 -10%<br/>결실: 짧은 획 추가 -10%"]
    I --> IC["게이지 회복 +40%<br/>마르는 먹 결실"]
```

- 생존: `본체 최대 체력 +1×4(먹피 결실에서 분신 체력 +1) / 피격 뒤 무적 +0.04초×4 → 본체 1회 방어막 / 피격 수평 밀림 -6%×3 → 먹분신 생성 최대 +1`
- 도약: `준비시간 -1.5%×4 / 점프 힘 +1%×5(결실에서 그린 먹선 +6%) / 점프 높이 +1.5625%×4(결실에서 하단 낙하 완화)`
- 먹 운용: `최대 먹 용량 +37.5%×4(결실에서 선명 유지 +0.6초) / 획당 먹 소모 -2%×5(결실에서 짧은 획 추가 -10%) / 게이지 회복 +10%×4`

완성하면 본체 체력은 1→5칸, 분신 체력은 1→2칸이며 피격 뒤 무적은 +0.16초가 됩니다.
장애물에 맞아 체력이 줄고 살아남은 본체는 다음 피격 1회를 막는 개인 방어막을 얻습니다. 피격 후 수평
속도 보존율은 82→64%, 먹분신 아이템은 1회 1→2마리, 최대 먹 용량은
3.2→8.0m, 획 소모는 -10%, 회복 속도는 +40%가 됩니다. `넓은 벼루 결실`은
용량 안의 먹선 선명 시간을 0.6초 늘리고 `가는 붓끝 결실`은 1.5m 이하 짧은 획을
추가로 10% 절약합니다. `돋는 먹발 결실`은 직접 그린 먹선 도약에 힘 +6%를,
`높은 먹발 결실`은 마지막 먹방울이 하단에 몰릴 때 18초마다 낙하 완화를 줍니다. 먹 회복 성장은 선명
유지시간을 늘리지 않고 1.1초 페이드만 줄여, 사라진 길이만큼 하단 게이지가 더 빨리
돌아오게 합니다. 황금 붓은 용량 제한만 잠시 풀고 자연 소멸은 계속 진행합니다.
