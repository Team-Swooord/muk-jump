...
- 하단 붓획 게이지는 그리는 중인 획까지 포함한 잔량 비율을 실시간으로 보여 주며,
  ...
  최대치가 늘면 숫자 없이 트랙 폭도 함께 길어집니다. 게이지 폭은 줄지 않고 같은
  먹색이 사용할수록 옅어지고, 먹이 돌아오면 다시 진해집니다.
...
- 화면 좌우의 보이지 않는 경계에 닿으면 안쪽으로 확실히 밀리며 높이가 조금씩 다른
  트램펄린 반동이 한 번 발생합니다. 같은 벽에 붙어 있는 동안에는 수직 반동을 반복하지
...
| 먹분신 | 기본 한 마리 추가. 최종 성장 `먹떼 결실` 해금 시 한 마리 추가, 획득 1회 최대 2마리. 전체 생존 상한 24마리 |

## 영구 성장

로비의 `성장`에서는 도전 결과로 얻은 먹빛으로 큰 먹나무를 해금합니다. 성장은
`생존 / 도약 / 먹 운용` 세 분류이며, 각 분류는 공통 뿌리 뒤 세 갈래로 나뉩니다.
전체 39개 노드의 비용은 모두 먹빛 1개이고, 대부분의 갈래는 처음부터 끝까지 하나의
능력치만 단계적으로 올립니다. 생존의 피격 여유 마지막 열매는 `숨 고르기 결실`,
피격 안정 마지막 열매는 `먹떼 결실` 패시브로 바뀝니다. 해금한 모든 수치와 패시브는
항상 함께 적용됩니다.

- 생존: `최대 체력 +1×4 / 피격 뒤 무적 +0.04초×4 → 1회 방어막 / 피격 수평 밀림 -6%×3 → 먹떼 결실 +1`
- 도약: `준비시간 -1.5%×4 / 점프 힘 +1%×5 / 점프 높이 +1.5625%×4`
- 먹 운용: `최대 먹 용량 +37.5%×4 / 획당 먹 소모 -2%×5 / 게이지 회복 +10%×4`

완성하면 체력은 1→5칸, 피격 뒤 무적은 +0.16초가 됩니다. 장애물에 맞아 체력이
줄고 살아남은 각 먹방울이는 다음 피격 1회를 막는 개인 방어막을 얻습니다. 피격 후 수평
