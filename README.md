...
flowchart TB
    ...
    S --> SB["숨 고르기<br/>무적 +0.16초<br/>결실: 본체 1회 부활"]
...
| 생존 | 숨 고르기 | 본체가 장애물·추락으로 쓰러질 때 판당 1회 체력 1로 부활 |
| 생존 | 먹피 | 모든 먹분신의 최대 체력을 1→2칸으로 증가 |
| 생존 | 먹떼 | 먹분신 아이템 1회 생성 수를 1→2마리로 증가 |

- 생존: `본체 최대 체력 +1×3(결실: 분신 체력 +1) / 피격 뒤 무적 +0.04초×4(결실: 본체 1회 부활) / 피격 수평 밀림 -6%×3(결실: 먹분신 생성 최대 +1)`
...
판당 한 번 부활시킵니다. 황금 붓은 용량 제한만 잠시 풀고 자연 소멸은 계속 진행합니다.
