...
| 도약 | 돋는 먹발 | 하강 중 벽에 최대 1.2초 붙고 화면 안쪽으로 다시 점프 |
...
- 도약: `준비시간 -1.5%×3(결실: 점프 아이템 종료 방어막) / 점프 힘 +1%×4(결실: 벽 매달림 1.2초) / 점프 높이 +1.5625%×3(결실: 매 자동점프 40% 2단점프)`
...
아이템 상승 종료 뒤 방어막을 주며, `돋는 먹발 결실`은 하강 중 벽에 붙어 다시
뛸 수 있게 합니다. `높은 먹발 결실`은 본체와 모든 분신의 일반 자동 점프마다 상승
