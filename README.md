...
- 본체는 기본 내구 1칸으로 시작하고, 영구 성장의 먹피 경로를 열면
  ...
  `먹피 IV`를 열면 2/2가 되며, 각 먹방울 머리 위에 자기 최대치만큼 분리된
  체력 칸으로 표시됩니다.
...
| 먹분신 | 기본 한 마리 추가. `먹떼 결실` 해금 시 획득 1회 최대 2마리, `먹피 IV` 해금 시 각 분신 체력 2칸. 전체 생존 상한 24마리 |
...
flowchart TB
    ...
    M --> S["생존 · 13개"]
    S --> SA["먹피 I–IV<br/>본체 체력 1→5<br/>결실: 분신 체력 1→2"]
...
- 생존: `본체 최대 체력 +1×4(마지막에 분신 체력 +1) / 피격 뒤 무적 +0.04초×4 → 본체 1회 방어막 / 피격 수평 밀림 -6%×3 → 먹분신 생성 최대 +1`
