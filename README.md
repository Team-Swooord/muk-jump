...
능력치만 단계적으로 올립니다. 생존의 피격 안정 갈래만 마지막 열매가 `먹떼 결실`로
바뀌어 먹분신 아이템의 추가 생성 상한을 한 마리 늘립니다. 해금한 모든 수치는 항상
함께 적용됩니다.

- 생존: `최대 체력 +1×4 / 피격 뒤 무적 +0.04초×5 / 피격 수평 밀림 -6%×3 → 먹떼 결실 +1`
- 도약: `준비시간 -1.5%×4 / 점프 힘 +1%×5 / 점프 높이 +1.5625%×4`
- 먹 운용: `총 먹자리 +37.5%×4 / 획 소모 -2%×5 / 게이지 회복 +10%×4`

완성하면 체력은 1→5칸, 피격 후 수평 속도 보존율은 82→64%, 먹분신 아이템은
1회 1→2마리, 총 먹자리는
