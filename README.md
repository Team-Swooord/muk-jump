...
4. 화면에 남길 수 있는 총 먹자리를 관리하며 다음 발판을 이어 그립니다.
...
> 첫 시작에서는 월드를 완전히 멈춘 5단계 한지 팝업으로 먹 그리기·발판·체력·
> 바람·맵 규칙을 먼저 익힙니다. 마지막 `시작하기`를 누른 뒤에만 먹방울이 움직이며,
> 같은 안내는 옵션에서 언제든 다시 볼 수 있습니다.
...
- 그린 발판은 3.4초 동안 선명하게 남고 이후 1.1초 동안 처음 그린 쪽부터 먹이
  마르듯 사라집니다. 처음 총 먹자리는 실전 발판 약 한 획인 3.2m이며, 이를 넘겨도
  가장 먼저 그린 획의 시작점부터 시각과 충돌 판정이 함께 천천히 지워집니다.
- 하단 붓획 게이지는 그리는 중인 획까지 포함한 잔량 비율을 실시간으로 보여 주며,
  화면에 남은 먹선의 총길이가 줄어드는 만큼 다시 차오릅니다. 성장과 붓 여유로
  최대치가 늘면 숫자 없이 트랙 폭도 함께 길어집니다.
...
- 각 먹방울은 기본 내구 1칸으로 시작합니다. 영구 성장의 먹피 경로를 한 칸씩 열면
  2→3→4→5칸까지 늘어나며, 자기 머리 위에 현재 최대치만큼 떨어진 체력 칸으로
  따로 표시됩니다.
...
- 카메라는 살아 있는 먹떼의 중앙을 따라가고 상위 75% 무리를 별도 안전선으로
  보호합니다. 높은 본체·분신이 죽으면 남은 먹떼로 한 번만 부드럽게 재구도합니다.

### 높아질수록 세계와 규칙이 달라집니다

- 첫 30m는 발판 그리기와 먹분신을 익히는 입문 구간입니다.
- 화면 좌우의 보이지 않는 경계는 상승 중 부딪히면 위쪽 힘을 끊고 안쪽으로
  돌려보냅니다. 경계 안쪽 0.95m에는 발판이 확정되지 않아 가장자리 먹선과 벽 사이를
  반복해서 타는 길은 만들 수 없습니다. 영구 성장 `벽의 먹발`은 하강 중 기존처럼
  경계에 한 번 매달린 뒤 안쪽으로 자동 점프합니다.
- 이후 이동 먹가시, 낙묵석, 어린 동양 용과 먹해태 같은 수묵 장애물이 차례로 등장합니다.
  좌·우 느낌표와 반투명 붉은 먹빛이 깜빡이면 해태가 표시된 높이를 가로지릅니다.
...
| 황금 붓 | 8초 동안 새 획을 그려도 오래된 획이 밀려나지 않음. 종료 뒤 총량을 한 번에 정산 |
| 먹 방어막 | 장애물 피해 또는 화면 하단 추락을 1회 막음. 보유 중 재획득은 소비하지 않음 |
| 먹분신 | 먹방울 한 마리를 추가함. 동시에 살아 있을 수 있는 먹방울은 최대 24마리 |
| 붓 여유 | 현재 판의 총 먹자리를 기본값의 25%만큼 누적 확장 |

## 영구 성장

로비의 `성장`에서는 도전 결과로 얻은 먹빛으로 큰 먹나무를 해금합니다. 성장은
`생존 / 도약 / 먹 운용` 세 분류이며, 각 분류는 공통 뿌리 뒤 세 갈래로 나뉩니다.
각 갈래에는 일반 노드 3개와 마지막 비기 1개가 있어 전체는 39개 노드입니다.

모든 노드의 비용은 먹빛 1개입니다. 일반 노드는 해금 즉시 항상 적용되고, 강력한
비기는 해금한 것부터 전부 함께 적용합니다. 도약 비기의 벽 매달림 점프, 5회마다 생기는
단방향 발판, 자동 2단점프도 해금한 만큼 동시에 작동합니다. 플레이 중 무작위 증강이나
성장 카드 선택은 없으므로 도전의 핵심은 발판 그리기와 아이템 대응에 집중합니다.
먹 운용은 `총 먹자리 / 획의 먹자리 점유 절약 / 오래된 획의 소멸 완화` 세 갈래입니다.
총량 경로는 해금할 때마다 기본 먹자리의 30%가 늘어 `3.2m → 4.16 → 5.12 →
6.08 → 7.04 → 8.0m`로 성장하며, 마지막 비기까지 해금하면 시작의 약 2.5획을
화면에 남길 수 있습니다.

시간 만료와 총량 초과는 같은 소멸 연출을 사용합니다. `붙잡은 먹길`을 장착하면
기본 4.5초가 최대 5.25초로 늘어나며, 황금 붓은 용량 제한만 잠시 풀고 자연 소멸은
계속 진행합니다.
...
- 24마리 먹떼를 고려한 중앙/상위 안전선 카메라, 개체별 체력, VFX 예산 관리
