...
- `PermanentGrowthView`는 불투명 한지 배경의 전용 전체 화면으로 열리고
  ...
  `TreeCanvas`는 0.84 배율로 렌더링해 현재 가지를 더 넓게 조망한다. 뿌리에서는
  `먹그릇 1~2 → 숨고르기 1~2 → 먹결 1`의 다섯 열매를 단일 공통 줄기로 먼저
  해금한다. 마지막 공통 열매에서만 좌측 생존, 중앙 먹 운용, 우측 도약의 세
  주가지가 갈라지고, 각 계보 안의 실제 선행 조건 분기와 다중 부모 합류도 가로
  간격이 드러나게 배치한다. 공통 줄기는 `IsCommonTrunk`로 UI 의미만 구분하며
  기존 저장 트랙 ID·rank·효과는 바꾸지 않는다. 계보 표찰은 뿌리가 아니라 실제
  분기 입구에 둔다. 상단에는 로비 버튼과 먹빛 아이콘·숫자만 고정한다.
  완성 나무 배경은 42% 불투명도로 제한해 전경 열매와 간선의 대비를 보존한다.
  모듈형 가지는 조각별 실제 alpha 가로 범위를 기준으로 배치 폭과 중심을 보정해
  논리 간선 양끝을 각각 18px 덮는다. 열매 받침과 계보 표찰은 alpha 1을 사용하고
  계보명·선택 정보는 Normal 굵기로 렌더링한다.
  각 노드의
