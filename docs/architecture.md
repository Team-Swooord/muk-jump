...
11. 먹분신은 실제 물리 캐릭터 최대 24마리다. 아이템 한 개마다 `GameManager`가
    `1 + PermanentSnapshot.InkCloneItemExtraCount(0~4)`마리를 요청하고 남은 슬롯만큼
...
- `PermanentGrowthCatalog`는 생존·도약·먹 운용 각 13개, 총 39개 stable-ID 노드를
  소유한다. 각 분류는 공용 뿌리 뒤 A/B/C 단일 수치 갈래로 나뉘며, 마지막 K 열매도
  별도 비기가 아니라 같은 수치의 4단계다. 비용은 모두 먹빛 1개다.
...
지속 수치의 합성 순서는 `직렬화 기본값 × 영구 성장 × 환경`이다. 판 한정 붓 여유는
그 결과에 `직렬화 기본값 × 누적 25%`를 더해 성장·먹비가 아이템 한 개의 가치를
증폭하지 않게 한다. 먹 운용 상한은 총 먹자리 +150%(3.2→8.0m), 획 먹자리 소모
-10%, 게이지 회복 속도 +40%다. 도약 상한은 자동 점프 준비 -6%, 기본 점프 힘
+5%, 점프 정점 높이 +6.25%다. 생존 상한은 체력 5칸, 피격 무적 +0.20초,
먹분신 아이템 1회 최대 5마리다. 분신 보호·마지막 숨·피격 보존·벽 매달림·안전
