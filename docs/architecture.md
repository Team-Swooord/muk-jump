...
- `PermanentGrowthCatalog`는 생존·도약·먹 운용에 정확히 13개씩, 총 39개의
  ...
  각 가지는 일반 열매 세 개 뒤 비기 하나로 끝난다. 모든 열매 비용은 1이며 비기는
  자기 부모와 같은 계보 일반 열매 총 6개를 요구한다.
- `PermanentGrowthProfile`은 `schemaVersion=1`, `balanceVersion=2` 저장 문서에서
  먹빛 지갑, 소유 node ID 목록, 계보별 장착 비기 ID 하나와 마지막 정산 run ID를
  소유한다. 일반 열매는 항상 적용하고 비기는 계보마다 최대 하나만 무료 장착·해제한다.
  v1 rank 저장은 계보별 결정적 순서로 최대 13개까지 옮기며 초과 단계는 한 단계당
  먹빛 1개로 돌려준다. 구 지갑은 구 남은 총비용 진행률을 새 남은 노드 수에 비례
  변환한다. 알 수 없는·중복 node ID와 잘못 장착된 비기는 로드 시 제거한다.
...
- 정상 게임오버는 `GameManager`가 `ScoreManager.SaveBest()` 전에 이전 최고 고도를
  ...
  중도 로비 복귀는 지급하지 않고, 지갑+소유 노드는 총비용 39를 넘지 않는다.
- `PermanentGrowthView`는 불투명 한지 배경의 전용 전체 화면에서 세 독립 계보와
  39개 열매를 드래그 가능한 먹나무로 표시한다. 9:16에서는 `TreeCanvas`를 0.66배로
