...
1~4로 만든다. 분신은 이 세 단계의 `Vitality`를 받지 않고 기본 2칸을 유지하지만,
마지막 `S-KA 먹피 결실`이 현재 선택 갈래일 때 보너스 1칸을 받아 최대 3칸이 된다. 노드 ID·부모·
저장 schema는 유지한다. 각 `PlayerController`가 `CurrentHealth`를 따로 소유하고,
`ConfigureAsClone()`은 새 분신을 현재 런 스냅샷에 따라 2/2 또는 3/3으로 초기화한다.
...
개체별 체력 Billboard는 본체 1~4셀, 분신 2~3셀의 독립된 먹 테두리를 공유 Sprite로
