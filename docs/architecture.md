...
9. 이동 먹가시와 어린 용은 `ObstacleSpawner`의 같은 예약 슬롯과 풀을 공유한다.
   ...
   시 둘 다 끈다. 한 화면의 용은 최대 한 마리다.
10. 먹분신은 실제 물리 캐릭터 최대 24마리다. 아이템 한 개의 +3/+4/+5 생성은
    `GameManager`가 0.08초 간격으로 나눠 한 프레임의 `Instantiate` 피크를 제한한다.
    분신의 방어막은 모트 4개·파편 6개만 지연 생성하고, 0.14초 안의 동시 사망
    VFX·음원·진동은 한 번으로 합친다.

일반 드로잉 발판은 동시에 충돌 가능한 객체를 엄격히 최대 4개로 제한한다. 오래된 발판은
페이드에 들어가는 순간 `EdgeCollider2D`부터 끄고 active 목록에서 제거한다. 로컬 스타일
