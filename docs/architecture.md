...
기본 4.8m를 넘긴 초과분도 FIFO로 가장 오래된 획의 앞부분부터 같은 소멸 경로에
...
- 일반 드로잉 발판과 시작 지형: `EdgeCollider2D` + `PlatformEffector2D` 단방향 충돌.
  아래에서 통과하고 하강할 때 윗면에만 착지하며 발판 방향 회전을 유지한다.
- 안전 발판은 생성하지 않는다.
- 풍맥 발판도 같은 단방향 충돌을 사용해 아래에서 위로 통과할 수 있다.
...
본체 기본 최대 체력은 코드 규칙으로 3에 고정하고 씬의 과거 직렬화 값에 의존하지 않는다.
`S-A1`, `S-A2`, `S-A3`의 `Vitality` 합계를 0~3으로 제한해 본체 최대 체력을
3~6으로 만든다. 분신은 이 세 단계의 `Vitality`를 받지 않고 기본 1칸을 유지하지만,
마지막 `S-KA 먹피 결실`이 현재 선택 갈래일 때 보너스 1칸을 받아 최대 2칸이 된다. 노드 ID·부모·
...
소진된 경우에만 공통 `Kill()` 경로로 들어간다. `S-KB 숨 고르기 결실`을 선택한 본체는
장애물 또는 추락으로 쓰러질 때 판당 한 번 체력 1로 부활하고 `LaunchInkDrop(50)`의
검증된 물리 상승과 공유 먹물 VFX 풀을 재사용한다. 추락은 먼저 카메라 하단 안쪽 0.8m로
옮긴 뒤 50m를 솟는다. 이 부활 상승은 점프 아이템 종료 방어막을 예약하지 않으며,
분신은 사용권을 소비하거나 적용받지 않는다.

장애물 `TakeHit()`은 기존 방어막을 먼저 소모한다. 기본 분신은 첫 피해 뒤 1칸으로
생존하고 두 번째 피해에 사망한다. `S-KA 먹피 결실` 분신은 두 피해 뒤 1칸으로 생존하고
세 번째 피해에 사망한다. 아이템 방어막은 본체와 분신이
각자 소유한다. `I-KB 가는 붓끝 결실`은 황금 붓을 획득한 개체에게 즉시 방어막을
주고, `J-KA 고른 박자 결실`은 먹물방울 점프의 상승이 끝난 뒤 획득 개체에게 방어막을
준다. 이미 방어막이 있는 개체에는 중첩하지 않는다.

개체별 체력 Billboard는 본체 3~6셀, 분신 1~2셀의 독립된 먹 테두리를 공유 Sprite로
...
새 상승분을 추가하지 않아 급경사 먹선과의 상승 래칫을 차단한다. 단, `J-KB 돋는
먹발 결실`이 활성화된 본체와 모든 분신은 이동 상태와 관계없이 즉시 반동 대신
벽에 접착한다. 이때 `IsGrounded`·벽의 안쪽 노멀을 일반 발판과 같은 `AutoJump`
경로에 넘겨 충전 뒤 화면 안쪽으로 다시 뛰며, 벽을 떠난 직후 0.35초 재부착만 막는다.
각 벽의 `LineRenderer`는 레거시 진단용으로만
...
- `PermanentGrowthRunSnapshot`은 플레이 시작 순간의 소유 node ID와 계보별 선택
  A/B/C를 복사한 불변 계약이다. 공용 뿌리와 선택 갈래만 효과 합산에 포함한다.
  플레이어·자동 점프·드로잉·발판은 프로필을 실시간 조회하지
...
- 구매 기록은 모두 영구 보존하지만 생존·도약·먹 운용마다 선택한 한 갈래만 판에
  적용한다. 구 장착 ID는 저장 호환을 유지하면서 현재 선택 A/B/C의 결실 ID를 저장한다.
  비어 있거나 유효하지 않으면 소유 노드가 가장 많은 갈래를 결정적으로 선택한다.
  Keystone Kind는 가지 끝 결실의 열매·금빛 후광과 전용 패시브를 구분하는
  표현·런타임 표식으로 사용한다.
...
- `PermanentGrowthView`는 불투명 한지 전체 화면의 드래그 가능한 먹나무에 39개
  ...
  결실은 일반 노드보다 조금 큰 열매 면과 상시 금빛 후광을 사용하고 마지막 일반
  열매보다 310px 위에 배치해 고정 헤더 아래에서 터치 가능하게 한다. 상단 누적 거리
  막대는 520×32px와 내부 `0 / 100%` 진행률을 사용한다. 2회 확인 `노드 초기화`는
  소유 노드만 비우고 먹빛을 전액 환급하며 누적 거리·수령 이력은 보존한다.
...
- 영구 성장 전용 화면에는 개발용 DEBUG 서랍이나 재화 고정을 만들지 않는다. 사용자용
  `노드 초기화`만 제공하며 저장 복구의 `새로 시작`과 달리 누적 거리·보상 이력은 지우지 않는다.
- `GameplayHudView`도 Editor·Development Build 여부와 무관하게 직렬화된 구형
  `ItemTestControls`를 비활성화해 플레이 화면에 DEBUG 창을 노출하지 않는다.
- `LobbyOptionsView`는 로비 전용 5장 다시 보기, 세로로 쌓인 BGM·효과음 조절행,
  고객센터 안내만 소유한다. 설정 카드·토글은 밝은 한지 면을 사용하고 `닫기`·`다음`
  같은 주 행동만 검은 붓획을 사용한다. 계정·언어·외부 로그인 목업은 노출하지 않으며
  로그인 기능을 도입하기 전까지 인증 토큰도 저장하지 않는다.
...
먹 용량 확장 아이템은 사용하지 않는다. 먹 운용은 선택 갈래에 따라 최대 먹 용량
일반 열매 +112.5%(4.8→10.2m) 뒤 `넓은 벼루 결실` ×2(20.4m), 획당 먹 소모
-8% 뒤 황금 붓 획득 즉시 방어막, 또는 게이지 회복 +40%를 제공한다. 도약은
자동 점프 준비 -4.5% 뒤 먹물방울 상승 종료 방어막, 기본 점프 힘 +4% 뒤 좌우 벽
상시 자동점프, 또는 점프 높이 +4.6875% 뒤 힘 40%의 2단점프를 제공한다.
2단점프는 본체와 분신이 각자의 모든 일반 자동 점프 상승 속도가 최초 속도의 15%로
낮아진 정점 직전에 한 번씩 사용하며 먹물방울·풍맥 같은 특수 상승에는 발동하지 않는다. 생존은 본체 최대 체력 4칸 뒤 분신 최대
체력 3칸, 피격 무적 +0.16초 뒤 본체 판당 1회 부활·50m 상승, 또는 수평 속도 보존율
64% 뒤 먹분신 아이템 1회 최대 2마리를 제공한다. 같은 계보의 세 갈래는 동시에
적용하지 않으며 공용 뿌리와 현재 선택한 한 갈래만 런 스냅샷에 들어간다. 안전 발판은
영구 성장에서 사용하지 않는다.
