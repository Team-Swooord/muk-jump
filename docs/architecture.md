...
사망하고 먹피 IV 분신은 두 번까지 버티며, 아이템 방어막은 본체와 분신이 각자 소유한다.
...
- `PermanentGrowthCatalog`는 생존·도약·먹 운용 각 13개, 총 39개 stable-ID 노드를
  소유한다. 각 분류는 공용 뿌리 뒤 A/B/C 갈래로 나뉜다. 대부분의 마지막 K 열매는
  같은 수치의 4단계이며, 생존 `S-KC`만 먹분신 아이템 추가 생성 +1 최종 패시브다.
  비용은 모두 먹빛 1개다.
...
- 구매한 모든 노드는 영구 적용한다. 구 Keystone Kind와 장착 ID는 저장·레이아웃
  호환용이며 런타임 장착·교체나 고유 패시브를 만들지 않는다.
...
-10%, 게이지 회복 속도 +40%다. 도약 상한은 자동 점프 준비 -6%, 기본 점프 힘
+5%, 점프 정점 높이 +6.25%다. 생존 상한은 본체 체력 5칸·분신 체력 2칸,
