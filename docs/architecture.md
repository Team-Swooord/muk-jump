...
- `PermanentGrowthCatalog`는 생존 13·도약 19·먹 운용 13, 총 45개의
  stable-ID 열매를 직접 소유한다. 각 `PermanentGrowthNodeDefinition`에는
  이름·설명·아이콘 키·효과 ID와 값·부모 ID·계보·`Root/Stat/Mechanic/Keystone`
  종류·먹나무 좌표가 들어간다. 계보별 독립 뿌리에서 A/B/C 세 가지가 갈라지고,
  생존·먹 운용의 각 가지는 일반 열매 3개, 도약의 각 가지는 일반 열매 5개 뒤
  비기 하나로 끝난다. 모든 열매 비용은 1이며 비기는 자기 부모와 같은 계보 일반
  열매 총 6개를 요구한다.
- `PermanentGrowthProfile`은 `schemaVersion=1`, `balanceVersion=3` 저장 문서에서
  먹빛 지갑, 소유 node ID 목록, 계보별 장착 비기 ID 하나와 마지막 정산 run ID를
  소유한다. 일반 열매는 항상 적용하고 비기는 계보마다 최대 하나만 무료 장착·해제한다.
  v1 rank 저장은 먼저 39노드 v2 경제로 옮긴 뒤, v2→v3에서는 소유 ID·지갑·정산
  이력·장착 비기를 보존한다. 기존 도약 비기 보유자는 새 4·5단계를 완료한 것으로
  보정해 열린 비기가 끊긴 가지에 남지 않게 한다. 알 수 없는·중복 node ID와 잘못 장착된 비기는 로드 시 제거한다.
  구매 가능 여부와 잠금 사유는 모두 같은 부모·계보 보유 수 조건을 검사한다.
- `PermanentGrowthRunSnapshot`은 플레이 시작 순간의 소유 node ID와 장착 비기를
  복사한 불변 계약이다. `RunGrowthController`가 한 판 동안 이 스냅샷과 공용
  재사용 대기시간을 소유하고, 플레이어·자동 점프·드로잉·발판·날씨는 프로필을
  실시간 조회하지 않고 스냅샷만 읽는다. 로비의 장착 변경은 진행 중인 판에
  소급되지 않는다.
- 정상 게임오버는 `GameManager`가 `ScoreManager.SaveBest()` 전에 이전 최고 고도를
  잡고 run ID로 한 번만 정산한다. 첫 정상 게임오버는 1개를 보장하고, 이후에는
  먹떼 진행 12m 이상이면서 실제 조작 가능 시간이 20초 이상일 때 1개를 준다.
  최고 기록이 처음 100/250/500/750/1000m를 넘을 때마다 1개를 더한다. 디버그 판과
  중도 로비 복귀는 지급하지 않고, 지갑+소유 노드는 총비용 45를 넘지 않는다.
- `PermanentGrowthView`는 불투명 한지 배경의 전용 전체 화면에서 세 독립 계보와
  45개 열매를 드래그 가능한 먹나무로 표시한다. 도약의 준비·힘·높이 경로는 각각
  먹청록·쪽빛·옅은 자주먹의 얇은 상태선과 어두운 받침으로만 구분한다. 9:16에서는 `TreeCanvas`를 0.66배로
  보여 첫 화면에 세 뿌리가 함께 보이고, 19.5:9·20:9에서는 실제 논리 폭에 맞춰
  나무만 추가 축소한다. 화면·지도 뷰포트는 Canvas 전체를 stretch하고, 완성 나무 배경은
  42% 불투명도, 실제 가지·열매 받침·계보 표찰은 alpha 1을 사용한다. 노드 입력면은
  시각보다 크게 두고 `NodeContrast` 진한 먹 받침 위에 잠금 봉오리·해금 빨간 열매·
  선택 먹고리·장착 금빛 고리를 별도 레이어로 합성한다.
  지도에는 반복 노드명을 넣지 않고 고정 `SelectedGrowthAction` 팝업에서 이름·설명·
  효과·비용 또는 잠금 상태를 읽은 뒤 단일 버튼으로 구매한다. 설명·상태 문구는
  최소 28px·25px까지의 best-fit으로 카드 안에서 줄바꿈한다. 일반 열매는 해금 후
  `적용 중`, 비기는 `장착/해제`를 표시한다. 같은 계보의 다른 비기로 바꿀 때 첫 탭은
  교체 대상을 확인하고 두 번째 탭만 실제 교체한다. 팝업 중에는 나무 드래그를 잠근다.
  부모–자식 간선은 `pg_branch_piece_01..06`을 결정적으로 골라 실제 먹선이 양끝을
  덮도록 보정하며, 개별 조각이 없으면 `pg_branch`로 폴백한다. UI 트리는 최초 진입
  때 한 번 만들고 이후 리소스·표현 객체를 재사용한다.
...
영구 상시 상한은 최대 먹 +6%, 먹 회복 +8%, 발판 수명 +4%, 자동 점프 준비 -6%,
기본 점프 힘 +2%, 직접 그린 발판 점프 +3%다. 9개 비기는 마지막 생존·피격 안정·
분신 연계·착지 박자·짧은 발판·하단 낙하·자연 소멸 환급·저먹 회복·낙묵석 방어 중
계보마다 하나만 활성화한다. 추락 부활·시작 분신·아이템 빈도·점수 배율은 제공하지
않는다.
