...
기본 최대 체력은 코드 규칙으로 1에 고정하고 씬의 과거 직렬화 값에 의존하지 않는다.
`S-A1`, `S-A2`, `S-A3`, `S-KA`의 `Vitality` 합계를 0~4로 제한해 실제 최대 체력을
1~5로 만든다. 노드 ID·부모·저장 schema는 유지하므로 기존 소유 노드는 손실 없이 새
체력 단계로 해석된다. 원본과 분신은 판 시작 스냅샷을 공유하지만 `CurrentHealth`는
각 `PlayerController`가 따로 소유하며, 새 분신은 그 판의 최대 체력으로 초기화한다.
...
줄고 살아남았으며 `숨 고르기 결실`을 소유한 경우에만 해당 `PlayerController`에
다음 피격 1회용 방어막을 부여한다. 방어막 소모·치명 피해·화면 하단 추락은 생성
조건이 아니므로 방어막 자체가 무한 재생성되지 않는다.

개체별 체력 Billboard는 최대 체력 수만큼 독립된 먹 테두리 셀을 공유 Sprite로
...
+5%, 점프 정점 높이 +6.25%다. 생존 상한은 체력 5칸, 피격 무적 +0.16초이며,
장애물 피격으로 체력이 줄고 살아남은 개체는 다음 피격 1회용 방어막을 얻는다. 피격 뒤 수평
