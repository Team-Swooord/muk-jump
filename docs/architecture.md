...
분신을 구분하지 않고 살아 있는 개체만 다시 정렬한다. 높은 개체 사망으로 남은
...
`S00`, `S-A1`, `S-A2`, `S-A3`의 `Vitality` 합계를 0~4로 제한해 실제 최대 체력을
...
소진된 경우에만 공통 `Kill()` 경로로 들어간다. 장애물 전용 마지막 생존 비기는 이
경계에서 소비하지 않는다.
...
생존자가 남아 있고 기존 상단 안전점을 이끌던 높은 개체가 사망했을 때만 남은 중앙
대표를 화면 46%에 두는 1회성 하강 목표를 허용해 빈 화면을 막는다. 낮은 분신의
...
- 구매한 일반 노드와 비기는 전부 영구 적용한다. 비기는 계보별 장착 슬롯을 서로
  교체하는 장비가 아니며, 소유 ID가 스냅샷에 들어오면 다른 비기와 동시에 발동한다.
...
증폭하지 않게 한다. 먹 운용 상시 노드는
총 먹자리 +120%, 전역 먹자리 점유 -3%, 짧은 획 점유 -6%, 오래된 획 소멸
+0.20초와 시작 지연 +0.10초를 제공한다. 해금 비기는 용량 +30%, 전역 점유 -5%,
소멸 +0.45초를 모두 더한다. 도약 상한은 자동 점프 준비 -6%, 기본 점프 힘 +5%,
점프 정점 높이 +6.25%이며 9개 비기는 해금한 만큼 전부 함께 활성화한다.
