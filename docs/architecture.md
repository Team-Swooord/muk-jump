...
10. 먹분신은 실제 물리 캐릭터 최대 24마리다. 아이템 한 개마다 `GameManager`가
    정확히 한 마리를 즉시 복제하므로 한 번의 획득에서 대량 `Instantiate`가 발생하지
    않는다. 분신의 방어막은 모트 4개·파편 6개만 지연 생성하고, 0.14초 안의 동시
    사망 VFX·음원·진동은 한 번으로 합친다.
...
새 분신은 화면 안 13개 후보 중 기존 생존자와 가장 멀리 떨어진 좌표를 선택한다.
