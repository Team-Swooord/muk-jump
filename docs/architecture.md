...
일반 드로잉 발판은 전역 총 먹자리 ledger와 시간제 수명을 함께 쓴다. 유효 스무딩
...
- `PermanentGrowthRunSnapshot`은 플레이 시작 순간의 소유 node ID 전체를
  ...
  정적 먹자리 장부를 복구해 성장 용량과 하단 게이지가 기본값으로 되돌아가지 않게 한다.
...
먹자리 확장 아이템은 사용하지 않는다. 먹 운용 상한은 총 먹자리 +150%(3.2→8.0m), 획 먹자리 소모
