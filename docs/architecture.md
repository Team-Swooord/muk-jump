...
1~5로 만든다. 분신은 런 스냅샷의 `Vitality`와 관계없이 최대·현재 체력을 항상 1로
고정한다. 노드 ID·부모·저장 schema는 유지하므로 기존 소유 노드는 손실 없이 본체의
새 체력 단계로 해석된다. 각 `PlayerController`가 `CurrentHealth`를 따로 소유하고,
`ConfigureAsClone()`은 새 분신을 즉시 1/1로 초기화한다.
...
조건이 아니므로 방어막 자체가 무한 재생성되지 않는다. 분신은 1체력이라 실제 피해를
받으면 즉시 사망하지만, 별도로 획득한 아이템 방어막은 본체와 분신이 각자 소유한다.

개체별 체력 Billboard는 본체 1~5셀, 분신 1셀의 독립된 먹 테두리를 공유 Sprite로
...
+5%, 점프 정점 높이 +6.25%다. 생존 상한은 본체 체력 5칸·분신 체력 1칸,
