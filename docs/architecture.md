...
1~4로 만든다. 분신은 이 세 단계의 `Vitality`를 받지 않고 기본 1칸을 유지하지만,
...
장애물 `TakeHit()`은 기존 방어막을 먼저 소모한다. 기본 분신은 실제 피해 한 번에
사망하고 `S-KA 먹피 결실` 분신은 두 번까지 버틴다. 아이템 방어막은 본체와 분신이
...
개체별 체력 Billboard는 본체 1~4셀, 분신 1~2셀의 독립된 먹 테두리를 공유 Sprite로
...
체력 2칸, 피격 무적 +0.16초 뒤 본체 판당 1회 부활·50m 상승, 또는 수평 속도 보존율
