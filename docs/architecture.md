...
Low·Medium·High는 핵심 먹획·아이콘·제목을 유지하고 장식 방울 수만 4/6/8개로 바꾼다.
...
- `PermanentGrowthView`는 불투명 한지 배경의 전용 전체 화면으로 열리고
  ...
  연결선과 잠금·구매 가능·보유·완성 상태를 가지며, 잠긴 노드도 선택해 하단
  공용 상세판에서 잠금 이유를 읽을 수 있다. 노드 선택은 표시 상태만 바꾸고,
  상세판의 강화 버튼만 `PermanentGrowthProfile` 구매를 호출한다.
