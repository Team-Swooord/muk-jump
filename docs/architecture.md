...
`CameraFollow`는 매 점프의 최고 정점을 진행도로 저장하지 않는다. 화면 높이 75%의
상단 데드존을 넘는 먹떼 대표만 추적하고 하강은 무시한다. 급상승 보정은
점프 줌의 현재 크기가 아닌 기본 orthographic 반높이로 뷰포트 90% 상한을 계산해,
줌 펄스가 수직 카메라 크리프를 만들지 않게 한다.
...
- `LobbyWorldSetup`은 로비 월드의 구버전 호환 계층이다. `StarterInkPlatform`만
  화면 폭 `±5.35`로 원자적으로 재구성하고 플레이어에 `LobbyCharacterWander`가
  없을 때만 추가한다. 일반 드로잉·풍맥 발판에는 개입하지 않으며, 여러 번 실행돼도
  컴포넌트나 붓선을 중복 생성하지 않는다.
