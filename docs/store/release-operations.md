...
- 보상이 없는 전체 최고 고도를 운영하며 순위표에 공개 닉네임을 표시한다. iOS·Android는 뒤끝
  TOP 10, iOS의 별도 Game Center 탭은 Apple 순위표, Apps in Toss는 공식 게임센터 화면을 사용한다.
- Apple 보드 ID는 `com.CYSB.MukJump.bestHeight`다. 뒤끝 계정과 Game Center 계정은 별개이며,
  뒤끝 계정 삭제가 Apple 순위까지 삭제했다는 뜻은 아니다. 출시 전 초기화 시 Apple 보드의 남은
  테스트 기록도 별도로 확인해야 한다. Apple ID 자체나 다른 게임의 기록은 절대 삭제하지 않는다.
