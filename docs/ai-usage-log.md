...
- 목적: 다른 GPT가 과거 대화 없이도 먹점프의 기획, 실제 수치, UI·아트, 성장 39노드,
  저장 복구, 기술 구조, 삭제 기능과 저장소 작업 규칙을 한 문서에서 파악하도록 전체
  프로젝트 기준서를 작성
- 주요 프롬프트/지시: “우리 게임의 전부인 문서를 작성해 전부 내용을 담아.” 기존
  소개 문장을 늘리는 대신 현재 `main` 코드와 전문 문서를 분야별로 교차 검사하고,
  코드와 문서가 충돌하는 게임오버 흐름·체력 상한·황금 붓·테스트 증거는 실제 구현과
  확인 범위를 분리해 적었다.
- 결과물: `docs/MUKJUMP_MASTER_GAME_BIBLE.md`, GitHub 첫 화면의 마스터 문서 링크
- 사람의 수정/검토 내용: 다음 GPT가 되살리면 안 되는 증강 카드·도감·안전 발판·먹
  초과 충전 아이템을 별도 폐기 목록으로 고정하고, 게임 수치와 아트 방향뿐 아니라
  `main` 단일 작업·씬 빌더·명시적 stage·AI 로그 동기화 규칙까지 인수인계 범위에 넣었다.
- 검증: 게임플레이, UI·아트·오디오·VFX, 성장·저장·아키텍처를 세 갈래 읽기 전용
  감사로 나눠 실제 C# 상수와 문서의 불일치를 확인했다. 현재 479개 테스트 선언과
  마지막 전체 실행 증거 472/472를 구분하고, 실기기·라이선스 미확인 사항은 완료로
  과장하지 않았다.
- 외부 에셋/라이선스: 새 외부 에셋 없음. 기존 에셋의 출처와 제출 전 확인 항목만 요약했다.

### 2026-08-07 — 제출 문서 3종 에디토리얼 전면 개편

- 사용 도구: OpenAI Codex, ReportLab, Pillow, Poppler, pypdf, 멀티에이전트 읽기 전용 감사
- 목적: 기존 제출 PDF의 반복 카드·동일 표지·추상적인 AI 문체를 걷어내고, 실제 게임
  화면과 한 페이지 한 메시지를 중심으로 게임 소개·AI 활용·팀 역할 문서를 다시 제작
- 주요 프롬프트/지시: 사용자가 제공한 Apple 디자인 분석에서 `제품 이미지 우선`,
  `낮은 정보 밀도`, `큰 제목`, `넓은 여백`, `단일 강조색` 원칙만 추출했다. Apple의
  색·구성은 복제하지 않고 먹점프의 한지 `#F3EEDF`, 먹 `#1C1B1A`, 낙관색
  `#AE1C3C`로 번역했다. 문장은 기능 홍보 문구보다 실제 문제·판단·수정 결과를 먼저
  쓰고, 세 문서가 같은 템플릿처럼 보이지 않도록 페이지 리듬을 각각 다르게 구성했다.
- 결과물: `docs/submission/NAN2026-{game-introduction,ai-utilization,team-roles}.md`,
  `tools/generate_nan2026_pdfs.py`, `docs/submission/assets/`의 실기기 화면 3장,
  `output/pdf/NAN2026_MukJump_*.pdf` 3종
- 사람의 수정/검토 내용: 먹 게이지를 `더 그릴 수 있는 남은 용량`으로 바로잡고,
  카메라의 최고 생존자 추적과 하위 중앙값 진행도 역할을 분리했다. 황금 붓은 자연
  건조까지 멈추는 것으로 과장하지 않았고, 검수 범위는 `iPhone 실기기 + Android
  세로 해상도 프리셋`으로 한정했다. 팀 역할의 근거 없는 기계 번역 QA를 삭제하고,
  대표 계정 중심 Git 기록을 개인 기여도 지표로 쓰지 않았다고 명시했다.
- 검증: 새 PDF 16쪽을 110dpi PNG로 전부 렌더링해 한글 표시·사진 크롭·페이지 리듬을
  육안 확인하고, A4 규격·페이지 수·메타데이터·금지 placeholder와 파일 재생성 가능
  여부를 검사했다.
- 외부 에셋/라이선스: 새 외부 에셋 없음. 문서에 추가한 화면은 팀이 직접 실행해
  촬영한 먹점프 실기기 스크린샷이며, 나머지는 저장소의 프로젝트 전용 게임 자산이다.

### 2026-08-07 — NAN 2026 제출용 3종 PDF 최신화

- 사용 도구: OpenAI Codex, ReportLab, Pillow, Poppler, pypdf, 멀티에이전트 읽기 전용 감사
- 목적: 신청서에 요구된 게임 소개·AI 활용 기술·2인 팀 소개 문서를 최신 기획과
  실명으로 정리하고, 바로 업로드할 수 있는 A4 PDF로 제작
- 주요 프롬프트/지시: “팀명은 최연소밴드, 김승연·최성빈의 UI와 게임·기획 역할을
  나누어 정리하고, 약 3주의 개발 기간과 전 개발 공정에서 AI를 사용한 점을 강조한다.”
  Git 이력과 현재 코드·기획·AI 로그를 교차 확인해 과장 없이 수치와 기능을 표기했다.
- 결과물: `output/pdf/NAN2026_MukJump_Game_Introduction.pdf`,
  `NAN2026_MukJump_AI_Utilization.pdf`, `NAN2026_MukJump_Team_Roles.pdf`와
  재편집용 `docs/submission/` 원문 3종
- 사람의 수정/검토 내용: 팀의 최신 역할과 이름을 확정하고, “모든 것을 AI가 자동
  제작”이 아니라 “AI가 전 공정에 상시 참여하고 팀원이 최종 판단·통합·실기기 검수”한
  AI 네이티브 방식으로 표현했다. GitHub 조직명은 저장소 URL에만 유지했다.
- 검증: 15쪽을 PNG로 전부 렌더링해 글자 잘림·겹침·깨진 한글을 검사하고, A4 규격,
  페이지 수, 텍스트 추출, 팀명·실명·placeholder 제거를 자동 확인했다.

### 2026-08-01 — 고정 UI·게임 브랜치로 저장소 통합

- 사용 도구: OpenAI Codex, Git, GitHub CLI, 멀티에이전트 읽기 전용 감사
- 목적: 병렬 작업으로 늘어난 브랜치를 정리하고 제출 기준 `main`과 두 고정
  작업선 `feature/ui`, `feature/game`만 유지
- 주요 프롬프트/지시: “현재 결과를 main으로 내보내고 앞으로는 main + UI +
  game 구조만 사용한다.” 미커밋 파일은 다른 채팅의 작업일 수 있으므로 강제
  커밋·reset·stash하지 않고, 확정 커밋의 조상 관계와 자동 병합 가능성을 먼저 검사했다.
- 결과물: 일반 병합 PR #14·#15, `docs/branch-policy.md`, `AGENTS.md` 고정 브랜치 규칙
- 사람의 수정/검토 내용: `main` 외 작업 브랜치는 총 두 개라는 의미로 해석했다.
  병합이 확인된 옛 원격·로컬 브랜치만 삭제했고 현재 작업 폴더의 미커밋 UI·씬
  변경은 그대로 보존했다.

### 2026-08-01 — 한 판 증강 수묵 카드 런타임 아트 통합

- 사용 도구: OpenAI Codex, OpenAI ImageGen, Pillow, Unity Test Framework
- 목적: 사용자가 확정한 1:3 세로형 산수화 카드 프레임을 한 판 증강 선택 UI에
  실제 적용하고, 8종 증강을 작은 모바일 화면에서도 즉시 구분되는 개별 스프라이트로 제작
- 주요 프롬프트/지시: 사용자 제공 무문자 프레임은 팔레트·한지 질감·붓결의 기준으로만
  사용했다. `먹두께·도약·큰 벼루·먹샘·긴 여운·겹친 획·굳은 획·길운`을 각각
  2~3개의 큰 먹 덩어리로 단순화하고, 녹색 크로마 배경 외에는 글자·숫자·붉은색·낙관·
  캐릭터·마법 문양·일본/중국식 장식을 생성하지 않도록 지시했다. 전체 프롬프트는
  `docs/ai-artifacts/ui/augmentation_card_runtime_prompt.md`에 보존했다.
- 결과물: `Assets/Resources/MukJump/UI/Growth/growth_card_frame.png`,
  `growth_card_icon_*.png` 8종, `GrowthChoiceView`의 공통 프레임/투명 입력면/동적 설명,
  `MukJumpSceneBuilder`의 Sprite 주입 및 모바일 임포트 설정
- 사람의 수정/검토 내용: 생성 결과를 한 장씩 확인하고 크로마키 제거 뒤 피사체를
  768×768 투명 캔버스에 같은 최대 치수로 정규화했다. 프레임은 카드 표시 비율에 맞춰
  512×1536으로 정리했다. 기존 원형 캐릭터 아이콘과 발판 공용 아이콘은 사용하지 않고,
  긴 여운(3단 잔상)과 겹친 획(평행 2획)을 별도 자산으로 분리했다. 사용자 제공 프레임은
  프로젝트 내부 사용을 전제로 했으며 제3자 로고·문구·워터마크는 포함하지 않았다.
- 검증: `feature/game` 격리 worktree에서 스프라이트 알파·해상도·피사체 경계를 확인했다.
  성장 카드 계층·8종 아이콘/설명 매핑·선택 시 프레임 색 보존·임포트 예산과 기존 씬의
  런타임 교체를 포함한 전체 EditMode `314/314`가 통과했고 C# 컴파일 오류는 0건이다.

### 2026-07-30 — 영구 성장 강화 먹획 연출 통합

- 사용 도구: OpenAI Codex, AVFoundation 프레임 분석, Unity Test Framework,
  프로젝트 `docs/VFX/SKILL.md`
- 목적: 강화 성공 때 나무와 가지가 붉게 차오르는 어색한 채움 연출을 제거하고,
  사용자가 제공한 영상처럼 화면 먹획과 중앙 결과 표식으로 성공을 분명히 전달
- 주요 프롬프트/지시: “강화하기를 눌렀을 때 참고 동영상처럼 이펙트를 넣고,
  나무가 빨갛게 칠해지는 연출은 빼 달라.” 참고 URL:
  `https://v1.pinimg.com/videos/iht/hls/f7/ff/73/f7ff73d677929f4f7730fae5f7927fb8.m3u8`
- 결과물: `PermanentGrowthView`의 붉은 나무·가지 채움 계층과 갱신 코드 제거,
  `GrowthUnlockPresentation`의 1.26초 해금/0.86초 강화 모드,
  `InkUiFeedbackController`의 고정 UI 재사용 연결과 회귀 테스트·설계 문서 갱신
- 사람의 수정/검토 내용: 원본 영상의 가로 화면, 캐릭터, 로고, 네온 색과 음원은
  복사하지 않고 `대각선 프레임 → 중앙 먹번짐 → 원형 확정`의 시간 구조만 분석했다.
  반복 강화에는 `Lv.N → 성장 강화` 문구를 사용하고 먹색·한지색·금빛만 유지했다.
  구매 실패에는 연출을 내지 않으며, 런타임 생성이나 새 패키지 없이 기존 고정 Image
  계층과 품질별 4/6/8개 장식 슬롯을 재사용한다.
- 검증: 스테이징한 파일만 분리한 Unity 6000.3.10f1 프로젝트에서
  `GrowthUnlockPresentationTests` 6/6 및 붉은 나무·가지 제거 구매 회귀 1/1 통과,
  C# 컴파일 오류 0건. 전체 `PermanentGrowthArtTests`의 기존 강화 버튼 높이 기대값
  불일치(기대 100, 실제 88)는 이번 연출 변경과 분리해 워킹트리에 보존했다.
...
### 2026-07-30 — Play 전 Main과 런타임 로비 표시 동기화

- 사용 도구: OpenAI Codex 멀티 에이전트, Unity Test Framework,
  Unity 6000.3.10f1 배치모드
- 목적: Unity의 Play 버튼을 누르기 전 Main Game View에만 남아 있던 인게임 HUD,
  DEBUG 버튼, 플레이어와 시작 먹선을 제거해 실제 런타임 로비와 같은 화면을 유지한다.
- 주요 프롬프트/지시: “플레이 버튼을 누르기 전 화면과 플레이했을 때의 로비가
  다르다. 없는 요소를 없애고 항상 Main과 로비를 똑같게 한다.”
- 결과물: `GameplayCanvas`의 저장 초기값 비활성화, 편집 모드 HUD 자동 비노출,
  플레이어 SpriteRenderer·시작 발판 LineRenderer의 저장 초기값 비활성화,
  `LobbyWorldSetup`의 편집 모드 로비 표시 동기화와 회귀 테스트.
- 사람의 수정/검토 내용: 상태 변경과 첫 점프 물리가 계속 작동하도록 GameObject,
  Collider와 컨트롤러는 활성 상태로 두고 화면에 그리는 컴포넌트만 숨겼다.
  공유 중인 `Main.unity`의 다른 미커밋 작업은 덮어쓰지 않았으며, 다음 표준 씬 빌드도
  같은 초기 표시 상태를 저장하도록 빌더를 수정했다. 전체 EditMode `278/278`
  통과와 배치 로그 C# 컴파일 오류 0건을 확인했다.

### 2026-07-30 — 로비 최고 기록 칸 중앙 정렬

- 사용 도구: OpenAI Codex 멀티 에이전트, `emil-design-eng` UI 스킬,
  Unity Test Framework
- 목적: 로비 상단 최고 기록 붓획이 화면 중앙보다 오른쪽으로 밀린 문제를 수정한다.
- 주요 프롬프트/지시: “문제 1 가운데 맞춤이 안 되어 있음.”
- 결과물: 최고 기록의 별도 X `0.60` 레일을 로비 메뉴와 같은 화면 중앙 X `0.50`
  레일로 통일하고, 편집 모드와 런타임이 같은 레이아웃을 즉시 적용하도록 보강했다.
- 사람의 수정/검토 내용: 사용자가 맞춘 비대칭 붓 PNG의 배경 `+89`, 라벨 `-87`
  내부 보정과 크기·폰트는 유지했다. 1080 기준 최종 라벨 중심이
  `0.5×1080+89-87=542px`로 화면 중앙 540px의 허용 오차 안에 드는지 테스트한다.

### 2026-07-30 — 로비 메뉴 선택 먹 농도 동기화

- 사용 도구: OpenAI Codex 멀티 에이전트, `emil-design-eng` UI 스킬,
  Unity Test Framework
- 목적: 성장·도감·옵션을 눌러도 `시작` 버튼만 계속 진한 먹색으로 남는 문제를
  해결해 현재 선택 위치를 즉시 알아보게 한다.
- 주요 프롬프트/지시: “성장을 눌렀을 때 성장 버튼이 진해져야 하며 모든 버튼에
  같은 규칙을 적용한다.”
- 결과물: 네 메뉴 공통 선택 상태, 선택 배경 100%·나머지 78% 먹 농도 전환,
  성장·도감 전환 목적지 및 옵션 열림 상태 연동, 실패·닫기·로비 복귀 복원 테스트.
- 사람의 수정/검토 내용: 자주 누르는 상위 메뉴에는 지연 애니메이션을 추가하지 않고
  배경 먹 농도만 즉시 교체한다. 흰 글자와 CanvasGroup은 항상 100% 농도를 유지하고,
  상태값이 실제로 바뀔 때만 그래픽을 갱신해 모바일 Canvas 재빌드를 줄였다.

### 2026-07-30 — 공용 붓획 버튼 스프라이트 파손 수정

- 사용 도구: OpenAI Codex 멀티 에이전트, `emil-design-eng` UI 스킬
- 목적: 뒤로·닫기·확인 등 작은 공용 버튼에서 정상 붓획 대신 잘린 먹 얼룩만
  표시되는 문제를 해결한다.
- 주요 프롬프트/지시: “이런 버튼들 이미지 스프라이트가 다 깨짐.”
- 결과물: 공용 행동 버튼의 9-slice 테두리를 제거하고, 모든 크기에서 원본 붓획
  전체를 그리는 Simple 이미지 방식으로 통일했다.
- 사람의 수정/검토 내용: PNG 원본의 알파와 전체 실루엣은 정상임을 직접 확인했다.
  문제는 700×350 원본에 좌우 112px·상하 78px 테두리를 둔 뒤 작은 버튼에
  9-slice로 축소하면서 중앙 획이 사라지고 모서리만 남는 데 있었다. 버튼 크기나
  문구는 바꾸지 않고 렌더 방식만 교정했으며, 자주 쓰는 버튼에 추가 애니메이션은
  넣지 않았다.

### 2026-07-30 — 영구 성장 글자 실제 획 두께 보강

- 사용 도구: OpenAI Codex 멀티 에이전트, `emil-design-eng` UI 스킬
- 목적: 영구 성장 화면에서 Bold로 지정한 제목·부제·재화 글자가 세로 Game View
  축소 시 가늘게 보여 읽기 어려운 문제를 해결한다.
- 주요 프롬프트/지시: “글씨체 볼드로 잘 안 보임.”
- 결과물: `PermanentGrowthTypography`가 모든 성장 화면 글자에 동일 글자색의
  얇은 외곽선을 적용하고, 제목·부제·재화·성장명 순으로 시각 두께를 차등 보강한다.
- 사람의 수정/검토 내용: 사용자가 지정한 헬스셋 조릿대 Std 서체와 기존 글자 크기,
  배치, 색은 유지했다. 검은 대비 테두리를 추가하거나 글자를 키워 레이아웃을
  흔들지 않고, 1080×1920 기준 1.5~2.5px 범위에서 글자 자체의 획만 굵게 만들었다.

### 2026-07-30 — 영구 성장 카드·상세판 정렬 재구성

- 사용 도구: OpenAI Codex 멀티 에이전트, `emil-design-eng` UI 스킬
- 목적: 성장 카드와 하단 상세판의 글자가 패널 밖으로 나오고 좌우 카드마다 정렬이
  달라 보이는 문제를 해결한다.
- 주요 프롬프트/지시: “글씨를 담는 UI 패널들이 작아 글씨가 튀어나오며, 좌측 또는
  가운데 중 하나로 정렬을 통일한다.”
- 결과물: 성장 카드 `456×248`, 상세판 `920×300`, 네 카드 정보 좌측 정렬,
  Overflow 금지, 6px 이상 행 간격, 단계 숫자·노드와 정보 열 분리, 상세판 구분선과
  강화 버튼 열 재배치.
- 사람의 수정/검토 내용: 글자 크기를 다시 줄이는 방식은 사용하지 않았다. 카드보다
  1~10px 더 컸던 텍스트 Rect와 서로 반대였던 좌우 정렬을 먼저 교정하고, 단일 숫자
  단계 표시는 24px로 제한해 6개가 겹치지 않게 했다. 다른 작업 중인 성장 아트와
  장식 스프라이트 파일은 수정·스테이징 대상에서 제외했다.

### 2026-07-31 — 영구 성장 3대 계보 재설계

- 사용 도구: OpenAI Codex 멀티 에이전트, `emil-design-eng` UI 스킬,
  Unity Test Framework
- 목적: 읽기 어려운 4축 카드형 먹나무를 생존·도약·먹 운용의 3대 계보와
  단계형 소노드·최종 패시브가 한눈에 읽히는 세로 모바일 성장 화면으로 재구성한다.
- 주요 프롬프트/지시: “가독성을 높이고, 피통·점프력 등 방향별로 나눈 세 가지
  큰 가지 끝에서 부활 1회 같은 강력한 패시브를 여는 구조로 다시 제작한다.”
- 결과물: 생존·도약·먹 운용 3계보와 10개 노드, 선행 조건·잠금 사유·계보별
  최종 패시브, 공용 상세판 UI, 실제 체력·피격 유예·점프·먹발판 방어 효과를
  구현했다. Unity 6000.3.10f1 전체 EditMode 테스트 293/293을 통과했다.
- 사람의 수정/검토 내용: 기존 4개 stable ID·비용·효과·배열 순서와 schema 1
  저장 레벨을 보존했다. 실제 540×960 Game View에서 세로 배치를 열어 확인하고
  계보 요약을 짧게 바꾸고 노드 글자·잠긴 노드 대비·상세판 간격을 재조정했다.
  빨강은 선행 조건 및 먹빛 부족 안내에만 사용하고, 선택·완성은 금색과 먹색으로
  구분했다.

### 2026-07-31 — 먹나무 해금 열매·노드 위치 연출

- 사용 도구: OpenAI Codex 멀티 에이전트, 프로젝트 Unity 2D URP Android VFX 스킬,
  Unity Test Framework
- 목적: 3계보 노드 구조를 처음 기획한 수묵 먹나무의 줄기·가지 형태로 되돌리고,
  해금할 때마다 해당 노드에서만 붉은 열매가 피어나는 명확한 성장 피드백을 만든다.
- 주요 프롬프트/지시: “노드 구조는 유지하되 처음 디자인한 나무처럼 만들고,
  해금하면 나무 열매가 하나씩 빨갛게 빛나며 매번 이펙트를 넣는다.”
- 결과물: 수묵 줄기·가지 스프라이트를 노드 뒤에 복원하고, 잠긴 마른 봉오리와
  해금된 붉은 열매를 별도 이미지로 분리했다. 처음 해금한 열매의 화면 좌표에서
  짧은 붉은 번짐과 최대 4개 검은 먹방울이 피어나며 기존 고정 UI 계층을 재사용한다.
- 사람의 수정/검토 내용: 나무 전체·가지에는 붉은 채움을 넣지 않았다. 새
  Particle System·셰이더·런타임 생성 없이 핵심 열매 변화는 모든 품질에서 유지하고,
  장식 방울 수만 모바일 품질 단계에 따라 줄였다.

### 2026-07-31 — 모바일 드래그형 대형 먹나무 성장 지도

- 사용 도구: OpenAI Codex 멀티 에이전트, `emil-design-eng` UI 스킬,
  Unity Test Framework
- 목적: 한 화면에 압축된 세 성장 열을 큰 수묵 나무 한 그루로 바꾸고, 모바일
  세로 화면에서도 가지와 둥근 특성 노드를 직접 끌어 탐색하게 한다.
- 주요 프롬프트/지시: “성장 탭에 큰 나무를 통째로 그리고 왼쪽 위·오른쪽
  대각선으로 여러 가지가 뻗게 하며, 원형 특성 노드를 올리고 크게 만들어
  마우스로 전체를 확인한다.” 사용자 제공
  `docs/ai-artifacts/ui/permanent_growth_tree_concept_v1.png`를 구성 참고로 사용했다.
- 결과물: 고정 제목·먹빛·하단 상세판 사이에 마스킹된 양축 드래그 뷰포트와
  `2240×2180` 먹나무 지도를 만들었다. 먹 운용 중앙 주간, 생존 왼쪽 주가지,
  도약 오른쪽 주가지와 장식 곁가지를 배치하고 10개 특성을 큰 원형 열매 노드로
  확장했다. 지도는 Clamped 관성 이동을 사용하며 해금 연출 중에는 이동을 잠근다.
- 사람의 수정/검토 내용: 가로형 참고 시안의 우측 상세판을 그대로 복제하지 않고
  세로 모바일에서는 상세판을 하단에 고정했다. 신규 입력 API나 커스텀 제스처를
  추가하지 않고 기존 Input System EventSystem과 uGUI `ScrollRect`를 재사용했다.
  빨강은 이전 규칙대로 실제 해금 열매에만 남기고 나무·가지는 먹색으로 유지했다.

### 2026-07-31 — 전체 화면 먹나무와 모듈형 가지 구조

- 사용 도구: OpenAI Codex 멀티 에이전트, `emil-design-eng` UI 스킬,
  Unity Test Framework
- 목적: 영구 성장 화면의 큰 하단 설명판을 제거하고, 현재 먹나무 전체를 첫 화면에
  잘리지 않게 보여 주면서 후속 특성을 가지 조각으로 확장할 수 있게 한다.
- 주요 프롬프트/지시: “밑에 설명칸을 없애고 나무 전체가 성장 UI를 쓰도록 하며,
  나중에 특성을 유동적으로 추가할 수 있게 같은 그림체의 가지 조각 스프라이트를
  붙이는 형식으로 만든다. 나무 사진을 첨부해 생성할 프롬프트도 작성한다.”
- 결과물: `980×1440` 성장 뷰포트에 뿌리·줄기·세 계보·현재 10노드를 중앙
  배치하고, 하단 상세판 대신 선택 노드 곁의 비용·잠금 상태와 강화 버튼만 남겼다.
  계보·순서 기반 좌표와 `pg_branch_piece_01..06 → pg_branch` 폴백 로더,
  투명 PNG 여섯 장을 같은 붓결로 생성하는 이미지 프롬프트를 추가했다.
- 사람의 수정/검토 내용: 초기 선택 액션과 인접 노드의 겹침을 피하도록 노드 간격과
  터치 영역을 조정하고, 현재 나무 요소가 마스크 안쪽 8px 안전 여백에 들어오는지
  회귀 테스트로 고정했다. 조각 이미지가 아직 없어도 기존 가지로 동작하며, 새
  스프라이트는 파일명만 맞추면 런타임 코드 수정 없이 순서대로 적용된다.

### 2026-07-31 — 1포인트 분기 성장 그래프와 수묵 가지 6종

- 사용 도구: OpenAI Codex 멀티 에이전트, OpenAI 기본 이미지 생성,
  Unity Test Framework
- 목적: 한 카드 안에서 여러 번 올리던 영구 성장 단계를 한 번씩 해금하는 열매
  노드로 펼치고, 생존을 포함한 모든 계보에 여러 선택 경로를 만든다.
- 주요 프롬프트/지시: “특성을 더 세분화해 하나씩 찍고 노드가 점점 펼쳐지게
  하며, 생존도 한 길이 아니라 여러 길로 만든다. 검은사막처럼 촘촘한 구조를
  먹나무 가지 조각으로 생성해 붙인다.” 먹나무 줄기 이미지를 참조해 기본 곡선,
  S자, 직선, Y자 분기, 곁가지, 종결 가지를 균일한 녹색 배경에 각각 생성했다.
- 결과물: 생성 원본과 프롬프트 기록, 투명 RGBA 수묵 가지 6종,
  저장·효과 트랙과 화면 노드를 분리한 1포인트 성장 그래프 및 다중 부모 연결 구조.
- 사람의 수정/검토 내용: 기존 stable ID·저장 레벨·구매 비용·최대 효과는 유지해
  세이브를 초기화하지 않는다. AI 이미지의 녹색 배경은 가장자리 색 추정과
  soft matte·despill로 제거하고 여섯 파일 모두 알파 채널과 잔여 배경을 확인했다.
  빨강은 실제 해금 열매에만 사용하며 가지는 검정·회색 먹색만 남겼다.

### 2026-07-31 — 39개 열매를 받치는 한 그루의 큰 먹나무 배경

- 사용 도구: OpenAI Codex 멀티 에이전트, OpenAI 기본 이미지 생성,
  Unity Test Framework
- 목적: 모듈 가지가 노드 연결선처럼 보이던 성장 화면을 한 그루의 거대한
  먹나무 위에 열매가 맺히는 구성으로 바로잡는다.
- 주요 프롬프트/지시: “배경 큰나무 위에 올려져 있는 느낌이어야 한다.” 기존
  먹나무 줄기는 붓결 참고, 가로 성장 시안은 전체 나무의 구성 참고로만 사용해
  뿌리부터 여러 번 갈라지는 검정·회색 수묵 나무를 균일한 초록 배경에 생성했다.
- 결과물: `pg_tree_background_v2` 생성 원본·프롬프트·투명 RGBA 게임 자산,
  전체 성장 좌표를 덮는 고정 나무 배경과 그 위에 놓이는 39개 열매 노드.
- 사람의 수정/검토 내용: 초록 배경을 soft matte·despill로 제거하고 1024×1536
  알파 PNG를 확인했다. 나무는 해금 상태와 무관하게 고정 먹색으로 유지하며,
  연결선은 7px의 약한 보조선으로 낮추고 빨강은 해금 열매에만 남겼다.

### 2026-08-01 — 성장 지도 상단 확장·축소 조망·꽃봉오리 대비

- 사용 도구: OpenAI Codex 멀티 에이전트, `emil-design-eng` UI 스킬,
  Unity Test Framework
- 목적: 성장 화면 위쪽 여백까지 먹나무 탐색에 사용하고 전체 가지를 조금 더
  넓게 보이게 하며, 복잡한 먹나무 위에서도 39개 꽃봉오리를 즉시 구별하게 한다.
- 주요 프롬프트/지시: “UI 위쪽도 다 쓰고 화면이 축소되어 보이게 하며,
  꽃봉오리 뒤에 원형 검정색 같은 진한 받침을 넣어 가독성을 높인다.”
- 결과물: 성장 뷰포트를 `980×1660`으로 세로 확장해 고정 헤더 뒤 상단까지 배치하고,
  나무 캔버스를 90%로 축소했다. 모든 일반·최종 꽃봉오리 뒤에 입력을 받지 않는
  진한 먹 원형 `NodeContrast`를 추가했다.
- 사람의 수정/검토 내용: 제목·재화·뒤로 가기는 항상 지도보다 앞에 고정하고,
  기존 상태 표면과 충분한 터치 영역은 유지했다. 빨강은 해금된 열매에만 사용하며
  새 애니메이션·파티클·외부 에셋은 추가하지 않았다.

### 2026-08-01 — 잘림 없는 먹나무 v3와 최소 정보 성장 화면

- 사용 도구: OpenAI Codex 멀티 에이전트, OpenAI ImageGen,
  `imagegen`·`emil-design-eng` 스킬, Unity Test Framework
- 목적: 성장 지도 안에서 흰 사각형과 잘린 나뭇가지가 보이는 문제를 없애고,
  반복되는 제목·설명·노드 글자를 줄여 세로 모바일 가독성을 높인다.
- 주요 프롬프트/지시: 기존 먹나무의 수묵 붓결과 단일 나무 실루엣은 유지하되
  모든 뿌리와 가지 끝을 캔버스 안에서 자연스럽게 끝내고 네 변에 충분한 투명
  여백을 두도록 편집했다. UI는 “먹빛을 아이콘으로 간단히 표시하고 영구 성장
  제목을 빼며 글자를 최소화”한다는 사용자 지시를 기준으로 정리했다.
- 결과물: `pg_tree_background_v3.png`, 생성 원본과 프롬프트 기록,
  전체 높이 드래그 뷰포트, 먹빛 아이콘+숫자 HUD, 무글자 열매 지도와 선택 노드
  전용 한 줄 효과·비용 액션.
- 사람의 수정/검토 내용: ImageGen 작업색을 soft matte·despill로 제거하고 최외곽
  네 변 alpha 0, 비투명 경계 `x=52..955`, `y=65..1460`을 확인했다. 완성 나무와
  별도 줄기·장식 가지 중첩을 제거하고 종횡비를 유지했다. 다른 작업자의 미커밋
  메타·씬 변경은 건드리지 않은 격리 사본에서 EditMode 313/313을 통과했다.

### 2026-08-01 — 전체 화면 3대분류 다갈래 먹나무 배치

- 사용 도구: OpenAI Codex 멀티 에이전트, `emil-design-eng` UI 스킬,
  Unity Test Framework
- 목적: 성장 화면 네 변에 남아 있던 고정 여백을 제거하고, 39개 열매를 한 줄
  목록이 아니라 생존·먹 운용·도약의 세 큰 주가지와 실제 곁가지로 읽히게 한다.
- 주요 프롬프트/지시: “상하좌우 여백을 없애고 노드를 일직선으로 만들지 말며,
  여러 갈래로 하되 3종류의 큰 분류로 나눈다.”
- 결과물: 성장 지도 마스크를 고정 HUD의 Safe Area와 분리해 기준 해상도
  `1080×1920` 전체로 확장하고 내부 패딩을 제거했다. 생존은 왼쪽, 먹 운용은
  가운데, 도약은 오른쪽 영역에 두고,
  각 계보의 기존 선행 조건 분기와 최종 패시브 합류가 가로 간격으로 드러나도록
  39개 좌표를 다시 배치했다.
- 사람의 수정/검토 내용: 저장 ID·비용·효과·부모 그래프는 바꾸지 않았다. 첫 화면의
  뿌리와 세 계보 첫 열매가 모두 보이는 범위를 지켰으며, 각 계보의 공간 분리와
  분기 간 최소 가로 간격을 EditMode 회귀 테스트로 고정했다.

### 2026-08-01 — 성장 먹나무 배경 대비 조정

- 사용 도구: OpenAI Codex, Unity Test Framework
- 목적: 큰 먹나무가 전경 열매·간선과 경쟁해 성장 화면 정보가 흐려지는 문제를
  줄인다.
- 주요 프롬프트/지시: “뒤의 큰 나무 투명도를 조절해 가독성을 높인다.”
- 결과물: `InkTreeBackground`의 불투명도를 82%에서 42%로 낮추고, 열매·가지·
  계보 표식의 색과 불투명도는 유지했다.
- 사람의 수정/검토 내용: 배경 나무만 조정되도록 회귀 테스트로 42% 값을 고정하고
  기존 노드 입력 영역과 성장 수치에는 변화가 없음을 확인했다.

### 2026-08-01 — 성장 가지 연결과 전경 가독성 보정

- 사용 도구: OpenAI Codex 멀티 에이전트, Unity Test Framework
- 목적: 모듈형 나뭇가지 일부가 열매 사이에서 끊겨 보이고, 반투명 먹받침과 합성
  Bold 글자가 복잡한 나무 위에서 뭉쳐 보이는 문제를 고친다.
- 주요 프롬프트/지시: “끊긴 나뭇가지를 자연스럽게 잇고 검정색 칸은 100%
  불투명하게 하며 글씨 굵기를 낮춘다.”
- 결과물: 6개 가지 PNG의 실제 alpha 가로 범위를 측정해 논리 간선 양끝을 18px씩
  덮도록 폭과 중심을 보정했다. 열매 받침·계보 표찰은 alpha 1로 맞추고 계보명과
  선택 정보는 Normal 굵기와 약한 외곽선으로 변경했다.
- 사람의 수정/검토 내용: 핵심 재화 숫자·로비·강화 행동은 Bold를 유지했다. 모든
  가지의 보이는 alpha가 접속점 양쪽을 덮는지와 UI 굵기·불투명도를 회귀 테스트로
  고정했다.

### 2026-08-01 — 영구 성장 39열매 1방울 경제·개별 아이콘 기획

- 사용 도구: OpenAI Codex 멀티 에이전트
- 목적: 영구 성장 열매마다 같은 비용 1개를 사용하면서도 너무 빨리 전체를
  완성하지 않도록 보상 속도·효과 상한·선행 관계·39개 개별 아이콘 규칙을 함께 설계
- 주요 프롬프트/지시: “특성 해금 목록부터 작성하고 밸런스를 잡는다. 모든 특성은
  먹물방울 1개로 열며, 비슷한 특성의 이미지는 같은 계열로 보이되 조금씩 다르게
  전부 설정한다.” 현재 957비용/39노드 카탈로그와 실제 보상식·저장 마이그레이션·
  7개 공용 아이콘 사용 구조를 읽기 전용으로 감사했다.
- 결과물: `docs/design/permanent-growth-balance-v2.md`의 생존 8·도약 12·먹 운용
  19개 목록, 정상 판 1개와 고정 이정표 보너스 경제, 완주 34~39보상 판 목표,
  `balanceVersion=2` 보존형 변환안, 39개 256px 아이콘 이름·실루엣 규격.
- 사람의 수정/검토 내용: 월드 50m 상승 아이템과 구분해 UI 용어를 `성장 먹물방울`로
  한정했다. 먹비 구간 수명 페널티가 영구·한 판 성장을 모두 적용해도 남도록 수명을
  최대 +6%로 제한하고, 영구·한 판 낙묵석 방어 중복 후보를 제거하도록 정리했다.
  이번 단계에서는 사용자와 기획을 먼저 다듬기 위해 런타임 코드·저장값·이미지
  파일을 변경하지 않았다.

### 2026-08-01 — 영구 성장 13×3 특성·비기 세팅 재설계

- 사용 도구: OpenAI Codex 멀티 에이전트, 공식 검은사막 자료,
  공식 Zenless Zone Zero 공지와 HoYoLAB 성장 가이드
- 목적: 기존 39열매가 같은 수치 rank를 펼친 목록에 머무는 문제를 검토하고,
  검은사막·젠레스 계열의 단계 성장·조건부 심화·장착 세팅을 먹점프 영구 성장에 번역
- 주요 프롬프트/지시: “검은사막이나 젠존제처럼 특성을 찍는 구조인지 다시
  확인하고, 밸런스를 검토해 필요한 요소를 추가한다.” 저장소의 실제 피격·점프·
  먹·발판·한 판 증강 합성 코드를 읽고 v2의 8/12/19 분량과 1% 반복을 감사했다.
- 결과물: `docs/design/permanent-growth-trait-tree-v3.md`의 생존·도약·먹 운용
  각 13개 목록, 세 계보별 3개 비기와 장착 슬롯, A/B 프리셋, 12m·20초 보상 기준,
  영구 성장과 한 판 증강의 중복·상한 규칙, 저장 및 QA 기준.
- 사람의 수정/검토 내용: 검은사막과 젠레스가 로그라이크 자체는 아니라는 경계를
  명시하고 한 판 랜덤 성장은 기존 `증강`에 남겼다. 분신 수와 곱해지는 최대 체력,
  비정상 위쪽 반동, 장거리 점프 1.30배 상한, 낙묵석 방어 중복과 초보자의 20m
  정체를 별도 위험으로 반영했다. 이번 단계는 기획 확정 전이라 런타임·씬·이미지는
  변경하지 않았다.

### 2026-08-01 — 영구 성장 붉은사막 방향·단일 비기 장착 정정

- 사용 도구: OpenAI Codex
- 목적: 사용자가 의도한 성장 화면 참고작을 검은사막이 아닌 붉은사막식 수묵
  가지 화면으로 바로잡고, 필요 이상으로 추가한 A/B 프리셋 기능을 제거
- 주요 프롬프트/지시: “프리셋 기능은 빼고 검은사막이 아니라 붉은사막 느낌이다.”
- 결과물: `docs/design/permanent-growth-trait-tree-v3.md`에서 프리셋·빠른 세팅을
  삭제하고 계보별 현재 비기 하나만 저장하도록 단순화했다. 붉은사막 참고 범위는
  밝은 종이 전면 캔버스, 큰 수묵 줄기, 다갈래 노드와 최소 글자 구성으로 명시했다.
- 사람의 수정/검토 내용: 13×3 노드, 9개 비기, 계보별 최대 하나 장착과 무료 교체는
  유지했다. 게임 중 교체는 막고 시작 시 현재 장착 상태만 스냅샷으로 고정한다.
  이번 정정은 문서만 변경했으며 런타임·씬·이미지는 수정하지 않았다.

### 2026-08-01 — 영구 성장 공통 줄기·중간 3분기 구현

- 사용 도구: OpenAI Codex 멀티 에이전트, Unity Test Framework,
  `emil-design-eng` UI 설계 스킬
- 목적: 먹나무가 뿌리에서 즉시 세 갈래로 흩어져 성장 순서와 현재 계보를 읽기
  어려운 문제를 해결한다.
- 주요 프롬프트/지시: “큰나무 뿌리부터 중간까지는 공동 노드로 해금하고,
  중간부터 생존·먹 운용·도약 3갈래로 나누며 나뭇가지처럼 불규칙하게 배치한다.”
- 결과물: 기존 stable ID·39 rank·효과·비용·저장 버전을 유지하면서
  `먹그릇 1~2 → 숨고르기 1~2 → 먹결 1`을 실제 구매 선행조건인 단일 공통
  줄기로 묶었다. 마지막 공통 열매에서만 세 계보가 갈라지고, 각 가지를 좌·중·우
  영역 안에서 지그재그로 배치했으며 계보 표찰도 실제 분기점으로 이동했다.
- 사람의 수정/검토 내용: 씬 빌더·Main 씬·다른 작업자의 신규 PNG는 수정하지
  않았다. 단일 루트, 공통 5노드 순서, 정확한 세 입구, 비순환 그래프와 모바일
  터치 영역 비중첩을 회귀 테스트로 고정했다.

### 2026-08-01 — 성장 가지 완전 표시·노드 상세 팝업·개발 디버그

- 사용 도구: OpenAI Codex 멀티 에이전트, `emil-design-eng` UI 설계 스킬,
  Unity Test Framework
- 목적: 흐린 나뭇가지 때문에 해금 경로가 끊겨 보이는 문제를 없애고, 열매를
  누른 뒤 효과를 이해하지 못한 채 바로 구매하게 되는 흐름을 상세 확인형으로
  바꾸며 성장 QA 반복 시간을 줄인다.
- 주요 프롬프트/지시: “나뭇가지 투명도를 100%로 하고, 성장창에 초기화와
  먹빛 999 디버그 메뉴를 넣으며, 노드를 누르면 설명 팝업과 강화하기 버튼을
  보여준 뒤 이펙트를 재생한다.”
- 결과물: 모든 실제 가지 조각의 UI alpha를 해금 전후 1로 고정했다. 노드 선택 시
  계보·단계, 이름, 설명, 현재 누적 효과, 해금 후 효과, 선행 조건과 비용을 보여주는
  중앙 한지 팝업을 열고, 강화 성공 시 팝업을 닫은 뒤 기존 먹획·열매 해금 연출을
  그대로 재사용한다. 에디터/개발 빌드에서만 보이는 `DEBUG` 메뉴에 `초기화`와
  `먹빛 999`를 추가했다.
- 사람의 수정/검토 내용: 디버그 재화는 총 비용 957의 실서비스 저장 상한을
  변경하지 않는 세션 override로 분리했다. 초기화는 영구 rank와 소비량만 지우고
  첫 보상 여부·마지막 정산 ID는 보존해 중복 보상을 막는다. 상세 팝업 중에는
  나무 드래그를 잠그고, 제출용 비개발 빌드에는 디버그 UI와 API가 포함되지 않게
  전처리했다. 후속 검토에서 버튼명을 `노드 초기화`로 명확히 바꾸고, 이미 해금되어
  붉어진 열매가 초기화 직후 투명한 잠금 상태로 돌아가는 UI 회귀 테스트를 추가했다.

### 2026-08-02 — 영구 성장 v3 39열매·비기 장착·실게임 연결

- 사용 도구: OpenAI Codex 멀티 에이전트, Unity Roslyn 정적 컴파일,
  `emil-design-eng` UI 설계 스킬
- 목적: 개발용 성장 디버그는 유지하면서 구 11트랙·큰 비용 구조를 13×3 직접
  노드 구조로 교체하고, 저장·보상·비기·실게임 효과·모바일 팝업을 같은 계약으로 완성
- 주요 프롬프트/지시: “디버그는 개발용이므로 그대로 두고 나머지를 고친다.”
  밸런스·아키텍처·테스트 담당이 39개 노드, 런 스냅샷, 마이그레이션, UI 장착 상태와
  실제 피해·점프·먹·발판 경로를 독립 감사했다.
- 결과물: `PermanentGrowthCatalog`의 생존·도약·먹 운용 각 13개 stable-ID 열매,
  전 노드 비용 1, 계보별 비기 3개와 단일 장착, `balanceVersion=2` 마이그레이션,
  첫 판/12m·20초/신기록 이정표 보상, 최근 run ID 중복 정산 보호,
  `PermanentGrowthRunSnapshot`과 플레이어·점프·드로잉·발판·바람 효과 연결,
  빨간 해금 열매·금빛 장착 고리·먹색 선택 고리·2단계 비기 교체 팝업을 구현했다.
- 사람의 수정/검토 내용: J-KB가 그린 발판 배율에 중복되어 2.03배가 되던 경로,
  J-KA 3연속 착지가 이미 충전된 판에서 효과가 없던 경로, 자동 점프 낙하 특성이
  먹물방울·풍맥에도 섞이던 경로, 여유 먹을 저먹 판정에서 누락한 경로를 수정했다.
  19.5:9·20:9 화면은 실제 논리 폭에 맞춰 먹나무만 축소하고, 팝업 긴 문장은
  최소 글자 크기까지 자동 맞춤하며, 실제 노드 버튼·레이캐스트·비기 교체 링을 검증했다.
  Runtime·전체 Editor C# 정적 컴파일과 39노드·저장·보상·비기·실게임·UI 회귀
  테스트 컴파일을 통과했다. Unity 에디터가 열려 있어 Test Runner 실행은 후속
  실제 Game View QA 항목으로 남겼으며, 디버그 초기화·먹빛 999는 개발 빌드에 유지했다.

### 2026-08-02 — 성장 노드 유기적 배치와 상세 정보 위계 개선

- 사용 도구: OpenAI Codex 멀티 에이전트, Unity Roslyn 정적 컴파일,
  `emil-design-eng` UI 설계 스킬
- 목적: 도약 노드의 터치 영역이 세로 열에서 거의 맞닿고, 상세 팝업의 정보가 중앙
  적층으로 반복되어 성장 경로와 강화 효과를 빠르게 읽기 어려운 문제를 개선한다.
- 주요 프롬프트/지시: 붉은사막 특성 화면을 참고하되 나뭇가지처럼 적당히 불규칙한
  세 갈래 구조로 만들고, 상세창은 아이콘·제목·비용·적용 방식·핵심 효과·설명을
  간결한 위계로 정리한다.
- 결과물: 도약 계보를 뿌리에서 서서히 벌어지는 비대칭 부채꼴로 재배치하고 신규
  5단계 노드까지 동일 좌표 규칙을 적용했다. 팝업에는 먹색 정보판과 `적용 방식`
  행을 추가하고, 상태·효과·설명의 중복 문구와 합성 외곽선을 줄였다.
- 사람의 수정/검토 내용: 영구 성장 효과·비용·저장 데이터는 변경하지 않았다.
  UI 브랜치가 게임 브랜치의 도약 19노드 커밋과 합쳐질 것을 고려해 stable ID 기반
  표시 좌표만 재정의했으며, 런타임·Editor C# 정적 컴파일과 터치 영역 비중첩 테스트
  컴파일을 통과했다. 열린 Unity가 별도 복제본이어서 실제 Game View QA는 올바른
  `feature/ui` 프로젝트를 열어 확인해야 한다.

### 2026-08-02 — GitHub 첫 화면 게임 설명서 재구성

- 사용 도구: OpenAI Codex 멀티 에이전트
- 목적: 구현 세부와 디버그 설명이 길게 섞인 저장소 첫 화면을 처음 보는 플레이어와
  심사자가 게임의 목표·조작·콘텐츠·성장·실행 방법을 빠르게 이해하는 요약 설명서로 정리
- 주요 프롬프트/지시: “게임 요약문서를 GitHub 첫 페이지에 게임 설명서처럼 올리고,
  앞으로 게임 기획이 변경될 때마다 함께 수정한다.”
- 결과물: `README.md`를 게임 한눈에 보기, 플레이 방법, 조작, 발판·먹떼·세계 변화,
  아이템 5종, 영구 성장과 현재 판 증강 비교, 한 판 흐름, Unity 실행법, 기술 특징과
  상세 문서 링크 순으로 재구성했다. `CLAUDE.md`에는 기획·밸런스·콘텐츠·조작·성장
  변경 시 README를 같은 기능 커밋에서 갱신하는 영구 규칙을 추가했다.
- 사람의 수정/검토 내용: 저장소의 현재 코드와 프로젝트 브리핑을 교차 확인해 구현된
  기능만 소개하고, APK 다운로드나 완료되지 않은 온라인 기능처럼 검증되지 않은 내용은
  넣지 않았다. 내부 디버그 수치와 긴 풀링 세부는 첫 화면에서 제거하고 상세 문서로 분리했다.

### 2026-08-02 — 도약 성장 5단계 3갈래·구조 비기 구현

- 사용 도구: OpenAI Codex 멀티 에이전트, Unity Roslyn 정적 컴파일
- 목적: 도약 성장을 충전·점프력·점프 높이의 세 경로로 명확히 나누고, 각 경로를
  다섯 번 성장한 뒤 플레이 방식을 바꾸는 구조 비기를 해금하도록 확장한다.
- 주요 프롬프트/지시: “점프만 먼저 적용한다. 충전 시간·점프력·점프 높이를 각각
  5칸으로 나누고, 마지막에는 벽 매달리기·5회마다 안전 발판·2단 점프를 해금한다.
  세 갈래는 살짝 다른 색으로 보여 가시성을 높인다.”
- 결과물: 도약 계보를 공통 뿌리 1개와 5단계 3갈래, 구조 비기 3개를 포함한
  19개 노드로 확장했다. 충전은 총 6% 단축, 점프력은 총 5% 증가, 목표 정점 높이는
  총 6.25% 증가하도록 구성했고, 벽 매달리기 후 자동 점프·대표 캐릭터의 정상 점프
  5회마다 6초짜리 단방향 안전 발판·공유 12초 재사용 대기시간의 40% 2단 점프를
  실제 플레이에 연결했다. 세 경로의 연결선과 노드 대비색은 저채도 녹색·남색·자주색으로
  구분했다.
- 사람의 수정/검토 내용: 분신 수만큼 점프 횟수가 중복 집계되지 않도록 카메라가
  따라가는 먹떼 대표의 정상 자동 점프만 공유 카운터에 포함했다. 먹물방울·풍맥 같은 특수 상승은
  2단 점프와 안전 발판 집계에서 제외하고, 기존 저장의 도약 비기가 새 5단계 경로에서
  끊기지 않도록 `balanceVersion=3` 순차 마이그레이션을 추가했다. 수평 점프력과 목표
  높이 배율이 동시에 과도하게 곱해지지 않도록 높이는 제곱근 속도 배율로 환산했다.
  Runtime과 Editor 테스트 어셈블리는 Unity Roslyn 정적 컴파일을 통과했다. Unity
  에디터가 다른 작업 트리에서 열려 있어 Test Runner 실 실행은 후속 Game View QA로 남겼다.
- 외부 에셋/라이선스: 새 외부 에셋 없음. 경로 색상과 안전 발판 시각은 코드 기반으로 생성.

### 2026-08-03 — 한 판 증강·도감 제거와 영구 성장 39노드 단일화

- 사용 도구: OpenAI Codex 멀티 에이전트, ripgrep, Git diff
- 목적: 제출 일정에 맞춰 플레이 중 무작위 증강·성장 두루마리·카드 선택과 100종
  도감을 제거하고, 로비의 영구 성장 먹나무 하나만 남겨 게임 규칙과 UI를 단순화한다.
- 주요 프롬프트/지시: “로그라이크 요소와 게임 내 증강, 관련 문서, 도감을 모두
  빼고 영구 성장만 유지한다. 성장은 세 분류마다 세 줄기로 단순화한다. 긴 최종
  테스트는 요청할 때만 진행한다.”
- 결과물: 증강 카탈로그·선택 UI·월드 두루마리 스폰과 전용 카드 자산의 제거에 맞춰
  README, 프로젝트 브리핑, 아키텍처, 로비 UI 규격과 VFX 적용 문서를 현재 구조로
  갱신했다. 영구 성장은 `생존 / 도약 / 먹 운용` 각 13개,
  `뿌리 1 + 세 경로×(일반 3 + 비기 1)`의 총 39노드로 확정했다.
- 사람의 수정/검토 내용: 기존 AI 활용 내역은 제출 증빙의 역사 기록이므로 삭제하거나
  고쳐 쓰지 않고 이번 결정만 새 항목으로 추가했다. 결과창의 한지 두루마리처럼 증강과
  무관한 기존 표현은 유지했다. 장시간 Unity Test Runner와 반복 플레이는 사용자 요청
  전까지 실행하지 않고 빠른 참조·문서 diff 검사만 수행한다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-07 — README 심사용 Android 빌드 링크

- 사용 도구: OpenAI Codex, Git
- 목적: 심사자가 저장소 첫 화면에서 플레이어블 Android 빌드를 즉시 찾을 수 있게 한다.
- 주요 프롬프트/지시: “Google Drive APK 링크를 README 맨 위에 올린다.”
- 결과물: 프로젝트 제목 바로 아래에 NHN NAN 2026 심사용 APK 다운로드 링크를 추가했다.
- 사람의 수정/검토 내용: 사용자가 제공한 Google Drive 공유 URL을 그대로 사용했으며,
  기존 게임 설명과 소스 구조는 변경하지 않았다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-06 — 성장 결실 꼭대기 열매·도약·먹 운용 패시브

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 성장 저장·UI·밸런스 감사,
  ripgrep, Unity Roslyn 정적 컴파일
- 목적: 일반 성장과 구분되지 않던 마지막 결실을 먹나무 꼭대기의 목표 열매로
  시각화하고, 생존에만 있던 규칙형 결실을 도약·먹 운용에도 추가한다.
- 주요 프롬프트/지시: “결실 칸을 성장 UI에서 특별하게, 나무 꼭대기쯤에 두고
  도약과 먹 운용에도 결실 패시브를 넣는다.”
- 결과물: 기존 39개 stable ID·비용·저장 구조는 유지했다. 9개 K 열매를 일반
  노드보다 큰 터치 면·큰 붉은 열매·상시 금빛 후광으로 만들고 세 잔가지 꼭대기에
  왕관형으로 재배치했다. `돋는 먹발 결실`은 직접 그린 먹선 도약 힘 +6%,
  `높은 먹발 결실`은 마지막 생존자의 하단 낙하를 18초마다 35% 완화한다.
  `넓은 벼루 결실`은 총량 안의 먹선 선명 유지 +0.6초, `가는 붓끝 결실`은
  1.5m 이하 짧은 획의 먹 소모를 추가 10% 줄인다.
- 사람의 수정/검토 내용: 점프·먹 총 수치 상한은 기존 값을 유지하고 결실 효과만
  덧붙였다. 장착·교체 없이 이미 소유한 K ID에서 바로 파생하므로 구 저장도 자동으로
  새 패시브를 받는다. 총량 초과 FIFO 제거는 선명 유지 보너스로 지연하지 않으며,
  낙하 완화도 위로 튀기지 않고 현재 하강 속도만 줄인다. 새 외부 이미지 없이 기존
  성장 열매·고리 스프라이트를 재사용했다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-06 — 최고 생존자 카메라·좌우 벽 해태 즉시 예고

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 카메라·해태·성장 UI 감사,
  ripgrep, Unity Roslyn 정적 컴파일
- 목적: 분신이 본체보다 높이 올라가거나 본체가 죽었을 때 카메라 기준이 애매해지는
  문제를 없애고, 벽타기 대응 해태 경고를 시작 직후 바로 확인할 수 있게 한다.
- 주요 프롬프트/지시: “카메라는 가장 높은 애 기준으로 하고, 벽 장애물 해태는
  왼쪽·오른쪽 중 무작위 한쪽 벽의 위에서 아래로 쭉 지나가게 한다.”
- 결과물: 카메라는 본체·분신 구분 없이 가장 높은 생존자를 Y축으로 부드럽게 추적하고,
  그 선두가 죽으면 다음 최고 생존자로 한 번 재구도한다. 위험물·맵 진행은 기존 하위
  중앙값을 유지한다. 최초 튜토리얼이나 일시정지가 닫힌 첫 유효 프레임에 무작위 좌·우
  벽 한쪽에 해태 경고를 예약하며, 해태는 플레이어를 조준하지 않고 그 벽의 위에서
  아래 화면 밖까지 곧게 통과한다. 성장 지도 마지막 수치 단계는 일반 열매와 같은 크기로 통일하고 과거 장착용
  금색 고리를 제거했다.
- 사람의 수정/검토 내용: 해태 본체가 즉시 충돌하는 것이 아니라 기존 1.1~1.2초
  느낌표·붉은 먹빛 예고를 먼저 보여 공정성을 유지한다. 먹선에는 막히지 않고 선택된
  벽의 아래 화면 밖까지 통과한다. 사용자 지시에 따라 Unity Play와 전체 Test Runner는
  실행하지 않고 정적 컴파일만 수행한다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-06 — 먹해태 수직 벽 하강 최종 방향 확정

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 경로 감사, ripgrep,
  Unity Roslyn 정적 컴파일
- 목적: 해태가 플레이어 높이를 향해 수평으로 달려오는 연출을 없애고, 모바일에서
  예측 가능한 간헐적 벽 장애물로 고정한다.
- 주요 프롬프트/지시: “플레이어를 추적하거나 꺾지 말고, 왼쪽·오른쪽 벽 중 무작위
  한쪽의 위에서 아래로 쭉 내려와 화면 밖으로 지나가게 한다.”
- 결과물: 첫 유효 플레이 프레임 1회와 이후 기존 320m/12% 슬롯마다 무작위 벽 한쪽만
  선택한다. 선택된 벽에 세로 붉은 띠와 느낌표를 1.1~1.2초 먼저 표시하고, 해태는
  카메라 위 바깥에서 아래 바깥까지 고정 X로 이동한다. 경로 계산에서 플레이어 좌표를
  제거했으며 먹선·일반 발판·풍맥에도 멈추지 않는다. 첫 생존자 접촉 뒤에는 추가 피해
  판정만 끄고 이동 자체는 끝까지 유지한다.
- 사람의 수정/검토 내용: 사용자의 최종 방향 정정에 따라 아래→위가 아니라 위→아래로
  고정했다. Play와 전체 Test Runner는 실행하지 않고 정적 컴파일만 수행한다.
- 외부 에셋/라이선스: 새 외부 에셋 없음. 기존 먹해태 4프레임 시트와 경고 자식을 재사용했다.

### 2026-08-06 — 성장 상단 재화·정보 가독성 정렬

- 사용 도구: OpenAI Codex, ripgrep, Unity Roslyn 정적 컴파일
- 목적: 성장 화면 상단의 재화가 오른쪽으로 치우치고 세 계보 요약 글자가 작고 굵게
  뭉쳐 보이는 문제를 해결한다.
- 주요 프롬프트/지시: “재화 표시는 정가운데, 정보 텍스트는 키우고 Bold를 뺀다.”
- 결과물: 재화 HUD의 기준점을 화면 수평 중앙으로 옮기고 아이콘·수량을 중앙 묶음으로
  재배치했다. 누적 거리 글자는 30px, 생존·도약·먹 운용 정보는 34px로 확대했다.
  재화·누적 거리·세 요약의 FontStyle과 합성 Outline을 모두 끄고 색상 구분만 유지한다.
- 사람의 수정/검토 내용: 자동 글자 축소는 사용하지 않으며 세 요약 영역의 높이를 함께
  늘려 줄바꿈이나 잘림 없이 표시되도록 했다. Play와 Test Runner는 실행하지 않았다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-04 — 총 먹자리 HUD·피격 크기·먹떼 카메라 누락 보완

- 사용 도구: OpenAI Codex 멀티 에이전트, ripgrep, Unity Roslyn 정적 컴파일
- 목적: 총 먹자리 전환 뒤 최대량이 보이지 않던 하단 HUD와 다중 개체 체력·카메라의
  경계 조건을 사용자 기획과 일치시킨다.
- 주요 프롬프트/지시: 총 먹 게이지가 너무 짧고 최대치 UI가 적용되지 않았는지 확인하고,
  체력별 피격 확대와 본체·상위 분신 사망 뒤 카메라 기준까지 빠짐없이 보완한다.
- 결과물: 기본 먹자리를 24m로 늘리고 붓 여유는 25% 누적으로 재조정했다. 게이지는
  현재 획의 예상 점유까지 실시간 차감하며 남은 값/실제 최대값과 최대치 변화 폭을 함께
  표시한다. 최대 체력 4 이상에서도 매 피격마다 렌더만 커지고, 상단 추적을 이끌던
  높은 개체 사망 뒤에는 남은 먹떼로 한 번만 카메라를 재구도한다.
- 사람의 수정/검토 내용: 평상시 카메라는 추락을 따라 내려가 사망선을 무력화하지 않도록
  상승 전용을 유지하고, 실제 사망 알림이 있을 때만 0.5초 하강 재구도를 허용했다.
  Play 중 스크립트 재컴파일 뒤에도 하단 HUD가 `StrokeCapture`를 자동 재연결하고,
  구형 Main에 재구도 필드가 없어도 46%·0.5초 안전값을 쓰도록 연결 경계를 보강했다.
  사용자 요청에 따라 Unity Play와 전체 Test Runner는 실행하지 않는다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-04 — 회복형 먹 게이지를 총 먹자리 FIFO로 전환

- 사용 도구: OpenAI Codex 멀티 에이전트, ripgrep, Unity Roslyn 정적 컴파일
- 목적: 먹을 쓰고 기다려 회복하는 규칙을 없애고, 그린 길이 자체를 관리하는
  드로잉 중심 자원으로 전환한다.
- 주요 프롬프트/지시: 하단 먹 게이지를 그릴 수 있는 총량으로 사용하고 이미 그린
  먹선은 계속 남기되, 총량을 넘기면 가장 먼저 그린 부분부터 점점 사라지게 한다.
  이에 맞춰 먹 계열 영구 성장 패시브와 밸런스를 전부 교체한다.
- 결과물: 당시 기본 총 먹자리 18m와 최종 유효 스트로크 길이 ledger를 추가했다. 새 획은
  먹 부족으로 막히지 않으며 초과분만 FIFO로 가장 오래된 획의 시작점부터 기본
  1.1초 동안 비주얼과 충돌체가 함께 줄어든다. 황금 붓은 8초간 정산을 보류하고,
  붓 여유는 당시 판 한정 용량을 기본값의 35%씩 누적했다(후속 조정으로 24m·25%). 먹 운용 13노드는 총량·점유
  절약·소멸 완화 세 갈래로 교체하고 생존의 피격 먹 회복도 피격 무적으로 바꿨다.
- 사람의 수정/검토 내용: 저장 호환을 위해 39개 stable node ID와 기존 enum 숫자를
  유지하고 신규 효과 타입만 뒤에 추가했으며 의미 변경을 `balanceVersion=6`으로
  구분했다. 먹비 구간은 발판 시간 단축 대신 총
  먹자리 0.72배를 적용한다. 사용자 요청에 따라 Play와 전체 Test Runner는 실행하지
  않고 Runtime·Editor 정적 컴파일 및 정적 회귀 계약만 확인한다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-03 — 먹분신 인접 생성 위치 회귀 수정

- 사용 도구: OpenAI Codex 멀티 에이전트, ripgrep, Git diff
- 목적: 먹분신이 획득 캐릭터 바로 옆이 아니라 화면 반대편과 다른 높이에 멀리
  생성되는 회귀를 제거한다.
- 주요 프롬프트/지시: “분신은 캐릭터 기준 바로 옆으로 나와야 하며 왼쪽으로
  생성될 때도 너무 멀리 떨어지지 않게 한다. 긴 최종 테스트는 요청할 때만 진행한다.”
- 결과물: `GameManager.FindCloneSpawnPosition`의 화면 절반 65후보 최원거리 탐색을
  제거하고, 원본 SpriteRenderer와 Collider의 합친 외곽 폭에서 0.1m 옆에 같은
  높이로 생성하도록 변경했다. 화면 중앙은 좌우 교대, 가장자리는 안쪽 우선 규칙을
  유지하고, 생성 직후 분신을 바깥으로 밀던 0.45m/s 속도도 제거했다.
- 사람의 수정/검토 내용: 기본 반지름 0.4 Collider뿐 아니라 Tight 스프라이트의 실제
  월드 외곽도 포함해 회전·프레임 변화에서 겹침을 줄였다. 기존 원거리 분산 계약 테스트를
  인접 거리·같은 높이·동일 초기 속도 계약으로 교체했으며, 사용자 요청에 따라 장시간
  Unity Test Runner는 실행하지 않는다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-03 — `main` 단일 프로젝트·브랜치 운영 전환

- 사용 도구: OpenAI Codex, Git merge-tree, Git worktree·branch 상태 감사
- 목적: UI와 게임을 별도 폴더·브랜치에서 병렬 작업하며 생긴 경로 혼동을 끝내고,
  실제 Unity 프로젝트 하나에서 변경을 즉시 확인할 수 있게 저장소 운영을 단순화한다.
- 주요 프롬프트/지시: “앞으로 브랜치 하나로만 진행하고 메인으로 바로 작업한다.
  기존 작업을 메인과 머지하고 Unity 프로젝트를 실행해 변경을 바로 확인하게 한다.”
- 결과물: 커밋된 `feature/game`과 `feature/ui` 작업을 일반 merge commit으로 `main`에
  통합했다. `AGENTS.md`, `CLAUDE.md`, `docs/branch-policy.md`, 프로젝트 브리핑을
  `main` 단일 작성자·작은 기능 커밋·즉시 push 흐름으로 갱신하고, 더 이상 존재하지
  않는 UI 작업선을 대상으로 하던 평일 자동 머지 workflow를 제거했다.
- 사람의 수정/검토 내용: 기존 AI 활용 로그의 브랜치 기록은 당시 협업 이력 증빙이므로
  고쳐 쓰지 않았다. 과거 UI 워킹트리의 미커밋 자동 임포트·실험 파일은 기능 브랜치
  커밋에 포함되지 않았음을 확인했으며, 증강 제거 결정과 충돌하는 미추적 카드 자료는
  새 `main`에 복원하지 않았다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-03 — 성장 상세창 간결화·도약 계보 겹침 수정

- 사용 도구: OpenAI Codex 멀티 에이전트, ripgrep, Git diff
- 목적: 영구 성장 노드 상세창의 반복 정보와 카드 밖으로 보이는 행동 요소를 줄이고,
  먹 운용과 도약 계보의 열매가 겹쳐 선택하기 어려운 배치를 바로잡는다.
- 주요 프롬프트/지시: “아이콘과 이름은 유지하고 상태·적용 방식·핵심 효과 행을
  삭제한다. `점프 높이 +##` 같은 수치와 설명만 남기며 도약 노드를 오른쪽으로 민다.”
- 결과물: 상세창 본문을 효과 수치 한 줄과 설명으로 축약하고 비용·강화·닫기 요소를
  짧아진 한지 카드 안쪽으로 재배치했다. 도약 계보 전체를 오른쪽 300px 이동하고
  왼쪽 비기는 140px 더 띄워 먹 운용 터치 영역과 분리했으며 지도 폭도 함께 넓혔다.
- 사람의 수정/검토 내용: 구매·비기 장착 기능과 버튼의 잠금 문구는 유지하되,
  화면 본문에서 중복되던 상태 문장은 제거했다. 장시간 Test Runner와 Play 검증은
  사용자 요청에 따라 실행하지 않고 컴파일 로그와 정적 UI 계약만 확인한다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-03 — 성장 상세창 상단 안전 여백·설명 굵기 수정

- 사용 도구: OpenAI Codex, ripgrep, Unity Roslyn 정적 컴파일
- 목적: 간결화한 성장 상세창에서 아이콘·이름·닫기 붓획이 불규칙한 한지 테두리와
  검은 정보판 상단을 넘어 잘리고, 설명문 외곽선이 Bold처럼 뭉치는 문제를 수정한다.
- 주요 프롬프트/지시: “닫기부터 안쪽 아이콘과 글자들이 위로 튀어나와 안 보인다.
  설명글에는 볼드체를 빼 달라.”
- 결과물: 상단 헤더를 57~70px 아래로, 닫기 버튼을 54px 아래·20px 안쪽으로 옮겼다.
  설명문은 Normal을 유지하면서 합성 Outline을 비활성화하고 명도만 높였다.
- 사람의 수정/검토 내용: 실제 PNG의 불규칙한 가시 경계를 고려해 단순 Rect 경계보다
  추가 상단 안전 여백을 계약으로 고정했다. 사용자 요청 전 Play와 전체 Test Runner는
  실행하지 않는다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-03 — 미사용 이미지·AI 중간 산출물 정리

- 사용 도구: OpenAI Codex 멀티 에이전트, ripgrep, SHA-256 중복 감사, Git
- 목적: Unity 런타임과 제출 증빙에 필요하지 않은 구버전·미사용 이미지를 제거해
  자산 목록을 현재 구현과 일치시키고 유지보수 혼선을 줄인다.
- 주요 프롬프트/지시: “프로젝트 안의 필요 없는 PNG와 사용하지 않는 AI 생성
  이미지를 정리한다. 실제 참조와 동적 Resources 경로를 확인한 뒤 삭제한다.”
- 결과물: 현재 v3로 교체된 용 애니메이션 시트와 성장 나무 v2, 런타임에서 읽지
  않는 성장 UI 마스크·꽃·화폐 배지, 최종 v2 해태로 교체된 참조 없는 생성 원본을
  제거했다. 외부 폴더에 불필요하게 남은 Unity `.meta` 두 개도 함께 정리했다.
- 사람의 수정/검토 내용: 98개 래스터 이미지의 GUID·파일명·동적 `Resources.Load`
  참조와 SHA-256을 대조했다. 완전 중복은 없었으며, 캐릭터 원본·폴백 자산·붓 전환
  프레임과 `docs/ai-artifacts`의 프롬프트/생성 계보 증빙은 유지했다. 구 용 시트에
  의존하던 회귀 테스트는 임시 잘못된 텍스처를 만드는 방식으로 바꿔 같은 계약을
  유지한다.
- 외부 에셋/라이선스: 새 외부 에셋 없음. 제거 대상은 기존 프로젝트 내부 생성물.

### 2026-08-03 — 성장 상세창 고정 글자·모바일 닫기 영역 계약 수정

- 사용 도구: OpenAI Codex, 테스트 전담 작업의 정적 QA 보고, ripgrep
- 목적: 성장 상세 설명이 작은 화면에서 임의로 축소되는 문제와 닫기 버튼의 작은
  터치 영역, 계보 붓획과 이름 영역의 미세한 겹침을 제거한다.
- 주요 프롬프트/지시: 자동 글자 축소 금지 UI 규격과 120px 최소 터치 높이를
  위반한다는 테스트 전담 QA 결과를 현재 작업에 함께 반영한다.
- 결과물: 효과 수치는 34px, 설명은 30px 고정 크기를 유지하도록 Best Fit을
  제거했다. 닫기 버튼을 150×120px로 확장해 카드 안전 영역 안에 재배치하고,
  강화 버튼도 120px 터치 높이로 통일했다.
  계보 붓획을 15px 내려 이름과 11px 간격을 확보했다.
- 사람의 수정/검토 내용: 기존 회귀 테스트가 잘못된 Best Fit 동작을 필수 계약으로
  고정하던 부분을 반대로 수정하고, 최소 터치 높이와 두 텍스트 영역의 비겹침을
  정적 UI 계약으로 추가했다. 사용자 요청 전 Play와 전체 Test Runner는 실행하지 않는다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-03 — 일시정지 제목 중앙 정렬·성장 상세창 굵기 통일

- 사용 도구: OpenAI Codex, ripgrep, Unity Roslyn 정적 컴파일
- 목적: 일시정지 두루마리 제목의 좌측 치우침을 없애고, 성장 상세창에 남은 Bold와
  합성 외곽선을 전부 제거해 사용자가 지정한 가벼운 정보 위계를 적용한다.
- 주요 프롬프트/지시: “일시정지의 잠시 멈춤도 가운데 정렬하고, 성장 설명창의
  볼드체를 전부 뺀다.”
- 결과물: `잠시 멈춤` 제목의 기준점과 정렬을 패널 수평 중앙으로 변경했다. 성장
  상세창의 계보·이름·효과·설명·비용·강화·닫기 글자를 모두 Normal로 통일하고,
  버튼 공통 스타일이 추가하던 외곽선까지 상세창 범위에서 비활성화했다.
- 사람의 수정/검토 내용: 다른 화면의 공통 행동 버튼과 성장 지도 HUD의 Bold는
  유지하고, 상세창 하위 Text 전체가 Normal이며 Outline이 꺼지는 회귀 계약과
  일시정지 제목 중앙 좌표 계약을 추가했다. Play와 전체 Test Runner는 실행하지 않는다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-03 — 성장 저장 복구·풍맥 디버그 수명·방어막 중복 획득 보강

- 사용 도구: OpenAI Codex 멀티 에이전트, ripgrep, Unity Roslyn 정적 컴파일
- 목적: QA가 보고한 영구 성장 저장 덮어쓰기, 미래 버전 다운그레이드, 디버그 풍맥
  누적, 중복 방어막 소비와 복구 UI 미연결 문제를 제출 전 닫는다.
- 주요 프롬프트/지시: 손상·미지원 저장 원문을 보존하고 검증된 백업/격리 복구를
  제공하며, 풍맥 왕복 디버그와 비중첩 방어막 획득 계약을 회귀 테스트로 고정한다.
- 결과물: 정상 성장 저장은 primary 확정 뒤 backup을 갱신해 중간 종료 때 backup만
  앞선 세대로 남는 상황을 막고, 손상·미래 저장은
  읽기 전용으로 잠가 자동 덮어쓰기와 진행도 삭제를 막았다. 성장 화면에 백업 복원과
  2회 확인 초기화를 연결하고 복구 전 게임 시작을 차단했으며 결과창에도 저장 실패를
  표시한다. 현재 v5 필수 payload가 잘린 JSON도 손상으로 분류하고, 기존 정상 v5는
  primary 재기록 없이 backup만 최초 백필한다. v2부터 존재한 정산 이력 필드가
  절단된 구 저장도 복구 상태로 분리한다. 디버그 고도 리셋/Spawner 비활성화 때 소유 풍맥을 정리하고, 방어막 보유
  중 두 번째 픽업은 소비하지 않도록 변경했다.
- 사람의 수정/검토 내용: primary 원문은 명시적 복구 전에는 이동조차 하지 않고,
  복구 시 quarantine에 남긴다. 미래 balance의 미지 노드는 역직렬화하지 않으며 모든
  변경 API를 차단한다. 사용자 지시에 따라 Play와 전체 Test Runner는 실행하지 않고
  Runtime·Editor 전체 Roslyn 정적 컴파일만 사용한다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-04 — 제출 전 저장 트랜잭션·게임플레이 경계 회귀 보강

- 사용 도구: OpenAI Codex 멀티 에이전트, 테스트 전담 정적 QA, ripgrep,
  Unity 6000.3.10f1 Roslyn 정적 컴파일
- 목적: 영구 성장과 최고 기록의 부분 저장, 드로잉 할인·분할, 마지막 사망 틱,
  비기 예약과 결과창 재시도처럼 짧은 테스트에서 놓치기 쉬운 경계를 제출 전에
  데이터 보존 우선으로 정리한다.
- 주요 프롬프트/지시: 전체 코드 감사에서 보고된 저장 원자성·복구 접근성·UI 터치
  규격·플레이 경계 결함을 수정하되, 사용자가 요청하기 전 Play와 전체 Test Runner는
  실행하지 않고 빠른 Runtime/Editor 컴파일로 마무리한다.
- 결과물: 영구 성장 저장을 `balanceVersion=5`로 올리고 이정표 지급 watermark를
  필수 payload로 고정했다. primary→backup 동기화 target, reset/restore intent,
  quarantine과 읽기 전용 복구를 분리해 손상·미래·일시 I/O 실패에서 자동 초기화나
  오래된 backup 덮어쓰기를 막았다. 최고 기록은 실패 상태별 자동 재시도와 단조 저장을
  사용하고, 사용자가 재시도를 중단한 프레임부터 전환 완료까지 다시 정산하지 않는다.
  드로잉은 30m 경계를 보간해 중복 발판과 틈을 없애고 실제 먹 비용 판정 시 해금된
  짧은 획/휴식 할인을 적용한다. 자연 소멸 환급 원인을 보존하고, 특수 상승은 일반
  2단점프의 먹떼 공용 예약을 즉시 반환한다. 마지막 물리 틱의 고도·시간·다중 사망
  대표값, 4칸 체력 HUD, 중복 방어막과 디버그 풍맥 정리도 회귀 계약으로 고정했다.
- 사람의 수정/검토 내용: 손상 저장은 자동 복구보다 원문 보존과 명시 선택을 우선하고,
  기록 저장의 flush 결과가 모호한 경우에는 ‘포기’로 단정하지 않고 재시도 중단으로
  안내한다. Runtime·Editor Roslyn 정적 컴파일과 `git diff --check`를 통과했으며,
  사용자 요청에 따라 Play와 전체 Unity Test Runner는 실행하지 않았다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-04 — 개체별 체력 표시·피격 확대·먹떼 카메라 구도

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 감사, ripgrep,
  Unity Roslyn 정적 컴파일
- 목적: 본체와 최대 24분신의 체력을 각 개체에서 바로 읽게 하고, 작은 캐릭터의
  시인성과 본체 사망·분신 선행 상황의 카메라 구도를 개선한다.
- 주요 프롬프트/지시: “체력은 캐릭터 위에 개별로 표시하고 캐릭터를 더 크게 보이게
  한다. 체력이 깎일수록 조금 더 커지게 하며, 본체가 죽거나 분신이 더 높이 올라갈
  때도 카메라 기준이 애매하지 않게 한다.”
- 결과물: 월드 Canvas 없이 상태별 공유 스프라이트를 쓰는 개체별 체력 막대를 추가하고
  기존 상단 통합 체력 HUD를 제거했다. 캐릭터 PPU를 기본 780, 1회 피격 735, 2회
  피격 690으로 설정해 Collider를 건드리지 않고 그림만 단계적으로 키웠다. 드로잉
  안전 영역도 확대된 스프라이트 외곽을 반영한다. 카메라는 하위 중앙값을 부드러운
  중심으로 유지하면서 상위 75% 지점을 90% 화면 안전선으로 별도 보호하며, 사망한
  본체는 자동 제외하고 남은 분신들로 즉시 재계산한다.
- 사람의 수정/검토 내용: 단독 상위 이상치가 다수 먹떼를 밀어내지 않도록 4마리부터
  상위 75%를 사용하고, 1·2·5·24마리와 본체 사망 조건을 정적 회귀 계약으로 추가했다.
  사용자 요청에 따라 Play와 전체 Test Runner는 실행하지 않고 Runtime·Editor Roslyn
  정적 컴파일만 수행한다.
- 외부 에셋/라이선스: 새 외부 에셋 없음. 체력 막대는 런타임 생성 도형만 사용한다.

### 2026-08-05 — 추락 체력 피해·먹자리 숫자 제거·피격 가시성 보강

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 감사, ripgrep,
  `docs/VFX/SKILL.md`, Unity Roslyn 정적 컴파일
- 목적: 화면 아래 추락이 개체 체력을 건너뛰고 즉사하던 규칙을 체력 시스템과
  일치시키고, 하단 먹자리 HUD에서 불필요한 숫자 문구를 제거한다.
- 주요 프롬프트/지시: “밑에 떨어져도 체력 한 칸이 깎이고 튀어오르게 하며,
  먹 게이지 텍스트는 뺀다.”
- 결과물: 방어막을 우선 소모하고 무방어 추락은 개체 체력 1칸을 감소시킨다.
  생존한 먹방울은 카메라 하단 안전선으로 복귀해 다시 상승하고, 마지막 체력에서만
  기존 사망 연출로 들어간다. 먹자리 숫자·초과 문구는 모두 제거하고 붓획 잔량,
  최대치에 따른 트랙 폭, 붓 여유 표식은 그대로 유지했다. 머리 위 체력은 독립된
  칸으로 분리하고, 기존 단일 캐시 실루엣을 앞면 종이빛 플래시와 붉은 먹 번짐으로
  재사용하며 공용 풀의 짧은 링·충돌음·약한 진동을 실제 체력 감소에만 추가했다.
- 사람의 수정/검토 내용: 장애물 전용 마지막 먹숨이 추락에 발동하지 않도록 경계를
  유지하고, 방어막 우선순위·연속 추락·치명 추락 계약을 정적 회귀 테스트로 추가했다.
  체력 칸 분리와 피격 전·후 레이어, 루트 Transform·Collider 불변 계약도 추가했다.
  사용자 요청에 따라 Play와 전체 Test Runner는 실행하지 않고 Runtime·Editor Roslyn
  정적 컴파일만 수행한다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-05 — 누적 거리형 먹빛 여정 전환

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 저장·밸런스·UI 감사,
  ripgrep, Unity 6000.3.10f1 Roslyn 정적 컴파일
- 목적: 판당 조건 보상과 최고 기록 이정표를 없애고, 모든 플레이 결과가 조금씩
  영구 성장에 쌓이는 배틀패스형 누적 거리 보상으로 단순화한다.
- 주요 프롬프트/지시: “한 판의 m를 총 m에 반영하고 일정 누적 m마다 먹빛 하나를
  얻도록 39개 성장 노드 총비용에 맞춰 비례 밸런스를 계산해 바로 적용한다.”
- 결과물: 정상 결과 고도를 누적하는 39단계 여정을 추가했다. 1~5단계는 20m,
  6~13은 50m, 14~26은 100m, 27~39는 150m 간격이며 마지막 문턱은 3750m다.
  한 판에 여러 단계 통과 시 일괄 지급하고, 디버그 판과 중복 run ID는 반영하지 않는다.
  누적 거리·수령 단계·먹빛·run ID를 동일 저장 트랜잭션에 묶고 balanceVersion 7로
  이관했다. 구 저장은 소유 노드 수와 보유 먹빛 합만큼 이미 수령한 단계로 안전하게
  환산한다. 결과 두루마리와 성장 화면에는 다음 문턱까지의 누적 거리와 진행 막대를
  추가했다.
- 사람의 수정/검토 내용: 총 39개 노드와 지급 상한을 일치시키고, 분신별 높이·이동
  경로가 아닌 결과창의 한 판 고도만 한 번 합산해 파밍·중복 계산을 막았다. 작은 화면
...
### 2026-08-05 — 첫 플레이 인터랙티브 5단계 튜토리얼

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 코드·기획 감사, Apple 공식
  게임 온보딩·Human Interface Guidelines, Google Play `Cat Jump` 스토어 설명,
  Android 공식 사용자 경험 품질 가이드, ripgrep, Unity Roslyn 정적 컴파일
- 목적: 처음 설치한 사용자가 별도 조작 설명을 찾지 않아도 실제 첫 판에서 먹 그리기,
  발판, 장애물, 날씨와 맵 변화를 짧게 익히게 한다.
- 주요 프롬프트/지시: “처음 시작하면 Cat Jump처럼 바로 시작할 수 있는 간단한
  튜토리얼을 만들고, 먹 그리는 법과 장애물·발판·날씨·맵을 소개한다.”
- 조사·판단: 긴 사전 설명 대신 한 번에 한 행동만 요구하고 즉시 연습시키며, 선택적
  건너뛰기와 나중에 다시 보기를 제공하는 원칙을 적용했다. 실제 등장 시점이 늦은
  장애물·날씨·맵은 첫 30m 안전 구간에서 정보 카드로만 소개해 인위적인 위험 생성은
  피했다.
- 결과물: `GameplayTutorialCatalog`의 공용 5장 데이터, 첫 `Playing` 진입에서 유효
  먹선과 그린 발판 상단 착지를 실제 이벤트로 확인하는 `FirstRunTutorialController`,
  버전별 완료 저장, 2회 확인 건너뛰기, 옵션의 5장 다시 보기와 Android/iPhone
  Safe Area·패널 경계 회귀 계약을 추가했다.
- 사람의 수정/검토 내용: 성장 저장 복구 게이트를 먼저 유지하고, 튜토리얼 카드 전체가
  월드 드로잉을 차단하도록 했다. 중도 사망은 완료로 저장하지 않으며 착지가 늦어도
  6초 뒤 안내가 막히지 않게 했다. 사용자가 요청한 단계별 정책에 따라 Play와 전체
  Test Runner는 실행하지 않고 정적 컴파일만 수행한다.
- 외부 에셋/라이선스: 새 외부 에셋 없음. 기존 프로젝트 성장 아이콘을 재사용했다.

### 2026-08-05 — 화면 먹벽 무한 상승 차단

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 물리·밸런스·테스트 감사,
  Unity 6 공식 `Camera.ViewportToWorldPoint`·`LineRenderer` 문서, ripgrep,
  Unity Roslyn 정적 컴파일
- 목적: 모바일에서 화면 가장자리에 급경사 먹선을 세우고 경계벽 반동을 반복해
  무한히 상승할 수 있는 경로를 규칙이 보이는 벽 장애물로 차단한다.
- 주요 프롬프트/지시: “벽쪽에 선을 그리고 무한벽타기가 가능하므로 벽장애물을
  추가하되, 단계별 기획·조사 뒤 실제 게임에 적용한다.”
- 조사·판단: 급경사 발판 접착 자체보다 `ScreenSideWall`이 양수 y속도를 보존한 채
  x만 반사하는 조합이 상승 래칫의 직접 원인이었다. 양쪽 피해벽이나 전역 경사 제한은
  이후 기본 체력 1칸과 기존 대각선 접착·`벽의 먹발` 비기를 훼손하므로 사용하지 않았다.
- 결과물: 일반 경계벽의 최초 충돌과 접촉 유지는 상승 y만 0으로 만들고 하강 y는
  보존한다. 현재 카메라
  줌·세로 비율을 기준으로 좌우 0.95m를 먹선 금지 띠로 계산해, 캐릭터 외곽 필터와
  같은 무할당 패스에서 가장 긴 내부 획만 발판·먹자리 비용으로 확정한다. 기존 보이지
  않던 경계는 충돌 면을 중심으로 한 옅은 붉은 갈필 띠로 표시하고, 첫 플레이 장애물
  안내도 이 규칙을 설명한다.
- 사람의 수정/검토 내용: 9:16·19.5:9와 카메라 이동·점프 줌에서 범위가 따라가는지,
  플레이어가 없어도 가장자리 획이 제거되는지, 캐릭터 여백과 먹벽이 동시에 걸릴 때
  가장 긴 구간을 고르는지, 상승 반동만 제거하고 하강과 비기를 유지하는지 회귀 계약을
  추가했다. 사용자 지시에 따라 Play와 전체 Unity Test Runner는 실행하지 않았다.
- 외부 에셋/라이선스: 새 외부 에셋 없음. 기존 공용 갈필 재질과 팔레트만 재사용했다.

### 2026-08-05 — 기본 한 획·최대 2.5획 먹자리 재조정

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 먹 경제·저장 호환 감사,
  Unity 직교 카메라 공식 문서, GDC 게임 시스템·경제 밸런스 자료, ripgrep,
  Unity Roslyn 정적 컴파일
- 목적: 실기기에서 첫 판부터 약 8~9획을 남기던 24m 먹자리를 약 한 획으로 낮추고,
  영구 성장 선택이 눈에 보이는 2.5배 진행이 되도록 용량과 HUD를 함께 재조정한다.
- 주요 프롬프트/지시: “처음 먹 용량은 거의 한 줄, 성장 재화로 최대 약 2.5줄까지
  비례 성장하게 하고 실제 기기 기준으로 단계별 기획·조사 뒤 적용한다.”
- 조사·판단: 기존 설계 문서의 실전 획 2.5~3m를 기준 단위로 삼아 기본을 3.2m로
  정했다. 용량 열매 다섯 개는 각각 기본값의 30%를 더해 3.2m에서 정확히 8.0m까지
  선형 증가한다. 총량 비기와 절약 비기의 실효 길이도 약 8% 안에 두어 한 선택만
  정답이 되지 않게 했다.
- 결과물: `StrokeCapture` 튜닝 버전을 2로 올려 구 12·18·24m 씬 값을 3.2m로
  이관하고, `I00/I-A1/I-A2/I-A3/I-KA`를 각각 +30%로 바꿨다. 붓 여유는 성장된
  용량이 아니라 기본값의 25%인 0.8m만 더하도록 문서 계약과 구현을 일치시켰다.
  하단 먹 게이지는 기존 1.5배 폭 제한을 제거해 1.0→2.5배 성장을 실제 트랙 폭으로
  구분하며, 새 규칙을 첫 플레이 안내와 README·설계 문서에 동기화했다.
- 사람의 수정/검토 내용: 기존 stable node ID·부모·저장 구조는 유지해 이미 산 열매가
  사라지지 않게 했다. 기본·각 성장 단계·최종 8.0m, 붓 여유 고정 증가, 24m 구 씬
  이관과 HUD 2.5배 표현을 회귀 계약으로 추가했다. 사용자 지시에 따라 Play와 전체
  Unity Test Runner는 실행하지 않고 Runtime·Editor 정적 컴파일만 수행한다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-05 — 먹선 4.5초 자연 소멸 복원

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 생명주기·밸런스·회귀 감사,
  Unity 6 공식 `Time.deltaTime`·`Object.Destroy`·`LineRenderer` 문서, 프로젝트
  `Docs/VFX/SKILL.md`, ripgrep, Git 과거 구현 비교
- 목적: 총 먹자리 방식으로 바뀌며 영구화된 플레이어 먹선 때문에 화면과 충돌체가
  오래 남는 문제를 해결하고, 원래 기획처럼 시간이 지나면 오래된 획이 마르게 한다.
- 주요 프롬프트/지시: “먹이 처음 의도한 대로 시간 지나면 사라지게 하고, 단계별
  기획·딥리서칭 뒤 다음 작업을 바로 진행한다.”
- 조사·판단: 과거 총 수명 4.5초의 체감은 유지하되 현재 FIFO와 별도 수명 코드를
  동시에 돌리면 잘린 콜라이더가 복원될 수 있어 단일 소멸 진행도로 합쳤다. 기본은
  3.4초 선명 유지 뒤 1.1초 마름이며, 일시정지는 scaled delta time으로 수명도
  멈춘다. 황금 붓은 총량 제한만 유예하고 자연 소멸은 유지하며 먹비는 용량만 줄여
  이중 페널티를 만들지 않는다.
- 결과물: 일반 먹선은 자연 만료 순간 남은 먹자리 비용을 한 번만 반환하고, 기존
  FIFO 알파 스윕·`EdgeCollider2D` 절단 경로로 끝까지 사라진다. 시간 만료와 부분
  FIFO가 겹쳐도 목표 진행도는 뒤로 가지 않으며, 먹 여운 성장으로 기본 4.5초를
  최대 5.25초까지 늘린다. 첫 플레이 안내 버전을 3으로 올리고 현재 설계 문서를
  같은 규칙으로 갱신했다.
- 사람의 수정/검토 내용: 자연 소멸·부분 FIFO 결합, ledger 단일 반환, 씬 빌더의
  3.4초 유지값과 4.5초 기본 계약을 회귀 테스트로 추가했다. 사용자 지시에 따라
  Play와 전체 Unity Test Runner는 실행하지 않고 정적 컴파일만 수행한다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-05 — 기본 체력 1칸·영구 성장 최대 5칸

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 체력·저장 호환·UI 감사,
  ripgrep, Unity Roslyn 정적 컴파일
- 목적: 실기기에서 기본 3칸 체력이 영구 성장의 체감과 긴장감을 약하게 만들던 규칙을
  기본 1칸에서 투자에 따라 최대 5칸까지 자라는 명확한 진행으로 바꾼다.
- 주요 프롬프트/지시: “처음 체력은 1단계로 고정하고 성장 재화를 통해 최대 5단계까지
  성장 가능하게 하며, 단계별로 다음 작업을 바로 진행한다.”
- 조사·판단: 씬에 남은 과거 `maxHealth: 3` 값이 상수 변경을 덮지 않도록 기본 체력을
  런타임 코드 규칙으로 고정했다. 새 저장 필드나 노드를 만들지 않고 기존
  `S00/S-A1/S-A2/S-A3`의 stable ID·부모 관계를 유지한 채 각 효과를 `Vitality +1`로
  교체해 기존 사용자 저장도 같은 소유 상태로 새 밸런스를 적용받게 했다.
- 결과물: 모든 원본·분신은 기본 최대 체력 1로 시작하고 먹피 경로를 열 때마다
  2→3→4→5칸으로 증가한다. 스냅샷은 `Vitality` 합계를 0~4로 제한하며, 분신은 생성
  시 그 판의 최대 체력을 가득 채운다. 기존 머리 위 칸형 표시와 피격 단계별 확대는
  1~5칸을 그대로 지원하고 피격 유예는 별도 생존 경로에 남겼다. 첫 플레이 안내도
  버전 4로 올려 기본 1칸·먹피 성장 최대 5칸을 직접 알린다.
- 사람의 수정/검토 내용: 기본 1칸 피격·추락, 방어막 우선 소모, 최대 5칸의 네 번
  비치명 피해와 다섯 번째 사망, 1·3·5칸 Billboard, 4단계 확대, 카탈로그·스냅샷
  합산 규칙을 회귀 계약으로 갱신했다. 사용자 지시에 따라 Play와 전체 Unity Test
  Runner는 실행하지 않고 Runtime·Editor 정적 컴파일만 수행한다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-05 — 옵션 화면 정보 위계·모바일 가독성 재정리

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 UI·테스트 감사,
  `emil-design-eng` 인터페이스 폴리시 기준, ripgrep, Unity Roslyn 정적 컴파일
- 목적: 모든 옵션 항목이 동일한 검은 붓획 버튼으로 보여 설정값·보조 정보·주 행동의
  구분이 사라지고, 작은 기기에서 음량 조절 터치 영역이 좁아지던 문제를 해결한다.
- 주요 프롬프트/지시: “옵션을 깔끔하게 다시 만들고, 모바일에서도 삐져나오거나
  보이지 않는 항목 없이 정리한다.”
- 결과물: 설정 제목과 버전을 중앙 상단에 분리하고, UID를 한 줄 카드로 단순화했다.
  BGM/SFX는 120px 터치 높이를 가진 가로 조절행 두 줄로 쌓고, 언어·고객센터·계정
  연동·튜토리얼은 밝은 한지 2×2 카드로 정리했다. 검은 붓획은 `닫기`와 튜토리얼의
  `다음`처럼 흐름을 확정하는 행동에만 남겼으며 보조 문구의 Bold·외곽선을 제거했다.
  옵션 논리 폭은 820px로 줄여 좁은 iPhone Safe Area에서도 글자 축소 없이 표시한다.
- 사람의 수정/검토 내용: 기존 오브젝트 이름·공개 테스트 API·설정 저장 동작은 유지하고,
  카드/토글과 주 행동의 시각 역할, 120px 터치 영역, 슬라이더·토글 비겹침, 보조 글자
  Normal과 좁은 iPhone 원본 배율을 회귀 계약으로 추가했다. 사용자 지시에 따라 Play와
  전체 Unity Test Runner는 실행하지 않고 Runtime·Editor 정적 컴파일만 수행한다.
- 외부 에셋/라이선스: 새 외부 에셋 없음. 기존 한지 팔레트와 공용 붓획만 재사용했다.

### 2026-08-05 — 정지형 첫 튜토리얼 팝업·공통 모달 dim

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 튜토리얼 흐름·팝업 dim·모바일
  레이아웃 감사, `emil-design-eng` 인터페이스 폴리시 기준, ripgrep, Unity Roslyn 정적 컴파일
- 목적: 첫 튜토리얼의 설명 영역이 작고 플레이가 뒤에서 계속 진행되던 문제와, 화면별로
  팝업 배경 농도·입력 차단이 달랐던 문제를 함께 해결한다.
- 주요 프롬프트/지시: “튜토리얼 설명이 꽉 차는 팝업을 띄우고, 읽는 동안 게임은 진행하지
  않으며, 모든 팝업 뒤에는 검은 반투명 dim을 올린다.”
- 결과물: 첫 플레이 안내를 Safe Area 중앙 820×1180 한지 카드와 680×280 설명 영역으로
  확장하고 5장을 이전·다음으로 직접 넘기게 했다. `FirstRunTutorial` 전용 정지 소유권으로
  마지막 `시작하기` 전까지 물리·스폰·날씨·기록 시간과 오디오를 멈춘다. 옵션·일시정지·
  게임오버·성장 상세·저장 복구·튜토리얼은 공통 68% 먹색 dim과 전체 Raycast를 사용한다.
- 사람의 수정/검토 내용: 기존 튜토리얼 오브젝트 경로와 완료 버전 저장을 유지하고, 긴 설명의
  고정 글자 크기·패널 내 배치·Android/iPhone Safe Area 양축 맞춤·정지 소유권·dim 입력 차단을
  회귀 계약으로 추가했다. 사용자 지시에 따라 Play와 전체 Unity Test Runner는 실행하지 않고
  Runtime·Editor 정적 컴파일만 수행한다.
- 외부 에셋/라이선스: 새 외부 에셋 없음. 프로젝트 한지·먹색·기존 성장 아이콘만 재사용했다.

### 2026-08-06 — 총 먹자리 게이지의 소멸 연동 회복 표시

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 HUD·먹선 생명주기 감사,
  ripgrep, Unity Roslyn 정적 컴파일
- 목적: 하단 붓획 게이지가 시작 값과 드로잉 소모만 표시하고, 오래된 먹선이
  마르는 동안 다시 차오르는 변화를 제대로 보여 주지 못하던 문제를 해결한다.
- 주요 프롬프트/지시: “하단 게이지는 총 먹게이지 용량이며, 먹게이지가 회복되면
  UI도 함께 회복되어야 한다.”
- 결과물: 게임 판정용 즉시 반환 장부와 HUD용 실제 가시 먹선 길이를 분리했다.
  게이지는 현재 획과 보이는 먹선을 합산하고, 자연 소멸로 화면의 총 먹선 길이가
  줄어드는 1.1초 진행도만큼 매 프레임 다시 채워진다.
- 사람의 수정/검토 내용: 예산 판정은 기존 `ActiveInkCost`를 유지해 게임 규칙을
  바꾸지 않고, HUD만 `ActiveVisibleInkCost`를 사용하도록 경계를 분리했다. 반쯤
  마른 시점과 완전 소멸 시점의 게이지 증가를 회귀 테스트로 고정했다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-06 — 성장 DEBUG 제거·공통 팝업 DIM 강화

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 UI 감사,
  `emil-design-eng` 인터페이스 폴리시 기준, ripgrep, Unity Roslyn 정적 컴파일
- 목적: 영구 성장 전용 화면에 남아 있던 개발용 재화·초기화 메뉴를 사용자 화면에서
  제거하고, 팝업 뒤 내용이 지나치게 선명해 상세 정보의 위계가 약하던 문제를 해결한다.
- 주요 프롬프트/지시: “성장창에 디버그를 없애고 팝업 UI 뒤 DIM 패널을 더 어둡게 한다.”
- 결과물: 성장 화면의 `DEBUG`, `먹빛 999`, `노드 초기화` 버튼과 관련 화면 이벤트를
  제거했다. 옵션·일시정지·게임오버·성장 상세·저장 복구·첫 튜토리얼은 공통 DIM을
  68%에서 78% 먹색으로 높여 뒤쪽 UI와 월드 입력을 같은 계층으로 눌러 준다.
- 사람의 수정/검토 내용: 게임플레이 회귀용 접이식 DEBUG 패널과 테스트용 성장 프로필
  헬퍼는 유지하고, 사용자에게 노출되는 성장 전용 DEBUG만 제거했다. 성장 화면에 개발
  컨트롤이 생성되지 않는지와 공통 DIM의 최소 농도를 회귀 계약으로 추가했다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-06 — 성장 상세 DIM 닫기·행동 버튼 하향·게임 DEBUG 비노출

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 팝업·DEBUG 생성 경로 감사,
  `emil-design-eng` 인터페이스 폴리시 기준, Unity Roslyn 정적 컴파일
- 목적: 성장 상세 카드의 중복 닫기 버튼과 정보판에 붙어 보이는 하단 상태 버튼을
  정리하고, 제출 플레이 화면에 남아 있던 개발용 DEBUG 창을 숨긴다.
- 주요 프롬프트/지시: “닫기는 DIM 패널을 누르면 뒤로가게 하고, 적용 중은 100 더
  내리며 디버그 창도 없앤다.”
- 결과물: 성장 상세의 보이는 `닫기` 붓획을 제거하고 카드 밖 전체 화면 DIM 버튼만
  닫기 행동을 소유하게 했다. 강화·적용 상태 버튼은 `y=-304`에서 `-404`로 정확히
  100px 내리고, 기존 46px 하단 여백을 보존하도록 한지 카드 높이를 820에서 1020px로
  늘렸다. 게임플레이 `ItemTestControls`는 Editor·Development Build에서도 항상
  비활성화되며 씬 빌더의 새 생성본도 처음부터 숨김 상태로 만든다.
- 사람의 수정/검토 내용: 카드 내부 탭은 닫히지 않고 바깥 DIM만 닫히는 Raycast 계층을
  유지했다. 내부 장애물·날씨·아이템 디버그 API는 회귀 테스트용으로 보존하되 사용자
  HUD에서는 호출할 수 없게 했다. DIM 닫기, CloseButton 부재, 100px 하향 위치,
  카드 경계와 개발 빌드 DEBUG 비노출을 회귀 계약으로 고정했다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-06 — 먹해태 좌·우 벽 경고와 수평 돌진 전환

> 같은 날 후속 기획에서 아래 수평 돌진은 폐기하고, 무작위 한쪽 벽의 위→아래
> 수직 하강으로 교체했다. 이 절은 변경 전 작업 이력이다.

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 해태 구조·모바일 VFX 감사,
  프로젝트 `docs/VFX/SKILL.md`, ripgrep, Unity Roslyn 정적 컴파일
- 목적: 항상 보이는 붉은 화면 벽을 간헐적인 수묵 장애물로 오해 없이 바꾸고,
  해태가 어느 쪽에서 어느 높이로 들어오는지 모바일에서도 먼저 읽히게 한다.
- 주요 프롬프트/지시: “벽 장애물은 우리 만든 해태로, 왼쪽이나 오른쪽에 느낌표가
  뜨고 빨간 투명한 것이 번쩍인 뒤 한 번씩 달려오게 한다.”
- 조사·판단: 보이지 않는 `ScreenSideWall` 물리 경계와 0.95m 먹선 금지 띠는 무한
  벽타기 차단의 실제 규칙이라 유지했다. 기존 320m 첫 보장·이동 장애물 슬롯 12%·
  해태 전용 풀 2개·용/낙묵석/강풍 동시성 제한도 현재 난도와 “한번씩” 빈도에 맞아
  새 스포너 없이 재사용했다. 전체 화면 플래시 대신 진입 측면의 단일 붉은 띠만
  알파 0.08~0.22로 사용해 모바일 오버드로와 광과민성 부담을 제한했다.
- 결과물: 좌·우 무작위 벽 안쪽에 붓먹 느낌표와 반투명 붉은 띠·고정 수평 경로가
  먼저 펄스한다. 예고 앞 48% 동안 해태는 완전히 숨고, 이후 낙관 교차 페이드로
  나타난 뒤 같은 높이로 반대편까지 달린다. 경고 중 판정은 꺼져 있고 경고 오브젝트를
  모두 숨긴 물리 틱에만 단발 판정을 켠다. 상시 붉은 먹벽 시각은 숨기되 물리 반동·
  드로잉 금지·`벽의 먹발`은 유지한다.
- 사람의 수정/검토 내용: 좌·우 방향과 느낌표 위치, 세로 화면 차선 고정, 경고 선행,
  판정 활성 순서, 풀 재사용 시 경고 자식 중복·잔상 없음, 보이지 않는 벽의 상승
  래칫 차단을 회귀 계약으로 갱신했다. 사용자 지시에 따라 Play와 전체 Unity Test
  Runner는 실행하지 않고 Runtime·Editor 정적 컴파일만 수행한다.
- 외부 에셋/라이선스: 새 외부 에셋 없음. 기존 먹해태 4프레임 시트와 공용 팔레트·
  갈필 재질·절차적 낙관을 재사용했다.

### 2026-08-06 — 먹 게이지·최대 체력 런 연결 복구

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 먹자리·체력·런타임 UI 감사,
  ripgrep, 로컬 PlayerPrefs 성장 저장 확인
- 목적: 성장 열매를 산 뒤에도 하단 먹 게이지가 기본 용량처럼 비어 보이거나,
  Play 중 재컴파일 뒤 최대 체력과 분신 체력이 새 판에 반영되지 않는 문제를 막는다.
- 주요 프롬프트/지시: “회복했을 때 먹게이지 색이 다시 채워지고, 찍은 최대 체력
  특성이 실제 본체와 분신에 반영되게 한다. 긴 테스트는 생략하고 바로 적용한다.”
- 결과물: 같은 `GameManager`·`RunGrowthController` 참조여도 이벤트를 다시 구독하고,
  진행 중 재컴파일에서는 영구 성장 스냅샷·카메라·살아 있는 먹선 장부를 복구한다.
  판 시작에는 모든 등록 생존자의 체력을 새 최대치로 맞춘다. 해금한 일반 열매와
  비기는 교체 없이 모두 함께 발동한다. 하단 붓획은 회복되는 비율만큼 두꺼운 쪽부터
  먹색이 즉시 드러나고 붓 아이콘의 가림 영역도 줄였다. 성장 화면 상단에는 생존·
  도약·먹 운용의 세 갈래씩 총 9개 현재 효과를 `기본값 → 현재값`과 계열색으로 표시한다.
- 사람의 수정/검토 내용: 로컬 저장에는 체력 열매 `S00`만 있어 현재 정상 최대 체력은
  2칸임을 확인했다. 체력 밸런스나 저장값을 임의로 바꾸지 않고 런 연결과 표시만
  강건화했다. 사용자 요청에 따라 Unity Play 및 Test Runner는 실행하지 않는다.
- 외부 에셋/라이선스: 새 외부 에셋 없음. 기존 먹 게이지·붓·체력칸 자산을 재사용했다.

### 2026-08-06 — 최초 접속 게임 직행 튜토리얼·버튼 통일

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 튜토리얼 상태 흐름·버튼 외형 감사,
  ripgrep, Unity Roslyn 정적 컴파일
- 목적: 최초 접속 사용자가 로비에서 시작 버튼을 다시 찾아야 하던 단계를 없애고,
  같은 튜토리얼 안에서 각진 한지 버튼과 붓획 버튼이 섞이던 시각 불일치를 해소한다.
- 주요 프롬프트/지시: “튜토리얼은 최초 접속 시 게임 시작 화면으로 바로 시작해 게임
  화면에서 팝업으로 나오게 하고, 버튼 UI를 통일한다.”
- 결과물: 완료 기록이 없는 설치 프로필은 로비 UI가 준비된 다음 프레임에 자동으로
  게임 상태로 진입하고, 물리·날씨·스폰이 정지된 게임 월드 위에서 5장 안내를 시작한다.
  완료 후 실행부터는 기존 로비를 유지한다. 첫 안내의 이전·다음·건너뛰기와 옵션 다시
  보기의 옵션으로·이전·다음은 모두 공용 수묵 붓획 버튼과 같은 120px 터치 높이를 쓴다.
- 사람의 수정/검토 내용: 성장 저장 복구가 필요한 상태에서는 자동 시작하지 않고,
  기존 사용자의 안내를 버전 증가로 강제 재노출하지 않는 설치 프로필 1회 규칙으로
  고정했다. 사용자 지시에 따라 Play와 전체 Unity Test Runner는 실행하지 않는다.
- 외부 에셋/라이선스: 새 외부 에셋 없음. 기존 공용 `action_button_brush`를 재사용했다.

### 2026-08-06 — 황금 붓 상태 표시 단순화

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 황금 붓 HUD·지속 VFX 경로 감사,
  ripgrep, Unity Roslyn 정적 컴파일
- 목적: 황금 붓 활성 중 하단 붓 아이콘이 다른 비율의 아이템 PNG로 커지며 금가루와
  후광이 겹쳐 보이던 시각 불일치를 제거한다.
- 주요 프롬프트/지시: “황금붓 이미지가 바뀌는 연출은 어색하니 게이지 색만 바꾼다.”
- 결과물: 무한 먹 8초 동안 기존 먹 게이지 실루엣의 채움만 금빛으로 표시한다. 붓 아이콘,
  아이콘 크기, 트랙과 붓 여유 표시는 평상시와 동일하다. 아이콘 교체·펄스·후광·별·
  게이지 금가루와 캐릭터 주변 24렌더러 지속 오라는 제거했다. 픽업 순간의 짧은 공용
  피드백과 무한 먹 기능·종료 후 FIFO 정산은 유지한다.
- 사람의 수정/검토 내용: 검정 원본 텍스처에 색을 곱하면 금색이 나오지 않으므로,
  원본 알파 마스크를 보존한 금빛 채움 텍스처를 최초 활성 시 한 번만 생성하고 HUD
  파괴 시 해제한다. 사용자 지시에 따라 Play와 전체 Unity Test Runner는 실행하지 않는다.
- 외부 에셋/라이선스: 새 외부 에셋 없음. 기존 먹 게이지 실루엣과 `InkPalette.Gold`를
  재사용했다.

### 2026-08-06 — 영구 성장 9개 단일 수치 경로 통일

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 카탈로그·레이아웃·먹분신 감사,
  ripgrep, Unity Roslyn 정적 컴파일
- 목적: 서로 다른 조건형 효과가 한 줄에 섞여 성장 결과를 예측하기 어렵던 먹나무를,
  한 잔가지가 하나의 능력치만 단계적으로 올리는 구조로 단순화한다.
- 주요 프롬프트/지시: “낮은 흔들림 노드를 없애고 한 줄에는 체력, 점프 높이,
  먹자리 총량, 회복, 획 소모 감소처럼 한 종류의 성장만 둔다. 분신 보호 능력은 모두
  빼고 먹분신 아이템을 먹을 때 생성되는 분신 +1로 바꾼다.”
- 결과물: 39개 stable node ID와 비용·부모 그래프는 저장 호환을 위해 유지하되,
  생존은 체력·피격 무적·먹분신 획득 생성 수, 도약은 준비시간·점프 힘·점프 높이,
  먹 운용은 총 먹자리·획 소모·게이지 회복의 9개 단일 수치 줄로 재해석했다. 완성값은
  체력 1→5, 피격 무적 +0.20초, 먹분신 아이템 1회 1→5마리, 준비 -6%, 점프 힘
  +5%, 높이 +6.25%, 먹자리 3.2→8.0m, 획 소모 -10%, 회복 +40%다.
- 사람의 수정/검토 내용: “분신 +1”은 게임 시작 분신이 아니라 먹분신 아이템 1회당
  추가 생성이라는 사용자 정정을 반영했다. 최대 24마리에서 남은 자리만 생성하고
  한 마리도 만들 수 없으면 픽업을 소비하지 않는다. 구 분신 보호·사망 회복·마지막 숨·
  피격 보존·벽 매달림·안전 발판·2단점프 효과는 런 스냅샷에서 비활성화했다. 사용자
  지시에 따라 Unity Play와 Test Runner는 실행하지 않고 정적 컴파일만 수행한다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-06 — 19.5:9 실기기 결과·붓 HUD·튜토리얼 보정

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 UI 감사, `emil-design-eng`
  인터페이스 폴리시 기준, ripgrep, Unity Roslyn 정적 컴파일
- 목적: 1179×2556 실기기에서 결과 두루마리의 행 정렬이 흔들리고, 하단 붓과 먹선이
  시뮬레이터보다 작게 느껴지며, 첫 튜토리얼의 장문·왼쪽 정렬·버튼이 한지 테두리를
  넘어가는 문제를 수정한다.
- 주요 프롬프트/지시: “종료창 글자 정렬, 인게임 붓 크기의 실기기 차이, 튜토리얼의
  불필요한 말과 왼쪽 정렬을 고치고 모든 글자를 UI 안에 넣는다.”
- 결과물: 결과 제목·이번 고도·최고 고도·먹빛을 공통 좌우선에 맞췄다. 용량에 따라
  두께까지 변하던 IMGUI 게이지를 분리해 트랙은 용량만, 붓획 높이와 붓 아이콘은
  기기 폭의 12%·14%로 표시한다. 첫 안내와 옵션 다시 보기의 본문을 가운데 정렬하고
  5장 문구를 3~4줄로 축약했으며 진행 수·그림·버튼을 카드의 밝은 안쪽으로 옮겼다.
- 사람의 수정/검토 내용: 전역 CanvasScaler와 Safe Area 계산은 정상임을 확인해 건드리지
  않고 문제가 난 세 화면의 고정 배치만 수정했다. 자동 글자 축소 없이 고정 글자 크기와
  충분한 영역을 유지하고 1179×2556·1080×2400 경계 계약을 갱신했다.
- 외부 에셋/라이선스: 새 외부 에셋 없음. 기존 한지 카드·붓획·먹 게이지 자산을 재사용했다.

### 2026-08-06 — 먹 게이지 전폭 농도 표시

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 HUD 감사, `emil-design-eng`
  인터페이스 폴리시 기준, ripgrep, Unity Roslyn 정적 컴파일
- 목적: 먹을 계속 소비하는 동안 잔량 폭이 짧아져 하단 게이지가 항상 0처럼 보이는
  문제를 줄이고, 같은 붓획 안에서 소모와 회복을 즉시 읽을 수 있게 한다.
- 주요 프롬프트/지시: “전체 게이지 색을 투명도로 채우고 빠지는 듯한 연출로 바꾼다.”
- 결과물: 트랙·용량 폭·붓 크기는 유지한 채 채움 이미지를 항상 전체 붓획에 그린다.
  남은 먹은 먹색 불투명도 0~100%로 표현하며, 자연 회복할수록 전체 붓획이 진해진다.
  황금 붓은 기존처럼 전폭 금색 100%를 유지한다.
- 사람의 수정/검토 내용: 매 프레임 텍스처를 만들거나 원본 픽셀을 바꾸지 않고
  IMGUI 색상 알파만 잠시 적용한 뒤 복원해 붓 아이콘·붓 여유 표시에는 영향을 주지 않았다.
- 외부 에셋/라이선스: 새 외부 에셋 없음. 기존 먹 게이지·붓 자산만 재사용했다.

### 2026-08-06 — 벽 트램펄린·위험 밀도·가로 분산 재조정

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 벽 반동·장애물 밀도·폐기 아이템
  참조 감사, ripgrep, Unity Roslyn 정적 컴파일
- 목적: 반복 플레이가 화면 벽으로 쏠리고 중앙 장애물 밀도가 높아지는 흐름을 완화하며,
  한 판 동안 최대 먹자리를 초과 확장하던 불필요한 `붓 여유` 아이템을 제거한다.
- 주요 프롬프트/지시: “벽에 닿으면 예상하지 못한 트램펄린처럼 튕기게 하고 몬스터를
  줄여 맵을 넓게 쓴다. 총 게이지를 초과해 늘리는 아이템은 삭제한다.”
- 결과물: 좌우 벽 첫 충돌은 화면 안쪽 3.2~4.2 속도와 2.4~3.2의 작은 무작위 상승으로
  반사하고, 0.45초 재발동 제한과 접촉 유지 보정으로 무한 벽타기를 막는다. 이동 장애물
  간격은 12~16m, 낙묵석은 저고도 7~10초·고고도 5.5~7.5초로 완화했다. 어린 용·
  먹해태 합산 확률은 후반 20%로 낮추고 시작 직후 강제 해태는 제거했다. 일반 장애물과
  아이템의 X 범위는 고정 좌표 대신 현재 카메라 가로 폭을 사용한다. `InkReserve=4`는
  과거 직렬화 호환용 폐기 번호로만 남기고 생성·효과·런 상태·HUD·디버그 경로를 제거했다.
- 사람의 수정/검토 내용: 벽 양옆 먹선 금지 폭 0.95와 첫 30m 안전 구간, 60m 첫 어린 용,
  320m 첫 먹해태 보장은 유지했다. 기존 Main 씬의 구 밀도 값은 런타임 버전 승격으로
  새 값에 맞추며 다음 씬 빌드부터 에디터 빌더도 같은 값을 기록한다. 사용자 지시에 따라
  Unity Play와 전체 Test Runner는 실행하지 않고 정적 컴파일만 수행한다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-06 — 먹해태 벽 하강 속도 완화

- 사용 도구: OpenAI Codex, ripgrep, Unity Roslyn 정적 컴파일
- 목적: 실기기에서 먹해태가 화면 위에서 아래까지 너무 빨리 통과해 경고를 보고도
  피할 시간이 부족한 문제를 완화한다.
- 주요 프롬프트/지시: “해태 내려오는 속도가 너무 빠르다.”
- 결과물: 실제 하강 시간을 고도에 따라 기존 0.65~0.75초에서 1.25~1.45초로 약 두 배
  늘렸다. 디버그·직접 생성 기본값도 1.35초로 통일했다.
- 사람의 수정/검토 내용: 1.1~1.2초 사전 경고, 좌우 무작위 벽 선택, 수직 고정 경로,
  한 번에 한 캐릭터만 피격하는 규칙은 유지했다. Unity Play와 Test Runner는 실행하지 않았다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-06 — 먹 게이지 동일 색상·농도 전환 보정

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 HUD 감사, `emil-design-eng`
  인터페이스 피드백 기준, ripgrep, Unity Roslyn 정적 컴파일
- 목적: 전폭 알파 방식이 들어가 있어도 불투명한 베이지 트랙과 즉시 변하는 잔량 때문에
  실기기에서는 검정 먹이 투명해지는 과정이 보이지 않던 문제를 수정한다.
- 주요 프롬프트/지시: “전체 용량을 꽉 채운 상태에서 시작하고, 사용할 때 색은 그대로
  둔 채 투명도만 점점 늘어나는 방식인지 확인하고 적용한다.”
- 결과물: 총 용량 실루엣은 같은 폭의 옅은 먹색으로만 남기고, 잔량 채움은 원본 먹색을
  유지한 채 0~100% 불투명도만 바꾼다. 한 획으로 잔량이 급감해도 0.42초 동안 농도가
  자연스럽게 옅어지며, 먹선 소멸로 돌아온 양은 0.22초 안에 다시 진해진다.
- 사람의 수정/검토 내용: 너비를 잘라 채우거나 색을 교체하는 연출은 추가하지 않았다.
  매 프레임 텍스처를 생성하지 않고 IMGUI tint와 unscaled 시간만 사용해 모바일 비용을
  억제했다. 사용자 지시에 따라 Unity Play와 전체 Test Runner는 실행하지 않는다.
- 외부 에셋/라이선스: 새 외부 에셋 없음. 기존 먹 게이지 fill·track을 재사용했다.

### 2026-08-06 — 먹떼 최종 패시브와 피격 안정 성장 재편

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 성장 감사, ripgrep,
  Unity Roslyn 정적 컴파일
- 목적: 먹분신 생성 수 증가를 여러 중간 노드에 반복 배치하지 않고 생존 갈래의
  마지막 목표로 만들며, 비어 난 세 노드에 즉시 체감되지만 생존을 무효화하지 않는
  안정 성장 수치를 배치한다.
- 주요 프롬프트/지시: “분신 업그레이드 노드를 최종 노드로 하고, 분신 아이템의
  추가 생성 최대치는 +1만 둔다. 그 앞 노드에는 다른 성장 효과를 넣는다.”
- 결과물: stable ID `S-C1~C3`는 `먹발 버팀 I~III`로 바꾸어 피격 뒤 수평 밀림을
  단계마다 6% 줄인다. 기본 수평 속도 보존율은 82%에서 완성 시 64%가 된다.
  마지막 `S-KC`만 `먹떼 결실`로 남겨 먹분신 아이템 1회의 생성 수를 기본 1마리에서
  최대 2마리로 올린다. 런 스냅샷과 아이템 생성 함수에도 보너스 0~1 하드 상한을 뒀다.
- 사람의 수정/검토 내용: 시작 분신 수와 전체 생존 상한 24마리, 첫 12m 먹분신 보장,
  출현 가중치는 유지했다. 기존 저장의 노드 ID는 바꾸지 않아 해금 상태를 보존한다.
  Unity Play와 전체 Test Runner는 실행하지 않고 Runtime·Editor 정적 컴파일만 확인한다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-06 — 생존·먹 운용 성장 가지 시각 분리

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 성장 그래프 감사, ripgrep,
  Unity Roslyn 정적 컴파일
- 목적: 생존의 먹발 버팀·먹떼 결실과 먹 운용의 넓은 벼루가 서로 다른 계보인데도
  같은 좌표에 겹쳐 하나의 가지처럼 보이던 성장 지도 문제를 수정한다.
- 주요 프롬프트/지시: “생존 먹떼 노드와 넓은 벼루는 연결되면 안 되고,
  생존·도약·먹 운용이 각각 3갈래로 분리되어야 한다.”
- 결과물: 논리 부모 관계는 그대로 독립 유지하고, 생존 우측 가지는 왼쪽 안쪽으로,
  먹 운용 좌측 가지는 오른쪽 안쪽으로 재배치했다. 같은 단계의 두 열매 중심은
  최소 280px 떨어지며 회귀 검사로 간격을 고정한다.
- 사람의 수정/검토 내용: stable ID, 해금 순서, 효과 수치와 저장 데이터는 바꾸지 않고
  지도 좌표와 시각 분리 계약만 수정했다. Unity Play와 전체 Test Runner는 실행하지 않는다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-06 — 성장 먹 용량 용어 통일

- 사용 도구: OpenAI Codex, ripgrep, Unity Roslyn 정적 컴파일
- 목적: 화면에 유지할 수 있는 먹선 총량을 `먹자리`라고 부르던 모호한 표현을 실제
  기능인 최대 자원 용량으로 명확히 바꾼다.
- 주요 프롬프트/지시: “성장의 먹자리는 먹의 최대 용량을 뜻하는 것 아닌가.”
- 결과물: 성장 화면과 노드 상세의 `총 먹자리`를 `최대 먹 용량`으로, 비용 감소 계열의
  `획 먹자리 소모`를 `획당 먹 소모`로 분리했다. 안내·기획·아키텍처·Inspector 용어도
  같은 기준으로 통일했다.
- 사람의 수정/검토 내용: 표시 문구만 바꾸고 3.2→8.0m 용량 수치, -10% 소모 감소,
  게이지 계산과 저장 ID는 변경하지 않았다. Unity Play와 전체 Test Runner는 실행하지 않는다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-06 — 숨 고르기 최종 방어막 패시브

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 피해·방어막 흐름 감사, ripgrep,
  Unity Roslyn 정적 컴파일
- 목적: 숨 고르기의 마지막 열매를 단순 무적시간 증가가 아니라 피격 뒤 다음 위험을
  한 번 넘길 수 있는 명확한 생존 패시브로 교체한다.
- 주요 프롬프트/지시: “숨 고르기 최종 패시브는 피격할 때마다 1회용 방어막 생성.”
- 결과물: stable ID `S-KB`를 `숨 고르기 결실`로 바꾸고, 장애물에 맞아 체력이 한 칸
  줄고 살아남은 해당 먹방울이에게 다음 피격 1회용 방어막을 생성한다. 기존 방어막이 막은
  피격은 새 방어막을 만들지 않으며 치명 피해 뒤에도 생성하지 않는다. 숨 고르기 수치
  상한은 +0.20초에서 +0.16초로 조정했다.
- 사람의 수정/검토 내용: 기존 `PlayerController.TryGrantShield()`의 비중첩 방어막과
  VFX 표시를 재사용해 본체와 각 분신이 방어막을 독립 소유한다. 화면 하단 추락은
  장애물 피격과 분리해 방어막 생성 조건에 포함하지 않는다. enum은 저장 호환을
  위해 새 값 39를 추가하고 기존 값은 재사용하지 않았다. 사용자 지시에 따라 Unity
  Play와 전체 Test Runner는 실행하지 않고 Runtime·Editor 정적 컴파일만 확인한다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-06 — 강한 측벽 트램펄린·실제 총 먹 잔량 HUD

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 벽·게이지 경로 감사,
  ripgrep, Unity Roslyn 정적 컴파일
- 목적: 실기기에서 측벽 반동이 느껴지지 않고, 먹 예산이 반환돼도 하단 게이지가
  빈 상태에 머무는 문제를 수정한다.
- 주요 프롬프트/지시: “벽에 부딪히면 과하게 튕겨 나오게 하고, 총 먹 게이지 UI에
  실제 소모·회복을 반영해 달라.”
- 결과물: 구 씬 값도 런타임에서 수평 7.5~9.0, 수직 4.8~7.0 반동으로 승격하고
  최초 충돌 시 0.18m 안쪽으로 분리했다. 게이지는 페이드 중인 옛 먹선이 아니라 실제
  ledger 점유량을 읽고, 빈 안내와 채움에 같은 실루엣을 사용해 색은 고정한 채 전체
  불투명도만 변하게 했다.
- 사람의 수정/검토 내용: 시각 먹선의 1.1초 소멸은 유지하되, 사용 가능한 먹은 예산
  반환 시점부터 0.32초 보간으로 HUD에 회복되게 분리했다. 사용자 지시에 따라 Unity
  Play와 전체 Test Runner는 실행하지 않고 Runtime·Editor 정적 컴파일만 확인한다.
- 외부 에셋/라이선스: 새 외부 에셋 없음. 기존 먹 게이지 fill 실루엣만 재사용했다.

### 2026-08-06 — 먹분신 체력 1 고정

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 분신 생성·체력·문구 감사,
  ripgrep, Unity Roslyn 정적 컴파일
- 목적: 영구 성장 체력과 누적 먹분신 수가 곱해져 플레이어 입력 없이도 먹떼가
  지나치게 오래 생존하는 문제를 줄이고, 본체 성장과 분신 수집의 역할을 분리한다.
- 주요 프롬프트/지시: “분신은 체력 1로 무조건 고정하고 관련 텍스트를 모두 수정한다.
  분신이 계속 생성되는 상태에서 체력 5까지 함께 받으면 게임이 저절로 올라간다.”
- 결과물: 본체만 먹피 영구 성장으로 최대 체력 1→5를 적용하고, 모든 런타임 분신은
  생성·회복·새 판 초기화 때 항상 1/1 체력으로 고정한다. 분신을 다시 복제하는 경우도
  같은 규칙을 적용하며 머리 위 체력 표시도 1칸만 만든다. 성장 요약·노드 상세·튜토리얼과
  README·기획·아키텍처 문서를 본체 성장/분신 1칸 규칙으로 통일했다.
- 사람의 수정/검토 내용: 먹 방어막 아이템은 기존처럼 분신도 별도로 한 번 막을 수 있지만,
  방어막이 없는 실제 피해는 분신을 즉시 사망시킨다. `숨 고르기 결실`의 피격 생존 방어막은
  실제 피해 뒤 살아남을 수 있는 본체에만 발동한다. 사용자 지시에 따라 Unity Play와 전체
  Test Runner는 실행하지 않고 Runtime·Editor 정적 컴파일만 확인한다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-06 — 성장 상단 정보 먹 패널·로비 버튼 재배치

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 성장 UI·모바일 Safe Area 감사,
  사용자 실기기 캡처 비교, ripgrep, Unity Roslyn 정적 컴파일
- 목적: 먹나무와 겹쳐 흐리게 보이던 성장 요약·먹빛 보유량의 대비를 높이고,
  화면 위에 몰린 로비 버튼을 왼쪽 아래로 옮겨 정보 계층을 분리한다.
- 주요 프롬프트/지시: “성장 텍스트와 먹물 보유량 뒤에 성장 상세창에서 사용한
  진회색 UI 패널을 넣고, 로비 버튼은 왼쪽 아래로 옮긴다.”
- 결과물: 상세창 `ActionInfoPanel`과 같은 `pg_hanji_card`·먹색 94% 조합을
  `HeaderInfoPanel`로 재사용했다. 먹빛·누적 거리·생존·도약·먹 운용 요약을 한 판에
  담고 기본 글자를 밝은 한지색으로 바꿨으며, 계보 강조색도 어두운 배경용으로 밝게
  조정했다. 로비 버튼은 Safe Area 기준 왼쪽 아래로 이동했다.
- 사람의 수정/검토 내용: 기존 재화 HUD와 요약의 계층·정중앙 재화 배치는 유지해
  데이터 연결을 바꾸지 않았다. 패널은 터치를 막지 않으며 모바일 최소 버튼 크기와
  노치·시스템 바 여백을 유지한다. Unity Play와 전체 Test Runner는 실행하지 않고
  Runtime·Editor 정적 컴파일만 확인한다.
- 외부 에셋/라이선스: 새 외부 에셋 없음. 기존 `pg_hanji_card`만 재사용했다.

### 2026-08-06 — 먹 게이지 전체 투명도에서 잔량 폭 표시로 수정

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 먹 게이지 렌더·테스트 감사,
  ripgrep, Unity Roslyn 정적 컴파일
- 목적: 일부 먹만 사용해도 게이지 전체가 한꺼번에 반투명해져 실제 남은 양을
  공간적으로 읽을 수 없던 문제를 수정한다.
- 주요 프롬프트/지시: “게이지는 소모된 양만큼 없어져야 하는데 전체에 불투명도가
  적용된다.”
- 결과물: 옅은 전체 붓획은 최대 용량 가이드로 남기고, 실제 채움은 항상 불투명한 채
  잔량 비율만큼 폭을 줄인다. 붓이 있는 오른쪽 끝을 고정해 소모 시 왼쪽부터 사라지고,
  회복 시 다시 왼쪽으로 차오른다. 텍스처 UV도 같은 비율로 잘라 이미지가 눌리지 않으며,
  이미지가 없는 단색 폴백도 같은 방향으로 줄어든다. 황금 붓은 전체 금색 채움을 유지한다.
- 사람의 수정/검토 내용: 실제 ledger 기반 잔량과 Safe Area·실기기 크기 계산은 유지하고
  렌더 표현만 공간 채움으로 교체했다. 잘못된 전체 알파 테스트를 0·25·50·100% 채움 Rect와
  UV 계약으로 바꿨다. Unity Play와 전체 Test Runner는 실행하지 않고 Runtime·Editor
  정적 컴파일만 확인한다.
- 외부 에셋/라이선스: 새 외부 에셋 없음. 기존 게이지 실루엣과 붓 아이콘을 재사용했다.

### 2026-08-06 — 먹 게이지 회복 채움 완화

- 사용 도구: OpenAI Codex, ripgrep, Unity Roslyn 정적 컴파일
- 목적: 반환된 먹이 게이지에 너무 빠르게 나타나 순간적으로 튀어 보이는 문제를 줄인다.
- 주요 프롬프트/지시: “먹 게이지가 좀 더 서서히 차오르도록 한다.”
- 결과물: 소모 반응은 0.24초로 유지하고, 회복 채움만 0.32초에서 0.75초로 늘려
  붓이 있는 오른쪽에서 왼쪽으로 부드럽게 차오르게 했다.
- 사람의 수정/검토 내용: 실제 먹 ledger 반환 시점과 최대 용량·채움 폭 계산은 바꾸지
  않고 시각 보간 시간만 조정했다. Unity Play와 전체 Test Runner는 실행하지 않고
  Runtime·Editor 정적 컴파일만 확인한다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-06 — 영구 성장 README 시각화·먹피 IV 분신 체력 결실

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 성장 카탈로그·소비처 감사,
  ripgrep, GitHub Mermaid, Unity Roslyn 정적 컴파일
- 목적: GitHub 첫 화면에서 39개 영구 성장의 세 계보와 완성 효과를 빠르게 이해하게 하고,
  본체 체력 경로의 마지막 열매에 분신 체력 성장 보상을 더한다.
- 주요 프롬프트/지시: “성장과 관련해서 간단하게 시각화해서 README에 올리고,
  본체 체력 마지막 노드에는 분신 체력 +1, 기존 분신 노드는 분신 생성 최대 +1로 둔다.”
- 결과물: README 영구 성장 절에 `생존 / 도약 / 먹 운용` 3×13 노드의 완성 효과를
  요약한 Mermaid 도식을 추가했다. `S-KA 먹피 IV`는 기존 본체 최대 체력 +1에
  분신 최대 체력 +1을 묶어 분신을 1/1→2/2로 만들고, `S-KC 먹떼 결실`은 기존처럼
  먹분신 아이템 1회 생성 수만 1→2로 늘린다.
- 사람의 수정/검토 내용: 39개 stable ID와 저장 schema는 바꾸지 않고 S-KA 소유 여부에서
  분신 체력 보너스를 파생했다. 성장 요약·상세·튜토리얼·기획·아키텍처·밸런스 문구와
  회귀 계약을 같은 규칙으로 맞췄다. 외부 이미지 없이 GitHub 기본 Mermaid만 사용했다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-06 — 계보별 한 갈래 빌드·결실/진행 HUD 정리

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 밸런스·저장 호환·모바일 UI 감사,
  ripgrep, Unity Bee 응답 파일 기반 Runtime·Editor Roslyn 정적 컴파일
- 목적: 모든 성장이 한꺼번에 적용되던 구조를 유저가 빌드를 고르는 구조로 바꾸고,
  상단에 가려 탭할 수 없던 결실과 짧은 누적 거리 표시를 실기기에서 읽기 쉽게 고친다.
- 주요 프롬프트/지시: “생존·도약·먹 운용의 세 줄기 중 각각 하나만 선택하게 하고
  결실 패시브를 강화한다. 결실은 너무 높지 않게 내리고, 상단 패널에 노드 초기화를
  넣고, 먹빛 획득 게이지를 길게 만들어 0/100 진행률을 표시한다.”
- 결과물: 기존 39개 stable ID와 소유 기록은 보존하면서 각 계보의 공용 뿌리와 선택한
  A/B/C 한 갈래만 런 스냅샷에 적용하도록 바꿨다. 소유한 다른 갈래는 무료 교체하며,
  구매 노드 전체 초기화는 소비 먹빛만 환급하고 누적 거리·보상·정산 기록을 보존한다.
  결실은 마지막 일반 열매보다 310px 위, 216×240 터치 영역으로 내려 헤더 아래에서
  누를 수 있게 했고 일반 열매보다 조금 큰 면과 금빛 후광만 유지했다. 누적 거리 막대는
  520×32px로 늘려 내부에 현재 문턱 진행률을 `n / 100%`로 표시하고, 같은 정보판에
  2회 확인 `노드 초기화`를 추가했다. 9개 결실의 수치·설명·게임 소비처와 README,
  기획 요약, 아키텍처 문서를 같은 규칙으로 갱신했다.
- 사람의 수정/검토 내용: 구 저장은 기존 선택 ID를 유지하고 값이 없으면 소유가 가장
  많은 갈래를 결정적으로 선택해 노드를 삭제하지 않는다. 저장 실패 시 프로필 롤백과
  복구 안내가 즉시 갱신되도록 했으며, 사용자 지시에 따라 Unity Play와 전체 Test Runner는
  실행하지 않고 Runtime·Editor 정적 컴파일만 확인한다.
- 외부 에셋/라이선스: 새 외부 에셋 없음. 기존 먹나무·한지·열매 UI 리소스만 재사용했다.

### 2026-08-06 — 비선택 성장 갈래 가시덩굴 시각화

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 성장 상태·모바일 UI 감사,
  ripgrep, Unity Bee 응답 파일 기반 Runtime·Editor Roslyn 정적 컴파일
- 목적: 각 계보에서 선택한 한 줄기와 적용되지 않는 두 줄기를 나무의 형태를 유지한 채
  즉시 구분할 수 있게 한다.
- 주요 프롬프트/지시: “노드를 선택해 강화하면 나머지 두 줄기는 살짝 비활성화되고,
  나무 줄기에 가시덩굴이 휘어 감긴 듯 보이게 한다.”
- 결과물: 선택 줄기는 기존 농도를 유지하고 비선택 두 줄기는 가지·열매 농도를 낮췄다.
  생존·도약·먹 운용의 A/B/C마다 비차단 CanvasGroup을 두고, 기존 먹붓 스프라이트로
  굽은 덩굴과 가시를 절차 배치했다. 선택 전에는 모두 숨고 갈래를 바꾸면 즉시 다른
  두 줄기로 이동한다.
- 사람의 수정/검토 내용: 결실 보유 여부가 아니라 저장된 현재 갈래 ID를 단일 기준으로
  삼았다. 가시 이미지는 모두 레이캐스트를 끄고 노드 조회·무료 교체·지도 드래그를
  보존했으며, 새 이미지 에셋과 저장 schema 변경 없이 구현했다. 사용자 지시에 따라
  Unity Play와 전체 Test Runner는 실행하지 않고 Runtime·Editor 정적 컴파일만 확인한다.
- 외부 에셋/라이선스: 새 외부 에셋 없음. 기존 절차형 먹붓 스프라이트를 재사용했다.

### 2026-08-06 — 성장 선택 UI 실기기 경계·회귀 검증

- 사용 도구: OpenAI Codex, Unity Test Runner, Computer Use 기반 Unity UI 스모크 테스트,
  ripgrep, Git 상태·로그 검사
- 목적: 계보별 한 갈래 선택과 가시덩굴 시각화 이후 성장 화면이 모바일 경계 안에
  들어오는지 확인하고, 최신 노드 효과와 어긋난 자동 검증 계약을 바로잡는다.
- 주요 프롬프트/지시: “완료 후 main에 반영하고 Unity를 직접 조작해 UI와 버그를
  테스트하며 iPhone 15 실기기까지 확인한다.”
- 결과물: 첫 화면의 생존 씨앗이 20:9 왼쪽 마스크에 약 5px 걸리던 위치를 안쪽으로
  옮겼다. 활성 노드의 먹색 받침은 완전 불투명하게 유지하고, 낙묵석 발판 제거는
  Play/EditMode 양쪽에서 안전한 파괴 방식을 사용한다. 먹 운용 뿌리가 최대 용량이 아닌
  먹 소모 감소 씨앗으로 바뀐 최신 기획과 기본 체력 1·숨 고르기 결실 규칙에 맞춰 성장
  연출·효과·카탈로그 회귀 테스트 기대값을 갱신했다.
- 사람의 수정/검토 내용: Unity Game View에서 로비→성장→노드 상세/DIM→옵션→게임→
  일시정지/재개 흐름을 직접 확인했다. 최종 수정 뒤에는 성장 트리 전체 노출, 노드 상세의
  중앙 배치, DIM 바깥 터치 닫기, 옵션과 5단계 튜토리얼의 540×960 세로 화면 배치를 다시
  확인했으며 Unity 콘솔 오류는 0건이었다. 전체 EditMode 회귀 테스트 472개도 모두 통과했다.
  등록된 iPhone 15 Pro는 `devicectl`에서 unavailable로 표시되어 실제 설치·실행은 보류하고,
  iPhone 15 계열 Safe Area·종횡비 자동 검증과 Unity Game View 눈검사를 대신 수행했다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-07 — 기본 최대 먹 용량 50% 상향·고정 게이지 트랙

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 먹 용량 경로 감사, ripgrep,
  Unity 회귀 계약 및 Roslyn 정적 컴파일
- 목적: 첫 판의 최대 먹 용량이 지나치게 짧은 문제를 완화하고, 먹을 사용할 때 하단
  게이지 외형 자체가 줄어드는 것으로 오해되지 않게 최대 용량 실루엣을 유지한다.
- 주요 프롬프트/지시: “처음 최대 먹 게이지 총량을 지금보다 50% 올리고 이후 성장
  단계도 같은 비율로 올린다. 잔량이 줄어도 게이지 UI 자체는 줄이지 않고 표시만 한다.”
- 결과물: 기본 최대 먹 용량을 `3.2m → 4.8m`로 올리고 기존 +37.5% 네 단계를
  유지해 `4.8 → 6.6 → 8.4 → 10.2 → 12.0m` 진행을 만들었다. 튜닝 버전을 3으로
  올려 기존 `3.2m/version 2` 씬도 자동 변환한다. 하단 게이지는 최대 용량 기준의
  바깥 붓획 실루엣과 폭을 고정하고, 안쪽 불투명 잔량만 왼쪽부터 감소·회복한다.
- 사람의 수정/검토 내용: 성장 배율과 날씨 배율의 합성 순서는 유지하고 기본값만 1.5배
  조정했다. README·기획·아키텍처·성장 문서와 단계별 회귀 기대값을 함께 갱신했다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-07 — 영구 성장 결실 패시브 9종 전면 교체

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 코드·문서·테스트 감사,
  ripgrep, Unity Bee 응답 파일 기반 Runtime·Editor Roslyn 정적 컴파일
- 목적: 각 계보의 마지막 결실을 단순 추가 수치가 아니라 빌드 방향을 결정하는
  고유 패시브로 통일하고, 이전 결실의 숨은 효과와 기획 문서 불일치를 제거한다.
- 주요 프롬프트/지시: “넓은 벼루는 최대 먹 게이지 2배, 가는 붓끝은 황금 붓
  획득 시 방어막, 마르는 먹은 회복 증가, 고른 박자는 점프 아이템 종료 뒤 방어막,
  돋는 먹발은 벽 부착, 높은 먹발은 2단점프, 숨 고르기는 본체 1회 부활,
  먹피는 분신 체력 증가, 먹떼는 분신 생성 +1로 바꾸고 문서도 전부 수정한다.”
- 결과물: 39개 stable node ID와 기존 구매·선택 저장은 유지하면서 새 효과 enum을
  뒤에 추가했다. 먹 운용은 일반 최대 용량 10.2m 뒤 결실 적용 시 20.4m, 황금 붓
  획득 개체의 즉시 방어막, 회복 +40%로 정리했다. 도약은 준비 -4.5% 뒤 점프 아이템
  상승 종료 방어막, 점프 힘 +4% 뒤 1.2초 벽 매달림, 높이 +4.6875% 뒤 40% 힘의
  2단점프·먹떼 공용 12초로 정리했다. 생존은 본체 1→4칸 뒤 분신 1→2칸,
  피격 유예 +0.16초 뒤 본체 장애물·추락 공용 1회 부활, 수평 밀림 82→64% 뒤
  먹분신 아이템 생성 1→2마리로 정리했다. README·CLAUDE·프로젝트 요약·아키텍처·
  영구 성장 설계·밸런스 주석과 회귀 테스트 계약을 같은 기준으로 갱신했다.
- 사람의 수정/검토 내용: 결실 노드 ID는 바꾸지 않아 기존 해금 기록을 보존하고,
  실제 판에는 각 계보에서 선택한 한 갈래만 적용한다. 황금 붓 방어막은 사용자 문구대로
  획득 즉시, 먹물방울 방어막은 상승 종료 뒤 획득 개체에게 적용한다. 이전의 선명 유지,
  짧은 획 추가 할인, 그린 먹선 보너스, 낙하 완화, 피격 후 방어막은 중립화했다.
  Unity Play와 전체 Test Runner는 실행하지 않고 Runtime·Editor 정적 컴파일로 확인한다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-07 — 높은 먹발 매 자동점프 2단도약

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 2단점프 제약 감사, ripgrep,
  Unity Bee 응답 파일 기반 Runtime·Editor Roslyn 정적 컴파일
- 목적: 높은 먹발 결실을 열었는데 먹떼 공용 12초 대기와 대표 예약 때문에 2단점프가
  간헐적으로만 보이던 문제를 제거한다.
- 주요 프롬프트/지시: “2단점프를 찍으면 한 번씩이 아니라 계속 2단점프해야 한다.”
- 결과물: 먹떼 공용 쿨다운·대표 전용 예약·단일 전역 사용권을 삭제하고, 본체와 모든
  분신이 각자의 일반 자동 점프마다 첫 정점에서 힘 40%의 2단점프를 정확히 한 번
  사용하도록 바꿨다. 먹물방울·풍맥 같은 특수 상승은 현재 비행의 예약을 취소하며
  다음 일반 자동 점프부터 다시 정상 발동한다. 카탈로그·성장 설명·README·기획·
  아키텍처 문서와 연속 두 점프·분신 독립 사용 회귀 계약을 함께 갱신했다.
- 사람의 수정/검토 내용: 한 비행에서 두 번 이상 연쇄 발동하지 않는 로컬 사용 상태는
  유지하고, 간헐성의 원인이던 시간·대표·먹떼 공용 제한만 제거했다. 전체 Test Runner는
  사용자 요청 전까지 실행하지 않고 정적 컴파일로 우선 확인한다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-07 — 2단점프 연속감 보정과 Editor 전 특성 적용

- 사용 도구: OpenAI Codex 읽기 전용 물리 타이밍 감사, ripgrep,
  Unity Bee 응답 파일 기반 Runtime·Editor Roslyn 정적 컴파일
- 목적: 2단점프가 실제 정점에서 속도 0에 가까워진 뒤 발동해 `점프→정지→점프`로
  보이는 문제를 없애고, 사용자가 바로 전체 특성 상호작용을 확인할 수 있게 한다.
- 주요 프롬프트/지시: “점프 후 바로 점프하는 느낌으로 바꾸고, 일단 특성을 다
  해금해 테스트하게 하라. Unity 조작 없이 코드만 수정하라.”
- 결과물: 첫 자동 점프의 수직 속도가 15% 남은 정점 직전에 40% 2단도약을 이어
  정지 구간을 제거했다. 모바일 저프레임에서 임계점을 건너뛰어도 한 번 발동하고,
  같은 비행의 중복 발동과 특수 상승 발동은 계속 차단한다. Editor Play에서는 실제
  저장을 바꾸지 않고 모든 일반 노드와 결실을 동시에 적용하는 임시 테스트 override를
  추가했다. 출시 빌드에는 이 override가 포함되지 않는다.
- 사람의 수정/검토 내용: 정점 높이 밸런스와 40% 두 번째 점프 힘은 유지하고 발동
  시점만 앞당겼다. 전체 Test Runner는 사용자 요청 전까지 실행하지 않고 정적
  컴파일과 타이밍 회귀 계약만 갱신한다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-07 — 숨 고르기 부활 50m 먹물 상승

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 부활 흐름 감사, 프로젝트
  `docs/VFX/SKILL.md` 모바일 VFX 기준, ripgrep, Unity Bee 응답 파일 기반
  Runtime·Editor Roslyn 정적 컴파일
- 목적: 숨 고르기 결실의 1회 부활을 단순 체력 복구가 아니라 즉시 알아볼 수 있는
  생존 전환과 강한 상승 기회로 만든다.
- 주요 프롬프트/지시: “부활 패시브를 적용하면 부활할 때 이펙트와 함께 50m 점프하면서
  부활하게 한다.”
- 결과물: 본체가 장애물 또는 추락으로 치명 피해를 받을 때 공용 사용권을 한 번 소모해
  체력 1로 복귀하고, 기존 먹물방울의 검증된 50m 물리 상승과 최대 3묶음 공유 VFX 풀을
  재사용한다. 추락은 먼저 카메라 하단 안쪽 안전선으로 이동한 뒤 상승하며 점프 아이템
  종료 방어막은 잘못 예약하지 않는다. 성장 상세·상단 요약·README·기획·아키텍처와
  장애물·추락 회귀 계약도 같은 50m 규칙으로 갱신했다.
- 사람의 수정/검토 내용: 분신은 기존대로 부활 대상과 사용권 소비에서 제외했다. 새
  파티클·셰이더·외부 에셋을 추가하지 않고 이미 품질 단계와 풀 상한이 검증된 먹물 상승
  연출을 재사용해 모바일 오버드로와 런타임 할당 증가를 피했다. 전체 Test Runner는
  사용자 요청 전까지 실행하지 않고 정적 컴파일로 확인한다.
- 외부 에셋/라이선스: 새 외부 에셋 없음. 기존 프로젝트 제작 먹물방울 VFX만 재사용했다.

### 2026-08-07 — Editor 전 특성 해금과 선택 적용 복구

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 성장 해금·선택 흐름 감사,
  ripgrep, Unity Bee 응답 파일 기반 Runtime·Editor Roslyn 정적 컴파일
- 목적: 테스트를 위해 모든 성장 열매를 바로 선택할 수 있게 하되, 실제 판에는 사용자가
  고른 생존·도약·먹 운용의 한 줄기씩만 적용되는 원래 빌드 규칙을 유지한다.
- 주요 프롬프트/지시: “전부 동시 적용이 아니라 전부 해금만 하고, 활성화한 특성만
  적용되게 하라.”
- 결과물: Editor Play에서 모든 39개 노드는 해금된 상태로 표시한다. 노드를 눌러 고른
  A/B/C 줄기는 계보별 세션 선택값으로만 보관하고, 런 스냅샷은 그 선택 줄기와 공용
  뿌리만 적용한다. 기존 선택도 세 줄기 규칙 그대로 유지하며 선택이 없는 계보는 자동으로
  활성화하지 않는다. 테스트 선택은 실제 성장 저장·먹빛·해금 기록을 변경하지 않으며
  출시 빌드에는 이 미리보기 해금이 포함되지 않는다.
- 사람의 수정/검토 내용: 직전의 `모든 경로 동시 적용` 해석을 철회하고, 기존의 계보별
  단일 활성 줄기·비활성 줄기 시각 규칙을 그대로 복구했다. 전체 Test Runner는 사용자
  요청 전까지 실행하지 않고 정적 컴파일로 확인한다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-07 — 성장 상세 비용 아이콘 잔상·대비 정리

- 사용 도구: OpenAI Codex 이미지 확인, 멀티 에이전트 읽기 전용 성장 팝업 감사,
  ripgrep, Unity Bee 응답 파일 기반 Runtime·Editor Roslyn 정적 컴파일
- 목적: 성장 상세 정보판 하단에서 먹빛 아이콘과 숫자가 작게 겹치거나, 해금 상태 계산
  전에 비용 조각이 잠깐 남아 보이는 문제를 없앤다.
- 주요 프롬프트/지시: 성장 상세창 하단 먹빛 비용 표시 스크린샷을 기준으로 수정한다.
- 결과물: 비용 세 요소는 생성 직후 기본 비활성으로 두고 미해금 노드에서만 한꺼번에
  표시한다. 해금·적용 중 노드에서는 완전히 숨기며, 표시가 필요한 경우에는 밝은 받침
  위 검은 먹방울과 더 큰 숫자 사이 간격을 확보해 모바일에서도 읽히게 정렬했다.
- 사람의 수정/검토 내용: 성장 저장·가격·선택 로직은 바꾸지 않고 표시 계층과 대비만
  조정했다. 전체 Test Runner는 사용자 요청 전까지 실행하지 않고 정적 컴파일로 확인한다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-07 — 성장 상단 요약 Safe Area 고정

- 사용 도구: OpenAI Codex 이미지 확인, 멀티 에이전트 읽기 전용 모바일 Safe Area
  레이아웃 감사, ripgrep, Unity Bee 응답 파일 기반 Runtime·Editor Roslyn 정적 컴파일
- 목적: 세로로 긴 iPhone에서 성장 요약판이 노치 아래보다 과도하게 내려오고,
  `노드 초기화`와 세 계보 정보가 패널 가장자리에 붙어 보이는 문제를 없앤다.
- 주요 프롬프트/지시: “성장 상단 UI를 위에 고정하고 어떤 기기에서도 화면이나
  정보판 밖으로 삐져나오지 않게 한다.”
- 결과물: 1080×1920 고정 화면의 가운데 맞춤은 유지하되 성장 상단 정보 묶음만
  기기 Safe Area 상단에서 일정 간격으로 고정했다. 초기화 버튼을 넓히고 안쪽으로
  이동했으며 생존·도약·먹 운용 요약 열에도 좌우 안전 여백을 확보했다. 로비 버튼은
  같은 비율 축소의 영향을 받지 않고 Safe Area 좌하단에 고정된다.
- 사람의 수정/검토 내용: 상세·복구 팝업의 공통 Safe Area 배치는 변경하지 않았고,
  성장 저장·밸런스·노드 선택 로직에도 손대지 않았다. 전체 Test Runner는 사용자 요청
  전까지 실행하지 않고 정적 컴파일로 확인한다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-07 — 돋는 먹발 좌우 벽 상시 자동점프

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 벽 충돌·자동점프 흐름 감사,
  ripgrep, Unity Bee 응답 파일 기반 Runtime·Editor Roslyn 정적 컴파일
- 목적: 돋는 먹발 결실이 특정 하강 상황의 대표 먹방울에게만 간헐적으로 발동하던
  제한을 없애고, 좌우 벽을 일반 발판처럼 일관되게 활용할 수 있게 한다.
- 주요 프롬프트/지시: “벽에 붙는 패시브는 상시로, 좌우 벽에 붙었다가 바닥처럼
  똑같은 자동점프로 튀어 올라오게 한다.”
- 결과물: 결실을 선택한 본체와 모든 분신은 상승·하강·특수 상승·먹떼 대표 여부와
  관계없이 좌우 벽에 붙는다. 벽 접촉을 새 착지로 정리하고 기존 `AutoJump`의 충전,
  점프 힘, 벽 안쪽 노멀을 그대로 사용해 화면 안쪽으로 다시 뛴다. 최대 1.2초 만료와
  비행당 1회 제한은 제거하고, 벽을 떠난 직후 0.35초 재부착 지연만 물리 떨림 방지용으로
  유지했다. 성장 상세·상단 요약·README·기획·아키텍처 문서도 같은 규칙으로 갱신했다.
- 사람의 수정/검토 내용: 패시브가 없는 캐릭터의 기존 강한 벽 트램펄린과 화면 가장자리
  먹선 금지 띠는 유지했다. 새 벽 전용 점프 힘을 추가하지 않고 검증된 일반 자동점프
  경로를 재사용했다. 전체 Test Runner는 사용자 요청 전까지 실행하지 않고 정적 컴파일과
  회귀 계약으로 확인한다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-07 — 결실 9종 실행 검증과 2단점프 경계 보정

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 결실 소비처 감사, Unity
  6000.3.10f1 Test Runner EditMode, Unity Bee 응답 파일 기반 Runtime·Editor
  Roslyn 정적 컴파일
- 목적: 생존·도약·먹 운용의 결실 9종이 현재 선택 스냅샷에서 실제 체력·아이템·
  이동·먹 자원 소비처까지 이어지는지 각각 한 번 이상 실행 검증한다.
- 주요 프롬프트/지시: “성장 관련 결실 패시브들을 한 번씩 전부 테스트한다.”
- 결과물: 먹피, 숨 고르기, 먹떼, 고른 박자, 돋는 먹발, 높은 먹발, 넓은 벼루,
  가는 붓끝, 마르는 먹의 대상 회귀 13케이스를 실행했다. 첫 실행에서 높은 먹발이
  Rigidbody2D 속도 왕복의 미세 오차 때문에 정확한 15% 경계에서 발동하지 않는 문제를
  발견해 0.01 속도 허용치를 추가했다. 수정 뒤 13/13 통과했다.
- 사람의 수정/검토 내용: 2단점프 힘 40%와 발동 시점 15%는 바꾸지 않고 물리 경계의
  부동소수점 오차만 흡수했다. 다른 결실의 밸런스·선택 규칙·저장 데이터는 변경하지
  않았다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-07 — Unity Recorder 제출 영상 자동 시나리오

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 게임·UI·Recorder API 감사,
  ripgrep, Unity 6000.3.10f1 에디터 컴파일
- 목적: 30~60초 심사 영상을 반복 촬영할 때 로비·성장·드로잉·분신·아이템·장애물·
  게임오버가 매번 같은 순서와 시간으로 재현되게 한다.
- 주요 프롬프트/지시: “시나리오대로 움직일 수 있게 게임씬을 변형하고 Unity Recorder
  시스템으로 녹화한다.”
- 결과물: 다음 Play 한 번만 동작하는 Editor 전용 Recording Director와 촬영 창을
  추가했다. 실제 로비/성장 전환, StrokeCapture의 비용·스무딩·사운드 경로, 아이템 효과,
  먹분신, 해태·풍맥, 실제 사망·게임오버를 약 50초 타임라인으로 자동 재생한다. 자막과
  팀 엔드카드도 1080×1920 Canvas 기준으로 함께 출력한다.
- 사람의 수정/검토 내용: 프로덕션 Main 씬과 씬 빌더는 수정하지 않았다. 촬영 Play는
  메모리 Score·성장·옵션 저장소를 써 실제 사용자 데이터와 튜토리얼 상태를 보호한다.
  Recorder 패키지는 기존 dirty manifest를 건드리지 않도록 자동 설치하지 않고 설치명과
  권장 설정만 촬영 창·가이드에 제공했다.
- 외부 에셋/라이선스: 새 외부 에셋 없음. 기존 먹붓 UI, 수묵 폰트, 게임 VFX·SFX만
  재사용했다.

### 2026-08-07 — 옵션 연출 시나리오 4종

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 시작 흐름·게임플레이 API·모바일
  옵션 레이아웃 감사, ripgrep, Unity Bee 응답 파일 기반 Runtime·Editor 정적 컴파일
- 목적: 제출 영상과 시연에서 로비 조작 직후 분신·아이템·날씨·해태·고도별 맵을
  서로 다른 조합으로 빠르게 재현한다.
- 주요 프롬프트/지시: “옵션의 디버그 창에 상황 1~4를 넣고, 상황 1은 시작하자마자
  분신 10마리, 상황마다 맵과 업그레이드도 다르게 만든다.”
- 결과물: Editor·Development Build 옵션에 Safe Area 대응 전용 상황 선택 페이지를
  추가했다. 바람 능선 먹떼 10마리, 먹비 계곡 황금 붓, 검은 절벽 해태, 월련 성해
  2단도약을 실제 시작·아이템·날씨·장애물 API로 구성하고 선택한 성장 빌드는 해당 판의
  런 스냅샷에만 적용한다.
- 사람의 수정/검토 내용: 기존 옵션에 버튼을 빽빽하게 늘리지 않고 별도 페이지로
  분리했다. 모든 시나리오 판은 기록·먹빛 정산에서 제외하고 PlayerPrefs 영구 성장과
  튜토리얼 저장을 수정하지 않도록 경계를 확인했다. 일반 출시 빌드에서는 메뉴와 실행
  컴포넌트를 생성하지 않는다.
- 외부 에셋/라이선스: 새 외부 에셋 없음. 기존 수묵 UI와 게임 내 맵·아이템·해태·풍맥
  연출을 재사용했다.

### 2026-08-07 — 천하수 먹떼 연쇄 시나리오

- 사용 도구: OpenAI Codex, ripgrep, Unity Bee 응답 파일 기반 Runtime·Editor 정적 컴파일
- 목적: 시작 직후 서로 다른 우주 수묵 맵에서 먹물방울과 분신 획득이 반복되는 대규모
  먹떼 장면을 재현한다.
- 주요 프롬프트/지시: “아예 다른 맵에서 50m 상승을 세 번 먹고, 중간중간 분신이
  생겨 10마리 넘게 함께 플레이하며 다시 분신과 점프가 이어지는 상황을 추가한다.”
- 결과물: 1510m 천하수에서 2마리로 시작해 먹물방울 3회와 먹분신 획득 5회를
  교차 실행하고 최종 12마리까지 확장되는 상황 5를 추가했다. 기존 실제 아이템·먹떼
  상승·분신 도착 연출을 재사용하며 해당 판은 기록과 영구 보상에서 제외한다.
- 사람의 수정/검토 내용: 반복 상승 중 예상치 못한 사망을 줄이기 위해 짧은 보호 시간을
  주고, 화면 가장자리로 아이템 분신 생성이 실패하면 다른 생존자의 빈 슬롯을 찾아
  목표 인원을 보완하도록 제한했다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.

### 2026-08-07 — 먹분신 2칸 체력과 팝콘형 생성 탄성

- 사용 도구: OpenAI Codex 멀티 에이전트 읽기 전용 분신 체력·카메라·특수 상승
  경계 감사, ripgrep, Unity Bee 응답 파일 기반 Runtime·Editor 정적 컴파일
- 목적: 먹분신의 생존 여유를 기본 2칸으로 높이고, 여러 분신이 같은 높이와 속도로
  일렬 정렬되는 모습을 좌우로 튀는 활기 있는 먹떼 움직임으로 바꾼다.
- 주요 프롬프트/지시: “분신 체력은 2로 하고, 일렬로 딱딱 서지 않게 팝콘처럼
  팡팡 튀게 한다.”
- 결과물: 모든 새 분신은 기본 2/2 체력으로 생성되며 먹피 결실 선택 시 3/3이 된다.
  생성 위치의 바깥 방향으로 수평 탄성을 주고 인덱스별 상향 변주를 적용해 연쇄 생성도
  서로 다른 궤적으로 퍼진다. 일반 상승은 세대별 속도 누적을 18m/s로 제한하고,
  먹물방울·풍맥·추락 복귀·부활 같은 특수 상승은 기존 수직 속도를 그대로 보존해
  새 분신만 카메라를 끌고 가지 않게 했다. 상황 5는 순간이동 직후 첫 상승을 바로
  시작하도록 바꿔 맵 전환 대기 중 추락 위험도 제거했다.
- 사람의 수정/검토 내용: 저장 노드 ID와 먹피 결실의 +1 효과는 유지해 마이그레이션
  없이 기존 저장과 호환했다. 수평 변주는 난수 대신 분신 순번으로 계산해 영상 촬영과
  회귀 테스트에서 동일한 움직임을 재현한다. 체력 Billboard와 피격 확대는 기존 동적
  3칸 지원 경로를 재사용했다.
- 외부 에셋/라이선스: 새 외부 에셋 없음.
