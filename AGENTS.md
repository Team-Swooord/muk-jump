...
- **2026-08-03 사용자 결정: 브랜치는 `main` 하나만 유지한다.** UI·게임·문서 모두
  동일한 Unity 프로젝트의 `main`에서 직접 작업·커밋·push한다. 사용자가 다시 정책을
  바꾸기 전에는 새 브랜치, 추가 clone, 추가 worktree를 만들지 않는다.
- 커밋: Conventional Commits, 한국어 본문 (`feat(drawing): ...`, `fix(player): ...`).
  기능 단위로 잘게 나눠 커밋.
- 완료한 기능은 명시적으로 해당 파일만 stage하고 `main`에 커밋한 뒤 원격 `main`으로
  push한다. 세부 규칙은 `docs/branch-policy.md`를 따른다.
